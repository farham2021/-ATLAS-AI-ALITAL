...
def _daily16_management_png(kind, results, scoped_results, portfolio_risk=None, filename=None):
    """Dense DAILY16 decision PNGs built only from already-computed ATLAS state.

    1) MARKET 4H + Top10/Personal Deep Analysis
    2) Best Watch + Opportunity Ranking + canonical Trade Plans
    3) Personal Portfolio 4H + Signal Lifecycle + human-readable risk
    Full raw/history remains in Supabase; these PNGs are the human decision surface.
    """
    try:
        import matplotlib.pyplot as plt
        from matplotlib.patches import Rectangle
        ...
        dt=now_tehran(); ranked=_atlas_visual_rank(results or [])

        # Dense landscape board; no giant empty canvas.
        fig=plt.figure(figsize=(18,10),facecolor=bg)
        ax=fig.add_axes([.025,.065,.95,.855]); ax.axis("off")
        ...
        if kind==1:
            ...
            btc_h4=str(_daily16_row_metric(btc,("h4_trend","trend_h4"),"N/A"))
            btc_d1=str(_daily16_row_metric(btc,("d1_trend","trend_d1"),"N/A"))
            ax.text(.01,.965,f"Breadth {adv}↑  {dec}↓  {flat}→   •   BTC H4 {btc_h4} / D1 {btc_d1}",
                    transform=ax.transAxes,color=fg,fontsize=15,weight="bold",va="top")
            cols=[.015,.105,.19,.265,.345,.425,.505,.585,.675,.995]
            headers=["ASSET","DECISION","PRICE","4H / 1D","RSI","MTF","CONF","OPP","VOLUME / BLOCKER"]
            for x,h in zip(cols[:-1],headers):
                ax.text(x,.905,h,transform=ax.transAxes,color=blue,fontsize=9.5,weight="bold")
            y=.865
            # 10-15 real rows: Top10 + Personal ranked state, not only two WAITs.
            for r in ranked[:14]:
                sym=_aio_symbol(r); sig=_atlas_public_signal(r)
                px=safe_float(r.get("price")); conf=max(_aio_num(r.get("confidence"),0),_aio_num(r.get("decision_confidence"),0))
                opp=max(_aio_num(r.get("opportunity_score"),0),_aio_num(r.get("score"),0))
                h4=str(_daily16_row_metric(r,("h4_trend","trend_h4"),"N/A")); d1=str(_daily16_row_metric(r,("d1_trend","trend_d1"),"N/A"))
                rsi=_daily16_row_metric(r,("rsi14","rsi","rsi_14"),None); mtf=_aio_num(_daily16_row_metric(r,("mtf_agreement_pct","mtf_pct","mtf"),0),0)
                vr=_daily16_row_metric(r,("volume_ratio","vol_ratio","volume_ratio_4h"),None)
                blockers=why_not_trade(r)[:1] if sig=="WAIT" else []
                tail=(f"Vol {float(vr):.2f}x" if safe_float(vr) is not None else "Vol N/A")
                if blockers: tail += " • " + str(blockers[0])
                ax.add_patch(Rectangle((.005,y-.025),.99,.052,transform=ax.transAxes,facecolor=panel if int((.865-y)/.057)%2==0 else bg,edgecolor="#19384e",linewidth=.6))
                col=green if sig=="BUY" else red if sig=="SELL" else amber
                vals=[sym,sig,fmt(px) if px is not None else "N/A",f"{h4} / {d1}",f"{float(rsi):.1f}" if safe_float(rsi) is not None else "N/A",f"{mtf:.0f}%",f"{conf:.0f}%",f"{opp:.0f}",tail]
                xs=cols[:-1]
                for i,(x,v) in enumerate(zip(xs,vals)):
                    ax.text(x,y,str(v),transform=ax.transAxes,color=col if i==1 else fg if i<8 else muted,fontsize=8.6 if i<8 else 7.6,va="center",weight="bold" if i in (0,1) else "normal")
                y-=.057
            footer="MARKET 4H + Deep Analysis • full 27-row evidence/history and raw CSV remain in Supabase"

        elif kind==2:
            title="ATLAS • BEST WATCH + OPPORTUNITY RANKING + TRADE PLANS"
            confirmed=[r for r in ranked if _atlas_public_signal(r) in ("BUY","SELL")][:4]
            watch=_daily16_best_watch(results,10)
            plans=_daily16_trade_plan_rows(results,6)
            ax.text(.01,.96,"CONFIRMED ACTION",transform=ax.transAxes,color=blue,fontsize=13,weight="bold")
            if confirmed:
                x=.02
                for r in confirmed:
                    sig=_atlas_public_signal(r); col=green if sig=="BUY" else red
                    ax.text(x,.915,f"{_aio_symbol(r)} • {sig} • C {max(_aio_num(r.get('confidence'),0),_aio_num(r.get('decision_confidence'),0)):.0f}%",transform=ax.transAxes,color=col,fontsize=11,weight="bold"); x+=.23
            else:
                ax.text(.02,.915,"No confirmed BUY/SELL — ranking below remains decision-useful",transform=ax.transAxes,color=muted,fontsize=10.5)
            ...
            xs=[.015,.055,.14,.215,.275,.345,.42,.56]
            for x,h in zip(xs,hdr): ax.text(x,.81,h,transform=ax.transAxes,color=muted,fontsize=9,weight="bold")
            y=.77
            for i,r in enumerate(watch[:10],1):
                score=max(_aio_num(r.get("opportunity_score"),0),_aio_num(r.get("score"),0)); conf=max(_aio_num(r.get("confidence"),0),_aio_num(r.get("decision_confidence"),0)); mtf=_aio_num(r.get("mtf_agreement_pct"),0)
                ...
                if not trigger:
                    why=why_not_trade(r)[:1]; trigger=str(why[0]) if why else "Await confirmed trigger + acceptable RR"
                vals=[i,_aio_symbol(r),_atlas_public_signal(r),f"{score:.0f}",f"{conf:.0f}%",f"{mtf:.0f}%",setup,trigger]
                ax.add_patch(Rectangle((.005,y-.022),.99,.046,transform=ax.transAxes,facecolor=panel if i%2 else bg,edgecolor="#19384e",linewidth=.5))
                for x,v in zip(xs,vals): ax.text(x,y,str(v),transform=ax.transAxes,color=fg if x<.56 else muted,fontsize=8.4,va="center")
                y-=.048

            plan_y=.25
            ax.text(.01,plan_y,"CANONICAL EXECUTABLE TRADE PLANS",transform=ax.transAxes,color=blue,fontsize=13,weight="bold")
            plan_y-=.045
            if not plans:
                ax.text(.02,plan_y,"No executable BUY/SELL Entry / SL / TP plan. WAIT research levels are not promoted.",transform=ax.transAxes,color=muted,fontsize=10)
            else:
                for r in plans:
                    sig=_atlas_public_signal(r); col=green if sig=="BUY" else red
                    rr=_aio_num(_daily16_row_metric(r,("rr","risk_reward","risk_reward_ratio"),0),0)
                    ax.text(.02,plan_y,f"{_aio_symbol(r)} • {sig} | Entry {fmt(r.get('entry'))} | SL {fmt(r.get('sl'))} | TP1 {fmt(r.get('tp1'))} | RR {rr:.2f}",transform=ax.transAxes,color=col,fontsize=10.2,weight="bold"); plan_y-=.043
            footer="Best Watch + Opportunity Ranking + canonical Trade Plans • WAIT never becomes BUY/SELL by presentation logic"

        else:
            title="ATLAS • PERSONAL PORTFOLIO 4H + SIGNAL LIFECYCLE"
            holdings=_daily16_desk_holdings_for_chat(TELEGRAM_CHAT_ID)
            by={_aio_symbol(r).upper():r for r in (scoped_results or [])}
            # Also map common metal aliases so personal holdings can resolve against analysis rows.
            for r in (results or []): by.setdefault(_aio_symbol(r).upper(),r)
            ax.text(.01,.96,"PERSONAL PORTFOLIO 4H",transform=ax.transAxes,color=blue,fontsize=13,weight="bold")
            hdr=["ASSET","QTY","PRICE","DEC","H4 / D1","CONF","OPP","ENTRY / SL / TP"]
            xs=[.015,.11,.20,.30,.375,.49,.56,.63]
            for x,h in zip(xs,hdr): ax.text(x,.915,h,transform=ax.transAxes,color=muted,fontsize=9,weight="bold")
            y=.875
            shown=0
            for sym,qty in list((holdings or {}).items())[:12]:
                key=str(sym).upper(); r=by.get(key,{})
                px=safe_float(r.get("price")) if r else None; sig=_atlas_public_signal(r) if r else "N/A"
                h4=str(_daily16_row_metric(r,("h4_trend","trend_h4"),"N/A")) if r else "N/A"; d1=str(_daily16_row_metric(r,("d1_trend","trend_d1"),"N/A")) if r else "N/A"
                conf=max(_aio_num(r.get("confidence"),0),_aio_num(r.get("decision_confidence"),0)) if r else 0; opp=max(_aio_num(r.get("opportunity_score"),0),_aio_num(r.get("score"),0)) if r else 0
                plan="—"
                if r and _atlas_public_signal(r) in ("BUY","SELL") and bool(r.get("executable")):
                    plan=f"{fmt(r.get('entry'))} / {fmt(r.get('sl'))} / {fmt(r.get('tp1'))}"
                vals=[sym,fmt(qty),fmt(px) if px is not None else "N/A",sig,f"{h4} / {d1}",f"{conf:.0f}%" if r else "N/A",f"{opp:.0f}" if r else "N/A",plan]
                ax.add_patch(Rectangle((.005,y-.022),.99,.046,transform=ax.transAxes,facecolor=panel if shown%2==0 else bg,edgecolor="#19384e",linewidth=.5))
                for x,v in zip(xs,vals): ax.text(x,y,str(v),transform=ax.transAxes,color=fg,fontsize=8.3,va="center")
                y-=.048; shown+=1
            if not shown:
                ax.text(.02,.86,"No Desk holdings resolved for this chat; no portfolio values fabricated.",transform=ax.transAxes,color=muted,fontsize=10.5)
                y=.79

            ax.text(.01,max(.25,y-.015),"SIGNAL LIFECYCLE • IMPORTANT CURRENT STATES",transform=ax.transAxes,color=blue,fontsize=12.5,weight="bold")
            ly=max(.20,y-.06)
            for sym,sig,life in _daily16_signal_lifecycle_summary(results,8):
                col=green if sig=="BUY" else red if sig=="SELL" else amber
                ax.text(.02,ly,f"{sym} • {sig} • {life}",transform=ax.transAxes,color=col,fontsize=9.4,weight="bold"); ly-=.038
                if ly<.08: break

            # Human-readable risk only; never dump a raw Python dict into Telegram.
            if isinstance(portfolio_risk,dict):
                op=safe_float(portfolio_risk.get("open_risk_pct")); cap=safe_float(portfolio_risk.get("max_open_risk_pct")); status=str(portfolio_risk.get("risk_status") or portfolio_risk.get("status") or "N/A")
                longp=safe_float(portfolio_risk.get("long_risk_pct")); shortp=safe_float(portfolio_risk.get("short_risk_pct"))
                risk_parts=[]
                if op is not None:risk_parts.append(f"Open risk {op:.2f}%")
                if cap is not None:risk_parts.append(f"Cap {cap:.2f}%")
                if longp is not None:risk_parts.append(f"Long {longp:.2f}%")
                if shortp is not None:risk_parts.append(f"Short {shortp:.2f}%")
                risk_parts.append(f"Status {status}")
                ax.text(.55,.06,"RISK CONTEXT  •  "+"  |  ".join(risk_parts),transform=ax.transAxes,color=amber,fontsize=8.8,weight="bold")
            footer="Personal Portfolio 4H + lifecycle • full portfolio/risk/lifecycle history remains in Supabase"

        fig.text(.025,.965,title,color=fg,fontsize=20,weight="bold",va="top")
        fig.text(.975,.965,f"{dt.strftime('%Y-%m-%d %H:%M')} Tehran",ha="right",color=muted,fontsize=9.5,va="top")
        fig.text(.025,.018,footer,color=amber,fontsize=8.5,weight="bold")
        filename=filename or f"atlas_daily16_management_{kind}.png"
        plt.savefig(filename,dpi=240,bbox_inches="tight",facecolor=fig.get_facecolor(),pad_inches=.08)
        plt.close(fig)
        return filename
    except Exception as e:
...
def send_daily16_management_pngs(results, scoped_results, portfolio_risk=None):
    """Send the 3 agreed management PNGs before the 3 mandatory core dashboards."""
    sent=0;errs=[]
    captions={
        1:"🧠 ATLAS • Market + Deep Analysis",
        2:"🎯 ATLAS • Decision + Opportunity + Trade Plans",
        3:"🛡 ATLAS • Personal Portfolio + Signal Lifecycle",
    }
    for kind in (1,2,3):
        fn=_daily16_management_png(kind,results,scoped_results,portfolio_risk,
                                   f"atlas_daily16_management_{kind}.png")
        if not fn:
            errs.append(f"MANAGEMENT_PNG_{kind}: generation failed");continue
        for c in dict.fromkeys([x for x in (TELEGRAM_CHAT_ID,TELEGRAM_GROUP_CHAT_ID) if x]):
            try:
                if _telegram_send_photo_file(c,fn,captions[kind]):sent+=1
                else:errs.append(f"MANAGEMENT_PNG_{kind}[{c}]: send returned false")
            except Exception as e:errs.append(f"MANAGEMENT_PNG_{kind}[{c}]: {e}")
        try:os.unlink(fn)
        except Exception:pass
    return sent,errs
...
def _phase310_send_full_daily16(results, scoped_results, top10, macro, news, btc_regime, portfolio_risk, metal_results=None):
    ...
    # D) Three agreed MANAGEMENT PNGs. These are ADDITIVE and do not replace
    #    the three mandatory core dashboards sent later by main().
    try:

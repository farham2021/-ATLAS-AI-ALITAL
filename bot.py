...
def build_iran_macro_funds_dashboard(filename="atlas_iran_macro_funds.png"):
    ...
    try:
        ...
        for i,(label,sym,scale) in enumerate(specs):
            ...
            ax.text(x+.012,y+.255,label,transform=ax.transAxes,color=muted,fontsize=8,weight="bold")
            ...
            if ch is not None:
                ax.text(x+.012,y+.045,f"{ch:+.2f}%",transform=ax.transAxes,
                        color=green if ch>=0 else red,fontsize=8,weight="bold")
        ...
        for i,key in enumerate(order):
            ...
            ad.text(.03,y,key,transform=ad.transAxes,color=fg,fontsize=8)
            ad.text(.54,y,f"ADVISORY • {dec}",transform=ad.transAxes,color=col,fontsize=8,weight="bold")
            ch=safe_float(d.get("change_pct"))
        ...
        if not fs:
            ...
            af.text(.02,.94,"FUND",transform=af.transAxes,color=muted,fontsize=7,weight="bold")
            af.text(.55,.94,"PRICE",transform=af.transAxes,color=muted,fontsize=7,weight="bold")
            af.text(.72,.94,"CHANGE",transform=af.transAxes,color=muted,fontsize=7,weight="bold")
            af.text(.97,.94,"TRADE VALUE",ha="right",transform=af.transAxes,color=muted,fontsize=7,weight="bold")
            for i,x in enumerate(fs):
                y=.87-i*.066;pct=safe_float(x.get("pct"))
                af.text(.02,y,str(x.get("name") or x.get("symbol"))[:32],transform=af.transAxes,color=fg,fontsize=7.2)
                af.text(.55,y,_iran_fmt_price(x.get("price")),transform=af.transAxes,color=fg,fontsize=7.2)
                af.text(.72,y,f"{pct:+.2f}%" if pct is not None else "—",transform=af.transAxes,
                        color=green if (pct or 0)>0 else red if (pct or 0)<0 else muted,fontsize=7.2)
                af.text(.97,y,_iran_fmt_num(x.get("liquidity"),0),ha="right",transform=af.transAxes,color=muted,fontsize=7)
            af.text(.02,.035,"NAV is omitted when provider plan does not supply it • no fabricated premium/discount",
                    transform=af.transAxes,color=muted,fontsize=7)
...
def build_tse_intelligence_dashboard(filename="atlas_tse_intelligence.png"):
    ...
    try:
        ...
        for i,r in enumerate(stocks[:40]):
            ...
            ah.text(x+.006,y+.078,str(r.get("symbol"))[:10],transform=ah.transAxes,color=fg,fontsize=6.8,weight="bold")
            ah.text(x+.006,y+.025,f"{pc:+.2f}%" if pc is not None else "—",transform=ah.transAxes,
                    color=green if (pc or 0)>=0 else red,fontsize=6.6,weight="bold")
        ...
        ai.text(.05,.74,"BREADTH %",transform=ai.transAxes,color=muted,fontsize=8)
        facts=[
            f"Advance / Decline: {int(adv)} / {int(dec)}",
        ...
        for i,s in enumerate(facts):ai.text(.05,.62-i*.09,s,transform=ai.transAxes,color=fg,fontsize=8.5)
        ...
        ar.text(.02,.88,"INFLOW",transform=ar.transAxes,color=green,fontsize=8,weight="bold")
        ar.text(.52,.88,"OUTFLOW",transform=ar.transAxes,color=red,fontsize=8,weight="bold")
        for i,(n,r) in enumerate(top):
            y=.78-i*.11;ar.text(.02,y,str(r.get("symbol"))[:12],transform=ar.transAxes,color=fg,fontsize=8)
            ar.text(.42,y,_iran_flow_fmt(n),ha="right",transform=ar.transAxes,color=green,fontsize=8)
        for i,(n,r) in enumerate(bot):
            y=.78-i*.11;ar.text(.52,y,str(r.get("symbol"))[:12],transform=ar.transAxes,color=fg,fontsize=8)
            ar.text(.97,y,_iran_flow_fmt(n),ha="right",transform=ar.transAxes,color=red,fontsize=8)
        ...
        for i,r in enumerate(show[:12]):
            ...
            pc=safe_float((r.get("payload") or {}).get("pct"))
            am.text(x,y,str(r.get("symbol"))[:13],transform=am.transAxes,color=fg,fontsize=8)
            am.text(x+.42,y,f"{pc:+.2f}%",ha="right",transform=am.transAxes,
                    color=green if pc>=0 else red,fontsize=8,weight="bold")

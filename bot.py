...
VERSION = os.environ.get("ATLAS_VERSION", "ATLAS v11.5").strip()
...
def _metal_analysis(name):
    ...
    try:
        ...
        # Strong-alert context needs a real CLOSED D1 confirmation. This does
        # not change the existing H4 metal action/levels; it only adds evidence.
        d1_rows = yahoo_chart(symbol, "1d", "2y")
        d1_rows = strip_incomplete(d1_rows, "1d")
        d1_trend = trend_from_rows(d1_rows) if len(d1_rows) >= 60 else "UNKNOWN"
        h4_closed_ts = int(rows[-1][0]) if rows else None
        d1_closed_ts = int(d1_rows[-1][0]) if d1_rows else None
        session, session_label, session_multiplier = get_current_session()
        return {"coin":name,"price":price,"change":None,"h4_trend":trend,"d1_trend":d1_trend,"w1_trend":"UNKNOWN","rsi":rsi_v,"macd":macd_state,"atr_pct":atrp,"support":support,"resistance":resistance,"direction":direction,"action":action,"confidence":min(int(conf),100),"entry":(levels or {}).get("entry"),"sl":(levels or {}).get("sl"),"tp1":(levels or {}).get("tp1"),"tp2":(levels or {}).get("tp2"),"tp3":(levels or {}).get("tp3"),"tp4":(levels or {}).get("tp4"),"rr":rr,"reason":"روند 4H + MACD + ساختار قیمت","signal_candle_ts":h4_closed_ts,"d1_candle_ts":d1_closed_ts,"snapshots":{"4h":{"rows":rows},"1d":{"rows":d1_rows}},"session":session,"session_label":session_label,"session_multiplier":session_multiplier}
    except Exception as e:
...
def build_atlas_visual_dashboard(results, btc_regime=None, filename="atlas_dashboard.png"):
    ...
    try:
        ...
        mover_rows=[r for r in rows if any(r.get(k) is not None for k in ("change","change24","change_24h","pct_change_24h"))]
        gainers = sorted(mover_rows, key=_atlas_visual_change, reverse=True)[:5]
        losers = sorted(mover_rows, key=_atlas_visual_change)[:5]
        ...
        axl=panel(gs[9:14,7:14],"BOTTOM PERFORMERS (24H)"); axl.axis("off")
        ...
        if btc:
            ...
            if not btc.get("executable"):
                wr=why_not_trade(btc)[:2]
                if wr:insights.append("BTC WAIT: "+"; ".join(wr))
        breadth_bias="strong" if adv>dec*1.5 else "weak" if dec>adv*1.5 else "mixed"
        if breadth_bias=="strong" and regime in ("NEUTRAL","UNKNOWN"):
            insights.append("Breadth strong, but BTC trend is not confirmed.")
        elif breadth_bias=="weak" and regime not in ("BEARISH","BEAR","DOWN"):
            insights.append("Breadth weak; BTC regime has not confirmed downside.")
        else:
            insights.append(f"Breadth/regime synthesis: {breadth_bias} breadth • BTC {regime}.")
...
# Research synthesis upgrade: Trend baseline + causal structure + zone quality + anti-double-counting + SMC hypotheses + exit attribution
...
# ============================================================
# ATLAS RESEARCH SYNTHESIS — EVIDENCE-CALIBRATED SHADOW LAYER
# 2026-09 research implementation. Strictly advisory/shadow:
# - no canonical signal/action/confidence mutation
# - no Entry/SL/TP mutation
# - no execution/scheduler/lease mutation
# Purpose: measure incremental information before promotion to production.
# ============================================================
ATLAS_RESEARCH_SYNTHESIS_ENABLED = os.environ.get("ATLAS_RESEARCH_SYNTHESIS_ENABLED", "1").strip().lower() not in ("0","false","no","off")


def _atlas_rs_rows(r, tf="4h"):
    snap=(r.get("snapshots") or {}).get(tf) or {}
    rows=snap.get("rows") if isinstance(snap,dict) else None
    return rows if isinstance(rows,list) and len(rows)>=20 else None


def _atlas_rs_atr(rows, n=14):
    try:
        return float(atr(rows,n))
    except Exception:
        return None


def _atlas_rs_confirmed_pivots(rows, span=2):
    """Causal pivots: a pivot at i is usable only at i+span; no future leakage."""
    if not rows or len(rows)<2*span+5: return {"highs":[],"lows":[]}
    highs=[]; lows=[]
    for i in range(span,len(rows)-span):
        try:
            hi=float(rows[i][2]); lo=float(rows[i][3])
            wh=[float(x[2]) for x in rows[i-span:i+span+1]]
            wl=[float(x[3]) for x in rows[i-span:i+span+1]]
            if hi==max(wh): highs.append({"pivot_i":i,"confirmed_i":i+span,"price":hi})
            if lo==min(wl): lows.append({"pivot_i":i,"confirmed_i":i+span,"price":lo})
        except Exception: continue
    return {"highs":highs,"lows":lows}


def _atlas_rs_structure(r):
    rows=_atlas_rs_rows(r,"4h")
    if not rows: return {"available":False}
    piv=_atlas_rs_confirmed_pivots(rows,2); hs=piv["highs"]; ls=piv["lows"]
    if len(hs)<2 or len(ls)<2: return {"available":False}
    h1,h2=hs[-2],hs[-1]; l1,l2=ls[-2],ls[-1]
    state="BULLISH" if h2["price"]>h1["price"] and l2["price"]>l1["price"] else "BEARISH" if h2["price"]<h1["price"] and l2["price"]<l1["price"] else "MIXED"
    close=float(rows[-1][4]); prev=float(rows[-2][4]); a=_atlas_rs_atr(rows) or 0.0
    bos=None
    if prev<=h2["price"] and close>h2["price"]: bos="BULLISH_BOS"
    elif prev>=l2["price"] and close<l2["price"]: bos="BEARISH_BOS"
    displacement=abs(close-prev)/a if a>0 else None
    return {"available":True,"state":state,"bos":bos,"last_swing_high":h2["price"],"last_swing_low":l2["price"],"displacement_atr":None if displacement is None else round(displacement,3),"causal":True}


def _atlas_rs_trend_baseline(r):
    """Simple benchmark, intentionally not a trading signal."""
    rows=_atlas_rs_rows(r,"4h")
    if not rows: return {"available":False}
    c=[float(x[4]) for x in rows if len(x)>4]
    if len(c)<31: return {"available":False}
    def ret(n): return (c[-1]/c[-1-n]-1.0)*100.0 if len(c)>n and c[-1-n] else None
    r1,r7,r30=ret(1),ret(7),ret(30)
    vals=[x for x in (r1,r7,r30) if x is not None]
    score=sum(1 if x>0 else -1 if x<0 else 0 for x in vals)
    direction="UP" if score>=2 else "DOWN" if score<=-2 else "MIXED"
    a=_atlas_rs_atr(rows); atr_pct=(a/c[-1]*100.0) if a and c[-1] else None
    # Research-only inverse-volatility scalar, capped; never used for live sizing.
    vol_scalar=None
    if atr_pct and atr_pct>0: vol_scalar=round(max(0.25,min(2.0,2.5/atr_pct)),3)
    return {"available":True,"direction":direction,"vote":score,"ret_4h_pct":round(r1,3),"ret_28h_pct":round(r7,3),"ret_5d_pct":round(r30,3),"atr_pct":None if atr_pct is None else round(atr_pct,3),"volatility_scalar_shadow":vol_scalar}


def _atlas_rs_zone_quality(r):
    """Observable S/R-zone diagnostics; no claim of institutional orders."""
    rows=_atlas_rs_rows(r,"4h"); price=_aio_num(r.get("price"),0)
    if not rows or not price: return {"available":False}
    a=_atlas_rs_atr(rows) or 0.0; sup=_aio_num(r.get("support"),0); res=_aio_num(r.get("resistance"),0)
    level=sup if abs(price-sup)<=abs(price-res) and sup else res
    if not level: return {"available":False}
    tol=max(a*0.25,level*0.002) if a else level*0.003
    touches=0; last_touch=None
    for i,x in enumerate(rows[:-1]):
        try:
            if float(x[3])-tol<=level<=float(x[2])+tol:
                touches+=1; last_touch=i
        except Exception: pass
    age=(len(rows)-1-last_touch) if last_touch is not None else len(rows)
    distance_atr=abs(price-level)/a if a>0 else None
    return {"available":True,"nearest_type":"SUPPORT" if level==sup else "RESISTANCE","level":level,"touch_count":touches,"bars_since_touch":age,"fresh":touches==0,"distance_atr":None if distance_atr is None else round(distance_atr,3),"interpretation":"price_memory_hypothesis"}


def _atlas_rs_breakout_family(r, structure):
    """Aggregate correlated breakout evidence into ONE family to avoid double counting."""
    rows=_atlas_rs_rows(r,"4h")
    if not rows: return {"available":False}
    try:
        c=float(rows[-1][4]); o=float(rows[-1][1]); h=float(rows[-1][2]); l=float(rows[-1][3]); v=float(rows[-1][5] or 0)
        a=_atlas_rs_atr(rows) or 0.0
        body=abs(c-o); rng=max(h-l,1e-12); close_quality=(c-l)/rng
        vols=[float(x[5] or 0) for x in rows[-21:-1]]; av=sum(vols)/len(vols) if vols else 0
        vr=v/av if av>0 else None
        disp=structure.get("displacement_atr") if structure else None
        components={"bos":bool(structure.get("bos")) if structure else False,"displacement_atr":disp,"volume_ratio":None if vr is None else round(vr,3),"body_atr":round(body/a,3) if a>0 else None,"close_location":round(close_quality,3)}
        # Diagnostic 0-100; never added to canonical confidence.
        q=0.0
        q+=25 if components["bos"] else 0
        q+=min(25,max(0,(disp or 0)*20))
        q+=min(20,max(0,((vr or 1)-1)*20))
        q+=min(15,max(0,(components["body_atr"] or 0)*15))
        directional_close=max(close_quality,1-close_quality); q+=max(0,min(15,(directional_close-.5)*30))
        return {"available":True,"score_shadow":round(min(100,q),1),"components":components,"family":"BREAKOUT_QUALITY","double_count_guard":True}
    except Exception: return {"available":False}


def _atlas_rs_fvg_ob_hypotheses(r):
    """Mechanical ICT/SMC labels only; explicitly hypothesis/shadow-only."""
    rows=_atlas_rs_rows(r,"4h")
    if not rows or len(rows)<5: return {"available":False}
    out={"available":True,"bullish_fvg":False,"bearish_fvg":False,"order_block_candidate":None,"premium_discount":None,"research_only":True}
    try:
        a,b,c=rows[-3],rows[-2],rows[-1]
        out["bullish_fvg"]=float(a[2]) < float(c[3])
        out["bearish_fvg"]=float(a[3]) > float(c[2])
        body=float(b[4])-float(b[1])
        move=float(c[4])-float(b[4])
        if body<0 and move>0: out["order_block_candidate"]="BULLISH_CANDIDATE"
        elif body>0 and move<0: out["order_block_candidate"]="BEARISH_CANDIDATE"
        piv=_atlas_rs_confirmed_pivots(rows,2)
        if piv["highs"] and piv["lows"]:
            hi=piv["highs"][-1]["price"]; lo=piv["lows"][-1]["price"]
            if hi>lo:
                eq=(hi+lo)/2; px=float(rows[-1][4]); out["premium_discount"]="PREMIUM" if px>eq else "DISCOUNT" if px<eq else "EQUILIBRIUM"
                out["equilibrium"]=eq
    except Exception: pass
    return out


def _atlas_rs_exit_attribution(r):
    """Trade-lifecycle observability fields; counterfactual policies require later outcome candles."""
    entry=_aio_num(r.get("entry"),0); sl=_aio_num(r.get("sl"),0); price=_aio_num(r.get("price"),0)
    risk=abs(entry-sl) if entry and sl else 0
    live_r=None
    direction=str(r.get("direction") or "").upper()
    if risk and price:
        live_r=(price-entry)/risk if direction=="LONG" else (entry-price)/risk if direction=="SHORT" else None
    return {"planned_rr":r.get("rr"),"live_r_shadow":None if live_r is None else round(live_r,3),"policies_to_replay":["1R","1.5R","2R","3R","ATR","STRUCTURE","TRAIL_ATR","PARTIAL_RUNNER"],"requires_outcome_replay":True}


def _atlas_apply_research_synthesis(r):
    if not ATLAS_RESEARCH_SYNTHESIS_ENABLED: return r
    structure=_atlas_rs_structure(r); trend=_atlas_rs_trend_baseline(r); zone=_atlas_rs_zone_quality(r)
    breakout=_atlas_rs_breakout_family(r,structure); smc=_atlas_rs_fvg_ob_hypotheses(r)
    # Families are kept explicit so correlated subfeatures cannot cast independent votes.
    r["research_evidence_families_shadow"]={
        "TREND":trend,
        "STRUCTURE":structure,
        "LOCATION_LIQUIDITY":{"zone":zone,"smc_hypotheses":smc},
        "BREAKOUT_QUALITY":breakout,
        "EXECUTION_RISK":{"fractional_kelly_pct":r.get("fractional_kelly_pct_shadow")},
        "PERFORMANCE_ATTRIBUTION":_atlas_rs_exit_attribution(r),
    }
    r["research_shadow_only"]=True
    r["research_promotion_gate"]="WALK_FORWARD+ABLATION+OOS+COSTS"
    r["research_promotion_status"]="SHADOW_ONLY_NOT_PROMOTED"
    return r
...
def apply_phase34_research_shadow(results):
    for r in results or []:
        ...
        _atlas_apply_research_synthesis(r)
    return results
...
def build_phase311_advisory_intelligence(results, btc_regime=None):
    ...
    for r in results:
        ...
        if not rows:
            continue
        # Advisory is diagnostic-only: malformed/shape-shifted market data must never
        # fail DAILY16. Skip only the affected asset and keep the canonical report alive.
        try:
            ...
            for x in rows:
                if not isinstance(x, (list, tuple)) or len(x) <= 4:
                    continue
                v=safe_float(x[4], None)
                if v is not None:
                    close_vals.append(v)
            divs=_p311_swing_divergence(close_vals) if len(close_vals) >= 8 else []
            reqrr=_p311_required_rr(r)
            rr=_i_num(r.get('rr'), None)
            rows_out.append((coin, bi, mom, momscore, divs, reqrr, rr))
        except Exception as e:
            print(f"⚠️ Phase3.11 advisory skipped {coin} non-fatally: {e}")
            continue
...
# ============================================================
# ATLAS INTELLIGENCE REPORT v2 — additive portfolio/report layer
# ============================================================

# ATLAS Intelligence Report v2
# Drop-in reporting/portfolio-decision layer for ATLAS v11.5 / Phase 3.11.x
# Does NOT modify canonical Signal Engine, Entry/SL/TP formulas, Backtest Gate,
# Lifecycle, Paper Execution, Scheduler, or Supabase lease/watchdog logic.
...
CORE_SYMBOLS = ("BTC", "ETH", "SOL", "BNB", "XRP", "DOGE", "ADA", "TRX", "LINK")
DECISIONS = ("BUY", "ACCUMULATE", "HOLD", "WAIT", "REDUCE", "SELL")
...
def _s(v: Any, default: str = "UNKNOWN") -> str:
    if v is None:
        return default
    x = str(v).strip()
    return x.upper() if x else default
...
def _trend_score(r: Dict[str, Any]) -> float:
    score = 0.0
    for key, w in (("h1_trend", 0.8), ("h4_trend", 1.5), ("d1_trend", 1.8), ("w1_trend", 0.7)):
        t = _s(r.get(key))
        if t == "BULLISH":
            score += w
        elif t == "BEARISH":
...
def _ma_score(r: Dict[str, Any]) -> float:
    """Uses any MA/EMA fields already present in the canonical result."""
    p = _f(r.get("price"))
    if not p:
        return 0.0
    score = 0.0
    keys = (
        "ema20", "ema_20", "ma20", "ma_20",
        "ema50", "ema_50", "ma50", "ma_50",
        "ema100", "ema_100", "ma100", "ma_100",
        "ema200", "ema_200", "ma200", "ma_200",
    )
    seen = 0
    for k in keys:
        m = _f(r.get(k))
        if m:
            seen += 1
            score += 1 if p > m else -1
    return score / max(1, seen)


def _momentum_score(r: Dict[str, Any]) -> float:
    score = 0.0
    rsi = _f(r.get("rsi14"), _f(r.get("rsi")))
    if rsi is not None:
        if rsi <= 30:
            score += 0.7
        elif rsi < 45:
    ...
    macd = _f(r.get("macd"))
    macd_signal = _f(r.get("macd_signal"))
    macd_hist = _f(r.get("macd_hist"), _f(r.get("macd_histogram")))
    if macd_hist is not None:
        score += 0.8 if macd_hist > 0 else -0.8 if macd_hist < 0 else 0
    elif macd is not None and macd_signal is not None:
        score += 0.8 if macd > macd_signal else -0.8
    return score


def _regime_score(r: Dict[str, Any], btc_regime: Optional[Dict[str, Any]],
                  breadth: Optional[Dict[str, Any]], market_info: Optional[Dict[str, Any]]) -> float:
    score = 0.0
    br = breadth or {}
    state = _s(br.get("state"))
    if state == "BULLISH":
        score += 1.0
    elif state == "BEARISH":
        score -= 1.0
    ...
    reg = btc_regime or {}
    for k in ("state", "trend", "regime", "regime_trend"):
        x = _s(reg.get(k), "")
        if "BULL" in x:
            score += 0.8
            break
    ...
    stable_dom = _f(mi.get("stablecoin_dominance"))
    if stable_dom is not None and stable_dom > 12:
        score -= 0.2
    return score


def _event_score(r: Dict[str, Any], news: Optional[Dict[str, Any]], macro: Optional[Dict[str, Any]]) -> float:
    """Consumes existing ATLAS news/macro fields; never fabricates external events."""
    score = 0.0
    n = news or {}
    bias = _s(n.get("bias"))
    if bias in ("POSITIVE", "BULLISH"):
        score += 0.5
    elif bias in ("NEGATIVE", "BEARISH"):
        score -= 0.5

    for k in ("news_score", "sentiment_score", "event_score", "macro_score"):
        x = _f(r.get(k))
        if x is not None:
            # Accept either -1..1 or 0..100 scales conservatively.
            if -1.5 <= x <= 1.5:
                score += max(-0.8, min(0.8, x))
            else:
                score += max(-0.8, min(0.8, (x - 50.0) / 50.0))

    m = macro or {}
    risk = _s(m.get("risk_state"), "")
    if "RISK_OFF" in risk or "RISK-OFF" in risk:
        score -= 0.6
    elif "RISK_ON" in risk or "RISK-ON" in risk:
        score += 0.6
    return score


def _support_context(r: Dict[str, Any]) -> Tuple[float, List[str]]:
    price = _f(r.get("price"))
    support = _f(r.get("support"))
    resistance = _f(r.get("resistance"))
    score = 0.0
    why: List[str] = []
    if price and support:
        d = abs(price - support) / price * 100
        if d <= 1.5:
            score += 1.0
            why.append("price near support")
        elif price < support:
            score -= 0.6
            why.append("support lost")
    if price and resistance:
        d = abs(resistance - price) / price * 100
        if resistance > price and d <= 1.5:
            score -= 0.5
            why.append("resistance too close")
        elif price > resistance:
            score += 0.6
            why.append("resistance reclaimed")
    return score, why


def _canonical_state(r: Dict[str, Any]) -> str:
    return _s(r.get("decision_state"), _s(r.get("action")))
...
def portfolio_decision(
    ...
    """
    Non-destructive portfolio layer.
    Canonical executable signal remains untouched in decision_state/action.
    """
    canonical = _canonical_state(r)
    signal_score = _f(r.get("signal_score"), _f(r.get("confidence"), 50.0)) or 50.0
    model_strength = _f(r.get("model_strength"), signal_score) or signal_score
    executable = bool(r.get("executable")) or canonical in ("BUY CONFIRMATION", "SELL CONFIRMATION")

    t = _trend_score(r)
    ma = _ma_score(r)
    mom = _momentum_score(r)
    reg = _regime_score(r, btc_regime, breadth, market_info)
    evt = _event_score(r, news, macro)
    sr, sr_why = _support_context(r)
    composite = t + ma + mom + reg + evt + sr

    reasons: List[str] = []
    reasons.extend(sr_why)

    if t >= 2:
        reasons.append("H4/D1 trend supportive")
    elif t <= -2:
        reasons.append("H4/D1 trend bearish")
    if mom > 0.5:
    ...
    if reg > 0.6:
        ...
        reasons.append("market regime defensive")
    if evt > 0.4:
        reasons.append("news/macro supportive")
    elif evt < -0.4:
        reasons.append("news/macro headwind")

    # Canonical SELL has priority. Portfolio layer must not hide an executable exit.
    if canonical == "SELL CONFIRMATION":
        decision = "SELL"
    elif canonical == "BUY CONFIRMATION" and executable:
        decision = "BUY" if composite >= 1.0 else "ACCUMULATE"
    else:
        # No executable signal: map analysis into portfolio language.
        if composite >= 3.2 and model_strength >= 65:
            decision = "ACCUMULATE"
        elif composite >= 0.5:
            decision = "HOLD"
        elif composite > -2.5:
    ...
    # Meme/high-beta safety: never upgrade weak DOGE/PEPE style setup to BUY
    # without canonical confirmation.
    coin = _s(r.get("coin"), "")
    if coin in ("DOGE", "PEPE", "SHIB") and decision == "ACCUMULATE" and canonical != "BUY CONFIRMATION":
        decision = "WAIT"
        reasons.append("high-beta/meme safety gate")

    # Confidence = analytical confidence, not probability of profit.
    conf = 50.0
    conf += min(20.0, abs(composite) * 4.0)
    conf += max(0.0, min(15.0, (model_strength - 50.0) * 0.3))
    quality = _s(r.get("quality"), "")
    if quality == "HIGH":
        conf += 7
    elif quality == "LOW":
        conf -= 10
    if _s((breadth or {}).get("state")) == "LOW_SAMPLE":
        conf -= 5
    conf = int(max(45, min(96, round(conf))))

    out = dict(r)
    out["portfolio_decision"] = decision
    out["portfolio_confidence"] = conf
    out["portfolio_score"] = round(composite, 2)
    out["portfolio_why"] = reasons[:5]
    out["canonical_signal"] = canonical
    return out


def apply_portfolio_layer(
    ...
    return [
        portfolio_decision(r, btc_regime, breadth, market_info, news, macro)
        for r in results
    ]


def decision_delta(current: Dict[str, Any], previous: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    cur = _s(current.get("portfolio_decision"), "WAIT")
    prev = _s((previous or {}).get("portfolio_decision"), "")
    cc = int(_f(current.get("portfolio_confidence"), 0) or 0)
    pc = int(_f((previous or {}).get("portfolio_confidence"), 0) or 0)
    order = {"SELL": 0, "REDUCE": 1, "WAIT": 2, "HOLD": 3, "ACCUMULATE": 4, "BUY": 5}
    arrow = "•"
    if prev:
        arrow = "↑" if order.get(cur, 2) > order.get(prev, 2) else "↓" if order.get(cur, 2) < order.get(prev, 2) else "→"
    return {
        "previous": prev or None,
        "current": cur,
        "arrow": arrow,
        "confidence_delta": cc - pc if prev else None,
        "changed": bool(prev and prev != cur),
    }


def _find_previous(previous_results: Optional[Sequence[Dict[str, Any]]], coin: str) -> Optional[Dict[str, Any]]:
    if not previous_results:
        return None
    for r in previous_results:
        if _s(r.get("coin"), "") == coin:
            return r
    return None


def _technical_line(r: Dict[str, Any]) -> str:
    rsi = _f(r.get("rsi14"), _f(r.get("rsi")))
    macd_hist = _f(r.get("macd_hist"), _f(r.get("macd_histogram")))
    macd = _f(r.get("macd"))
    macd_signal = _f(r.get("macd_signal"))
    if macd_hist is not None:
        macd_text = "Bullish" if macd_hist > 0 else "Bearish" if macd_hist < 0 else "Flat"
    elif macd is not None and macd_signal is not None:
        macd_text = "Bullish" if macd > macd_signal else "Bearish"
    else:
        macd_text = "—"
    return f"RSI14={_num(rsi)} | MACD={macd_text} | H4={_s(r.get('h4_trend'))} | D1={_s(r.get('d1_trend'))}"


def _targets(r: Dict[str, Any]) -> str:
    vals = []
    for k in ("tp1", "tp2", "tp3", "tp4"):
        if _f(r.get(k)) is not None:
            vals.append(_price(r.get(k)))
    return " → ".join(vals) if vals else "—"


def _entry_text(r: Dict[str, Any]) -> str:
    lo = _f(r.get("entry_low"))
    hi = _f(r.get("entry_high"))
    if lo is not None and hi is not None:
        return f"{_price(lo)}–{_price(hi)}"
    return _price(r.get("entry"))


def _weather(results: Sequence[Dict[str, Any]], breadth: Optional[Dict[str, Any]]) -> Tuple[int, str]:
    if not results:
        return 50, "🟠"
    vals = [_f(r.get("portfolio_score"), 0.0) or 0.0 for r in results]
    avg = sum(vals) / max(1, len(vals))
    b = _f((breadth or {}).get("score"), 50.0) or 50.0
    score = int(max(10, min(90, round(50 + avg * 5 + (b - 50) * .25))))
    emoji = "🟢" if score >= 65 else "🟠" if score >= 40 else "🔴"
    return score, emoji



def load_previous_intelligence_report_results(
    current_cycle: str,
    current_results: Optional[Sequence[Dict[str, Any]]] = None,
    lookback_hours: int = 72,
) -> List[Dict[str, Any]]:
    """
    Load the previous FULL intelligence report from the existing
    atlas_hourly_signal_snapshots table. No schema migration is required.

    New runs stamp payload['_atlas_full_report_cycle'] before hourly persistence.
    We skip the current stamped batch and return the preceding FULL batch.
    During the first deployment (before stamped history exists), we safely
    fall back to the immediately preceding hourly batch so Decision Delta
    works from day one.
    """
    if not STORE.enabled:
        return []

    since = (now_utc() - timedelta(hours=max(6, int(lookback_hours)))).isoformat()
    try:
        rows = STORE.select("atlas_hourly_signal_snapshots", {
            "select": "captured_at,symbol,payload",
            "captured_at": f"gte.{since}",
            "order": "captured_at.desc",
            "limit": "1500",
        }) or []
    except Exception as ex:
        append_changelog("INTELLIGENCE_V2_PREVIOUS", None, None, f"load failed: {ex}")
        return []

    if not rows:
        return []

    # Group snapshots by run timestamp. All symbols from one ATLAS run share
    # effectively the same captured_at. Normalize to minute for safety.
    ...
    stamped_groups: List[Tuple[str, List[Dict[str, Any]]]] = []
    for key in ordered_keys:
        grp = groups[key]
        has_full_stamp = False
        for row in grp:
            ...
            if isinstance(payload, str):
                try:
                    ...
                    payload = {}
            if isinstance(payload, dict) and payload.get("_atlas_full_report_cycle"):
                has_full_stamp = True
                break
        if has_full_stamp:
            stamped_groups.append((key, grp))

    # Current FULL run has already been persisted before Telegram delivery.
    # Therefore the first stamped group is current; previous FULL is second.
    chosen = None
    if len(stamped_groups) >= 2:
        chosen = stamped_groups[1][1]
    elif len(stamped_groups) == 1:
        current_key = stamped_groups[0][0]
        for key in ordered_keys:
            if key != current_key:
                chosen = groups[key]
                break
    elif len(ordered_keys) >= 2:
        # First-deploy fallback: latest batch is current, previous batch is baseline.
        chosen = groups[ordered_keys[1]]
    elif ordered_keys:
        chosen = groups[ordered_keys[0]]
    ...
    for row in chosen or []:
        ...
        if not isinstance(payload, dict):
            payload = {}
        if not payload.get("coin") and row.get("symbol"):
            payload["coin"] = str(row.get("symbol")).upper()
        if payload.get("coin"):
            out.append(payload)

    # Re-run advisory portfolio mapping for old snapshots that predate v2 fields.
    if out and not any(r.get("portfolio_decision") for r in out):
        try:
            out = apply_portfolio_layer(out)
        except Exception:
            pass
    return out


def stamp_full_report_cycle(results: Sequence[Dict[str, Any]], daily_cycle: bool, nightly_cycle: bool) -> None:
    """
    Mark payloads before the existing hourly snapshot persistence. This keeps
    FULL-report history inside the already-existing Supabase snapshot table.
    """
    cycle = "DAILY16" if daily_cycle else "NIGHTLY23" if nightly_cycle else None
    if not cycle:
        return
    stamp = now_utc().isoformat()
    for r in results or []:
        r["_atlas_full_report_cycle"] = cycle
        r["_atlas_full_report_stamp"] = stamp


def build_intelligence_report_v2(
    results: Sequence[Dict[str, Any]],
    btc_regime: Optional[Dict[str, Any]] = None,
    breadth: Optional[Dict[str, Any]] = None,
    macro: Optional[Dict[str, Any]] = None,
    ...
    """
    Persian Telegram renderer. Use only on DAILY16/NIGHTLY23 FULL delivery.
    HOURLY/DEEP4H can still persist the portfolio fields without sending this text.
    """
    enriched = apply_portfolio_layer(results, btc_regime, breadth, market_info, news, macro)
    weather, emoji = _weather(enriched, breadth)
    now = datetime.now(TEHRAN).strftime("%Y-%m-%d %H:%M")

    priority = {c: i for i, c in enumerate(CORE_SYMBOLS)}
    enriched = sorted(
        enriched,
        key=lambda r: (
            priority.get(_s(r.get("coin"), ""), 999),
            -(_f(r.get("portfolio_confidence"), 0) or 0),
        ),
    )[:max_assets]

    lines = [
        f"📊 ATLAS INTELLIGENCE v2 — {cycle} — {now}",
        f"هوای بازار: {emoji} {weather}/100 | Breadth={_s((breadth or {}).get('state'))}",
        "",
        "🎯 تصمیم‌های عملیاتی",
    ]
    ...
    for r in enriched:
        coin = _s(r.get("coin"), "?")
        d = _s(r.get("portfolio_decision"), "WAIT")
        conf = int(_f(r.get("portfolio_confidence"), 0) or 0)
        prev = _find_previous(previous_results, coin)
        delta = decision_delta(r, prev)
        lines.append(
            f"{coin}: {d} {delta['arrow']} | {_price(r.get('price'))} | "
            f"Entry {_entry_text(r)} | SL {_price(r.get('sl'))} | "
            f"TP {_targets(r)} | C={conf}%"
        )

    lines.extend(["", "🧭 تکنیکال و دلیل تصمیم"])
    for r in enriched[:9]:
        coin = _s(r.get("coin"), "?")
        why = "; ".join(r.get("portfolio_why") or []) or "no strong incremental edge"
        lines.append(f"{coin} — {_technical_line(r)}")
        lines.append(f"↳ {why}")

    changed = []
    if previous_results:
        for r in enriched:
            prev = _find_previous(previous_results, _s(r.get("coin"), ""))
            dd = decision_delta(r, prev)
            if dd["changed"]:
                changed.append(
                    f"{_s(r.get('coin'))}: {dd['previous']} → {dd['current']} {dd['arrow']}"
                    + (f" | ΔC {dd['confidence_delta']:+d}" if dd["confidence_delta"] is not None else "")
                )
    lines.extend(["", "🔄 تغییر نسبت به گزارش قبلی"])
    lines.extend(changed[:8] if changed else ["تغییر تصمیم معناداری ثبت نشده است."])

    mi = market_info or {}
    lines.extend([
        "",
        "🌍 وضعیت بازار",
        f"BTC Dominance: {_num(mi.get('btc_dominance'))}% | "
        f"Stable Dominance: {_num(mi.get('stablecoin_dominance'))}% | "
        f"Fear&Greed: {_num(mi.get('fear_greed'), 0)}",
        f"News bias: {_s((news or {}).get('bias'))} | "
        f"BTC regime: {_s((btc_regime or {}).get('state'), _s((btc_regime or {}).get('trend')))}",
        "",
        "⚠️ Portfolio Decision لایه مشورتی است؛ canonical Signal Engine، Backtest Gate، "
        "Entry/SL/TP و Execution بدون تغییر باقی می‌مانند. Confidence احتمال سود نیست.",
    ])
    return "\n".join(lines)


def build_persist_rows(results: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Rows suitable for JSON/Supabase persistence by the caller."""
    rows = []
    for r in results:
        rows.append({
            "coin": r.get("coin"),
            "price": r.get("price"),
            "canonical_signal": r.get("canonical_signal") or r.get("decision_state") or r.get("action"),
            "portfolio_decision": r.get("portfolio_decision"),
            "portfolio_confidence": r.get("portfolio_confidence"),
            "portfolio_score": r.get("portfolio_score"),
            "portfolio_why": r.get("portfolio_why"),
            "entry": r.get("entry"),
            "sl": r.get("sl"),
            "tp1": r.get("tp1"),
            "tp2": r.get("tp2"),
...
# ---------------------------------------------------------------------------
# 1) Import:
#    from atlas_intelligence_report_v2 import (
#        apply_portfolio_layer, build_intelligence_report_v2
#    )
#
# 2) Immediately AFTER canonical:
#       results = apply_decision_engine(results, btc_regime, breadth)
#    add:
#       results = apply_portfolio_layer(
#           results, btc_regime, breadth, market_info, news, macro
#       )
#
# 3) Keep the existing canonical store_signal() loop unchanged.
#
# 4) In DAILY16/NIGHTLY23 when ATLAS_DELIVERY_MODE == "FULL":
#       intel_text = build_intelligence_report_v2(
#           results, btc_regime, breadth, macro, news, market_info,
#           previous_results=previous_results_if_available,
#           cycle=os.environ.get("ATLAS_PHASE37_CYCLE", "FULL"),
#       )
#    Send intel_text through the existing Telegram text sender/report splitter.
#
# 5) HOURLY and DEEP4H:
#    DO NOT send this full renderer. Persist fields only; keep STORAGE_ONLY and
#    rare-alert behavior unchanged.
#
# 6) Do not replace apply_decision_engine(), store_signal(), execution,
#    scheduler lease, watchdog, or lifecycle functions with this module.
...
def _daily16_fx_state():
    """Latest USD/USDT from Iran Data Plane only; returns Toman display values."""
    try:
        rows=_v3_rows("fx",80)
        latest=_iran_dash_latest(rows)
        return {
            "USD_IRR":_iran_fx_to_toman(latest.get("USD_IRR",{}),"USD_IRR"),
            "USDT_IRR":_iran_fx_to_toman(latest.get("USDT_IRR",{}),"USDT_IRR"),
        }
    except Exception:
        return {"USD_IRR":None,"USDT_IRR":None}


def build_daily16_compact_snapshot(scoped_results):
    """Compact human-facing snapshot; full universe remains persisted in Supabase."""
    by={str(r.get("coin") or r.get("symbol") or "").upper():r for r in (scoped_results or [])}
    dt=now_tehran()
    lines=[f"📊 ATLAS DAILY16 • SNAPSHOT | {shamsi(dt)} • {dt.strftime('%H:%M')} تهران",""]
    for sym in ATLAS_PRIORITY_TOP10:
        r=by.get(sym)
        if not r: continue
        px=safe_float(r.get("price")); ch=safe_float(r.get("change"))
        if px is None: continue
        lines.append(f"{sym}: {_snapshot_price_text(px)}" + (f" | {ch:+.2f}%" if ch is not None else ""))
    metals=[]
    for sym in ATLAS_METALS:
        r=by.get(sym)
        if not r: continue
        px=safe_float(r.get("price")); ch=safe_float(r.get("change"))
        if px is not None:
            metals.append(f"{sym} {_snapshot_price_text(px)}" + (f" ({ch:+.2f}%)" if ch is not None else ""))
    if metals:
        lines += ["","🪙 فلزات جهانی: " + " | ".join(metals)]
    fx=_daily16_fx_state()
    usd,usdt=fx.get("USD_IRR"),fx.get("USDT_IRR")
    fx_parts=[]
    if usd is not None: fx_parts.append(f"USD {usd:,.0f} تومان")
    if usdt is not None: fx_parts.append(f"USDT {usdt:,.0f} تومان")
    lines += ["","💵 Iran Data Plane: " + (" | ".join(fx_parts) if fx_parts else "DATA_MISSING")]
    lines.append("ℹ️ قیمت دلار/تتر فقط از آخرین state ایران؛ بدون منبع موازی.")
    return "\n".join(lines)


def build_daily16_executive_brief(results, btc_regime=None, news=None):
    """One decision brief: confirmed actions + nearest waits + blockers."""
    rows=list(results or [])
    ranked=_atlas_visual_rank(rows)
    confirmed=[r for r in ranked if _atlas_public_signal(r) in ("BUY","SELL")]
    waits=[r for r in ranked if _atlas_public_signal(r)=="WAIT"][:5]
    regime=str((btc_regime or {}).get("regime") or (btc_regime or {}).get("trend") or "UNKNOWN").upper() if isinstance(btc_regime,dict) else "UNKNOWN"
    adv=sum(1 for r in rows if _atlas_visual_change(r)>.05)
    dec=sum(1 for r in rows if _atlas_visual_change(r)<-.05)
    flat=max(0,len(rows)-adv-dec)
    lines=[
        "🧠 ATLAS DAILY16 • EXECUTIVE DECISION BRIEF",
        f"BTC Regime: {regime} | Breadth: {adv}↑ / {dec}↓ / {flat}→",
        "",
        "🎯 CONFIRMED",
    ]
    if not confirmed:
        lines.append("BUY/SELL تأییدشده: none")
    else:
        for r in confirmed[:5]:
            lines.append(_atlas_market_board_line(r))
    lines += ["","👀 PRE-TRIGGER / BEST WAIT"]
    for r in waits:
        lines.append(_atlas_market_board_line(r))
    if not waits:
        lines.append("— none")
    # Only 2–3 concrete blockers for the closest non-executable ideas.
    blocker_lines=[]
    for r in waits[:3]:
        reasons=why_not_trade(r)[:3]
        if reasons:
            blocker_lines.append(f"{_aio_symbol(r)}: " + " | ".join(reasons))
    if blocker_lines:
        lines += ["","🧩 WHY NOT NOW"] + blocker_lines
    lines += ["","ℹ️ WAIT/PRE-TRIGGER سیگنال BUY/SELL نیست. Entry/SL/TP فقط از خروجی canonical معتبر."]
    return "\n".join(lines)


def _atlas_tse_session_open(dt=None):
    """Baseline TSE regular session: Sat–Wed, 09:00–12:30 Tehran.
    Fresh-data gates still apply; holiday/emergency closure is never invented.
    """
    dt=dt or now_tehran()
    # Python weekday: Mon=0 ... Fri=4, Sat=5, Sun=6
    if dt.weekday() in (3,4):  # Thu/Fri
        return False
    mins=dt.hour*60+dt.minute
    return 9*60 <= mins <= 12*60+30


def _daily16_strong_rows(rows, domain):
    """Strict visual-alert gate. Never upgrades WAIT/HOLD into BUY/SELL."""
    picks=[]
    for r in rows or []:
        sig=_atlas_public_signal(r)
        if sig not in ("BUY","SELL"): continue
        if not bool(r.get("executable")): continue
        conf=max(_aio_num(r.get("confidence"),0),_aio_num(r.get("decision_confidence"),0))
        strength,score=_atlas_signal_strength(r)
        if strength!="STRONG" or conf < max(70.0, float(MIN_CONFIDENCE)): continue
        if not all(safe_float(r.get(k)) is not None for k in ("entry","sl","tp1")): continue
        picks.append((score,conf,r))
    picks.sort(key=lambda x:(x[0],x[1]),reverse=True)
    return picks[:5]


def _daily16_tse_verified_signals():
    """Use only explicit per-symbol BUY/SELL state already present in TSE DB.
    Current collector normally has no such canonical field, so this safely
    returns [] rather than inventing a stock signal from price/flow alone.
    """
    if not _atlas_tse_session_open():
        return []
    out=[]
    try:
        latest=_iran_dash_latest(_v3_rows("tse",600))
        for sym,row in latest.items():
            if sym=="TSE_MARKET": continue
            p=row.get("payload") or {}
            sig=str(p.get("canonical_signal") or p.get("signal") or p.get("decision") or "").upper()
            conf=safe_float(p.get("confidence"))
            fresh=str(row.get("captured_at") or "")
            if sig in ("BUY","SELL") and conf is not None and conf>=70:
                out.append({"symbol":sym,"signal":sig,"confidence":conf,"price":row.get("price"),"captured_at":fresh})
    except Exception:
        return []
    return out[:5]


def build_daily16_strong_signal_png(results, metal_results=None, filename="atlas_strong_signals.png"):
    """Extra PNG only when a strong verified signal exists. Never replaces the 3 core dashboards."""
    crypto=_daily16_strong_rows(results,"CRYPTO")
    metals=_daily16_strong_rows(metal_results or [],"METALS")
    tse=_daily16_tse_verified_signals()
    if not (crypto or metals or tse):
        return None
    try:
        import matplotlib.pyplot as plt
        from matplotlib.patches import Rectangle
        _iran_set_hq_fonts(plt)
        bg="#071522"; panel="#102b40"; fg="#edf6fb"; muted="#9eb3c2"
        green="#36c98f"; red="#f0646d"; amber="#f0b44d"
        rows=[]
        for _,conf,r in crypto:
            rows.append(("CRYPTO",_aio_symbol(r),_atlas_public_signal(r),conf,r.get("entry"),r.get("sl"),r.get("tp1"),"4H/1D canonical"))
        for _,conf,r in metals:
            rows.append(("METALS",_aio_symbol(r),_atlas_public_signal(r),conf,r.get("entry"),r.get("sl"),r.get("tp1"),"closed-candle canonical"))
        for r in tse:
            rows.append(("TSE",r["symbol"],r["signal"],r["confidence"],None,None,None,"verified DB state • session open"))
        fig=plt.figure(figsize=(16,2.2+1.15*len(rows)),facecolor=bg)
        ax=fig.add_axes([.035,.08,.93,.82]); ax.axis("off")
        dt=now_tehran()
        fig.text(.035,.955,"ATLAS • STRONG VERIFIED SIGNALS",color=fg,fontsize=22,weight="bold",va="top")
        fig.text(.965,.955,f"{dt.strftime('%Y-%m-%d %H:%M')} Tehran",ha="right",color=muted,fontsize=10,va="top")
        y=.84
        for domain,sym,sig,conf,en,sl,tp,note in rows:
            col=green if sig=="BUY" else red
            ax.add_patch(Rectangle((.0,y-.085),1,.105,transform=ax.transAxes,facecolor=panel,edgecolor="#25455c",lw=1))
            ax.text(.02,y,f"{domain} • {sym}",transform=ax.transAxes,color=fg,fontsize=13,weight="bold",va="center")
            ax.text(.27,y,f"{sig} • C {conf:.0f}%",transform=ax.transAxes,color=col,fontsize=13,weight="bold",va="center")
            geo="E/SL/TP: —"
            if en is not None and sl is not None and tp is not None:
                geo=f"E {fmt(en)}  |  SL {fmt(sl)}  |  TP1 {fmt(tp)}"
            ax.text(.48,y,geo,transform=ax.transAxes,color=fg,fontsize=11,weight="bold",va="center")
            ax.text(.98,y,note,ha="right",transform=ax.transAxes,color=muted,fontsize=9,va="center")
            y-=.125
        fig.text(.035,.025,"No WAIT/HOLD promotion • canonical decisions preserved • fail-closed on missing evidence",
                 color=amber,fontsize=9,weight="bold")
        plt.savefig(filename,dpi=240,bbox_inches="tight",facecolor=fig.get_facecolor(),pad_inches=.10)
        plt.close(fig)
        return filename
    except Exception as e:
        print(f"⚠️ Strong-signal PNG failed: {e}")
        return None



ATLAS_STRONG_SIGNAL_MIN_CONF = max(70.0, float(os.getenv("ATLAS_STRONG_SIGNAL_MIN_CONF", "70")))
ATLAS_TSE_SIGNAL_FRESH_MINUTES = max(30, int(os.getenv("ATLAS_TSE_SIGNAL_FRESH_MINUTES", "90")))


def _atlas_parse_iso_dt(value):
    try:
...
def _atlas_row_fresh(row, max_minutes):
    d=_atlas_parse_iso_dt((row or {}).get("captured_at"))
    if d is None:return False
    try:
        age=(now_utc()-d.astimezone(timezone.utc)).total_seconds()/60.0
        return -5.0 <= age <= float(max_minutes)
    except Exception:
        return False


def _atlas_closed_candle_id(r):
    """Stable event identity from the latest CLOSED 4H candle."""
    ts=r.get("signal_candle_ts")
    if ts is None:
        try:
            rows=((r.get("snapshots") or {}).get("4h") or {}).get("rows") or []
            if rows:ts=rows[-1][0]
        except Exception:ts=None
    try:return int(ts or 0)
    except Exception:return 0


def _atlas_strong_event_key(domain, symbol, signal, event_id):
    raw=f"STRONG_CARD|{str(domain).upper()}|{str(symbol).upper()}|{str(signal).upper()}|{event_id}"
    return hashlib.sha256(raw.encode("utf-8")).hexdigest()[:32]


def _atlas_strong_event_seen(key):
    try:
        state,_src=_atlas_existing_alert_state()
        return bool((state.get(key) or {}).get("sent"))
    except Exception:
        return False


def _atlas_mark_strong_event(key, symbol, signal, candle_ts=0):
    """Reuse existing Supabase-primary alert persistence; no new table/schema."""
    row={
        "alert_key":key,
        "timestamp":now_utc().isoformat(),
        "symbol":str(symbol).upper(),
        "timeframe":"4h",
        "event_type":"STRONG_SIGNAL_CARD",
    ...
    return _atlas_persist_alert_rows([row])


def _atlas_crypto_strong_rows(results):
    """Canonical crypto only: closed candle + confirmed action + executable plan."""
    out=[]
    for r in results or []:
        sig=_atlas_public_signal(r)
        action=str(r.get("action") or "").upper()
        ...
        h4=str(r.get("h4_trend") or "").upper()
        d1=str(r.get("d1_trend") or "").upper()
        eid=_atlas_closed_candle_id(r)
        if sig not in ("BUY","SELL"):continue
        if action not in ("BUY CONFIRMATION","SELL CONFIRMATION"):continue
        if not bool(r.get("executable")):continue
        if conf < ATLAS_STRONG_SIGNAL_MIN_CONF or rr < float(MIN_EXECUTABLE_RR):continue
        if eid <= 0:continue
        if mtf < 75:continue
        if sig=="BUY" and not (h4=="BULLISH" and d1=="BULLISH"):continue
        if sig=="SELL" and not (h4=="BEARISH" and d1=="BEARISH"):continue
        if not all(safe_float(r.get(k)) is not None for k in ("entry","sl","tp1")):continue
        out.append({"domain":"CRYPTO","symbol":_aio_symbol(r),"signal":sig,"confidence":conf,
                    "entry":r.get("entry"),"sl":r.get("sl"),"tp1":r.get("tp1"),"tp2":r.get("tp2"),
                    "rr":rr,"mtf":mtf,"h4":h4,"d1":d1,"event_id":eid,
                    "note":"closed 4H • D1 aligned • canonical executable"})
    return sorted(out,key=lambda x:(x["confidence"],x["rr"]),reverse=True)[:5]


def _atlas_metal_strong_rows(metal_results):
    """Global metals: existing metal action + real CLOSED H4/D1 confirmation."""
    out=[]
    for r in metal_results or []:
        action=str(r.get("action") or "").upper()
        sig="BUY" if action=="BUY CONFIRMATION" else "SELL" if action=="SELL CONFIRMATION" else None
        if not sig:continue
        conf=_aio_num(r.get("confidence"),0);rr=_aio_num(r.get("rr"),0)
        h4=str(r.get("h4_trend") or "").upper();d1=str(r.get("d1_trend") or "").upper()
        eid=_atlas_closed_candle_id(r)
        if conf < 75 or rr < float(MIN_EXECUTABLE_RR) or eid<=0:continue
        if sig=="BUY" and not (h4=="BULLISH" and d1=="BULLISH"):continue
        if sig=="SELL" and not (h4=="BEARISH" and d1=="BEARISH"):continue
        if not all(safe_float(r.get(k)) is not None for k in ("entry","sl","tp1")):continue
        out.append({"domain":"METALS","symbol":_aio_symbol(r),"signal":sig,"confidence":conf,
                    "entry":r.get("entry"),"sl":r.get("sl"),"tp1":r.get("tp1"),"tp2":r.get("tp2"),
                    "rr":rr,"mtf":100.0,"h4":h4,"d1":d1,"event_id":eid,
                    "note":"global • closed 4H + closed D1 aligned"})
    return sorted(out,key=lambda x:(x["confidence"],x["rr"]),reverse=True)[:5]


def _atlas_tse_flow_signals():
    """Strict TSE advisory detector from fresh Data Plane client-type facts.

    It is intentionally OUTSIDE the canonical crypto engine and Execution.
    BUY/SELL here means FLOW-CONFIRMED ADVISORY, not an executable order.
    """
    if not _atlas_tse_session_open():return []
    try:
        tl=_iran_dash_latest(_v3_rows("tse",700))
        market=(tl.get("TSE_MARKET") or {}).get("payload") or {}
        stocks=[v for k,v in tl.items() if k!="TSE_MARKET" and _atlas_row_fresh(v,ATLAS_TSE_SIGNAL_FRESH_MINUTES)]
        if len(stocks)<20:return []
        values=sorted([safe_float(x.get("value")) or 0 for x in stocks])
        liq_cut=values[int(len(values)*0.70)] if values else 0
        market_rp=safe_float(market.get("market_real_buyer_power"))
        market_net=safe_float(market.get("market_real_net_volume"))
        out=[]
        session_day=now_tehran().strftime("%Y%m%d")
        for row in stocks:
            p=row.get("payload") or {}
            pct=safe_float(p.get("pct"));net=safe_float(p.get("real_net_volume"))
            bp=safe_float(p.get("real_buyer_power"));val=safe_float(row.get("value")) or 0
            price=safe_float(row.get("price"));sym=str(row.get("symbol") or "").upper()
            if None in (pct,net,bp,price) or val<liq_cut or not sym:continue
            sig=None
            if pct>=1.0 and net>0 and bp>=1.20:sig="BUY"
            elif pct<=-1.0 and net<0 and bp<=0.80:sig="SELL"
            if not sig:continue
            conf=70.0
            conf+=5 if abs(pct)>=2 else 0
            conf+=5 if (bp>=1.5 if sig=="BUY" else bp<=0.67) else 0
            aligned=(market_net is not None and ((sig=="BUY" and market_net>0) or (sig=="SELL" and market_net<0)))
            conf+=5 if aligned else 0
            if market_rp is not None:
                conf+=5 if ((sig=="BUY" and market_rp>=1.05) or (sig=="SELL" and market_rp<=.95)) else 0
            conf=min(90.0,conf)
            if conf<75:continue
            out.append({"domain":"TSE","symbol":sym,"signal":sig,"confidence":conf,
                        "entry":price,"sl":None,"tp1":None,"tp2":None,"rr":None,"mtf":None,
                        "h4":"N/A","d1":"N/A","event_id":session_day,
                        "pct":pct,"real_net_volume":net,"buyer_power":bp,"trade_value":val,
                        "note":"FLOW-CONFIRMED ADVISORY • fresh TSE Data Plane • session open"})
        return sorted(out,key=lambda x:(x["confidence"],x["trade_value"]),reverse=True)[:5]
    except Exception as e:
        print(f"⚠️ TSE strong advisory detector failed: {e}")
        return []


def _atlas_build_domain_signal_png(domain, rows, filename):
    ...
    try:
        ...
        bg="#071522";panel="#102b40";fg="#edf6fb";muted="#9eb3c2"
        green="#36c98f";red="#f0646d";amber="#f0b44d"
        fig=plt.figure(figsize=(16,2.4+1.25*len(rows)),facecolor=bg)
        ax=fig.add_axes([.035,.08,.93,.82]);ax.axis("off")
        dt=now_tehran()
        titles={"CRYPTO":"ATLAS • CRYPTO STRONG SIGNAL","METALS":"ATLAS • GLOBAL METALS STRONG SIGNAL","TSE":"ATLAS • TSE FLOW-CONFIRMED SIGNAL"}
        fig.text(.035,.955,titles.get(domain,f"ATLAS • {domain} STRONG SIGNAL"),color=fg,fontsize=22,weight="bold",va="top")
        fig.text(.965,.955,f"{dt.strftime('%Y-%m-%d %H:%M')} Tehran",ha="right",color=muted,fontsize=10,va="top")
        y=.84
        for r in rows:
            sig=r["signal"];col=green if sig=="BUY" else red
            ax.add_patch(Rectangle((0,y-.085),1,.11,transform=ax.transAxes,facecolor=panel,edgecolor="#25455c",lw=1))
            ax.text(.02,y,f"{r['symbol']} • {sig}",transform=ax.transAxes,color=col,fontsize=14,weight="bold",va="center")
            ax.text(.22,y,f"C {r['confidence']:.0f}%",transform=ax.transAxes,color=fg,fontsize=12,weight="bold",va="center")
            if domain=="TSE":
                facts=f"Δ {r.get('pct',0):+.2f}% | BuyerPower {r.get('buyer_power',0):.2f}x | RealFlow {_iran_flow_fmt(r.get('real_net_volume'))}"
                ax.text(.36,y,facts,transform=ax.transAxes,color=fg,fontsize=10.5,weight="bold",va="center")
            else:
                geo=f"E {fmt(r.get('entry'))} | SL {fmt(r.get('sl'))} | TP1 {fmt(r.get('tp1'))} | RR {r.get('rr'):.2f}"
                ax.text(.36,y,geo,transform=ax.transAxes,color=fg,fontsize=10.5,weight="bold",va="center")
            ax.text(.985,y-.035,str(r.get("note") or ""),ha="right",transform=ax.transAxes,color=muted,fontsize=8.8,va="center")
            y-=.135
        footer=("TSE advisory only • never routed to Execution" if domain=="TSE"
                else "Closed-candle gate • no WAIT/HOLD promotion • duplicate alerts suppressed")
        fig.text(.035,.025,footer,color=amber,fontsize=9,weight="bold")
        plt.savefig(filename,dpi=240,bbox_inches="tight",facecolor=fig.get_facecolor(),pad_inches=.10)
        plt.close(fig);return filename
    except Exception as e:
        print(f"⚠️ {domain} signal PNG failed: {e}")
        return None


def _atlas_send_domain_signal_rows(domain, rows):
    """Send only NEW strong states. Failed delivery is not marked terminal."""
    pending=[]
    for r in rows or []:
        key=_atlas_strong_event_key(domain,r["symbol"],r["signal"],r["event_id"])
        if not _atlas_strong_event_seen(key):
            rr=dict(r);rr["_alert_key"]=key;pending.append(rr)
    if not pending:return 0,[]
    fn=f"atlas_{domain.lower()}_strong_signal.png"
    path=_atlas_build_domain_signal_png(domain,pending,fn)
    if not path:return 0,[f"{domain}: PNG generation failed"]
    sent=0;errors=[];delivered=False
    for c in dict.fromkeys([x for x in (TELEGRAM_CHAT_ID,TELEGRAM_GROUP_CHAT_ID) if x]):
        try:
            if _telegram_send_photo_file(c,path,f"🚨 ATLAS • {domain} Strong Signal"):
                sent+=1;delivered=True
            else:errors.append(f"{domain}[{c}]: send returned false")
        except Exception as e:errors.append(f"{domain}[{c}]: {e}")
    if delivered:
        for r in pending:
            _atlas_mark_strong_event(r["_alert_key"],r["symbol"],r["signal"],
                                     r["event_id"] if isinstance(r["event_id"],int) else 0)
    try:os.unlink(path)
    except Exception:pass
    return sent,errors


def send_strong_signals_for_cycle(results, metal_results, *, deep_cycle=False, tse_check=False):
    """Timing layer only; canonical engines and scheduler mode semantics stay unchanged."""
    total=0;errs=[]
    if deep_cycle:
        for domain,rows in (("CRYPTO",_atlas_crypto_strong_rows(results)),
                            ("METALS",_atlas_metal_strong_rows(metal_results))):
            s,e=_atlas_send_domain_signal_rows(domain,rows);total+=s;errs.extend(e)
            print(f"🚨 {domain} strong signal: candidates={len(rows)}, sent={s}, errors={len(e)}")
    if tse_check and _atlas_tse_session_open():
        rows=_atlas_tse_flow_signals()
        s,e=_atlas_send_domain_signal_rows("TSE",rows);total+=s;errs.extend(e)
        print(f"🚨 TSE strong advisory: candidates={len(rows)}, sent={s}, errors={len(e)}")
    return total,errs



def send_daily16_strong_signal_png(results, metal_results=None):
    fn=build_daily16_strong_signal_png(results,metal_results)
    if not fn:
        print("🖼 Strong-signal PNG: none — no verified strong BUY/SELL")
        return 0,[]
    sent=0;errs=[]
    for c in dict.fromkeys([x for x in (TELEGRAM_CHAT_ID,TELEGRAM_GROUP_CHAT_ID) if x]):
        try:
            if _telegram_send_photo_file(c,fn,"🚨 ATLAS • Strong Verified Signal"): sent+=1
        except Exception as e: errs.append(f"STRONG_SIGNAL[{c}]: {e}")
    try: os.unlink(fn)
    except Exception: pass
    return sent,errs
...
def _daily16_signal_lifecycle_summary(rows, limit=6):
    """Human-facing lifecycle summary only; full lifecycle remains in persistence."""
    out=[]
    for r in _atlas_visual_rank(rows or []):
        sym=_aio_symbol(r)
        sig=_atlas_public_signal(r)
        life=str(r.get("signal_lifecycle") or r.get("lifecycle") or r.get("signal_state") or "").upper()
        if not life:
            # Do not invent lifecycle states; expose only a conservative current-state label.
            life="ACTIVE" if sig in ("BUY","SELL") else "WAITING"
        out.append((sym,sig,life))
        if len(out)>=limit:break
    return out


def _daily16_trade_plan_rows(rows, limit=5):
    """Only executable canonical plans; WAIT research levels are not trade plans."""
    out=[]
    for r in _atlas_visual_rank(rows or []):
        sig=_atlas_public_signal(r)
        if sig not in ("BUY","SELL") or not bool(r.get("executable")):continue
        en=safe_float(r.get("entry"));sl=safe_float(r.get("sl"));tp=safe_float(r.get("tp1"))
        if None in (en,sl,tp):continue
        out.append(r)
        if len(out)>=limit:break
    return out


def _daily16_best_watch(rows, limit=5):
    ranked=[]
    for r in _atlas_visual_rank(rows or []):
        if _atlas_public_signal(r)!="WAIT":continue
        score=max(_aio_num(r.get("opportunity_score"),0),_aio_num(r.get("score"),0))
        ranked.append((score,r))
    ranked.sort(key=lambda x:x[0],reverse=True)
    return [r for _,r in ranked[:limit]]


def _daily16_desk_holdings_for_chat(chat_id):
    """Read-only Desk lookup for the current Telegram user. No state mutation."""
    try:
        from atlas_desk import load_users, load_all_holdings
        users = load_users() or {}
        bags = load_all_holdings() or {}
        target = str(chat_id or "").strip()
        # Prefer exact chat-id match; fall back to uid only when it is literally the same id.
        for uid, user in users.items():
            if str((user or {}).get("chat_id") or "").strip() == target:
                return (bags or {}).get(str(uid), {}) or {}
        return (bags or {}).get(target, {}) or {}
    except Exception as e:
        print(f"⚠️ DAILY16 Desk holdings read failed: {e}")
        return {}



def _daily16_primary_desk_portfolio():
    """Resolve the same eligible Desk portfolio used by the proven personal-dashboard path.

    Prefer exact private chat match. If the configured private chat is not the
    user's Desk chat_id, fall back to the first eligible DM user with holdings.
    Read-only; no Desk mutation.
    """
    try:
        from atlas_desk import load_users, load_all_holdings
        users = load_users() or {}
        bags = load_all_holdings() or {}
        target = str(TELEGRAM_CHAT_ID or "").strip()

        # 1) Exact private-chat match.
        for uid, user in users.items():
            if str((user or {}).get("chat_id") or "").strip() == target:
                bag = (bags or {}).get(str(uid), {}) or {}
                if bag:
                    return str(uid), user, bag

        # 2) Same eligibility as the already-working personal dashboard sender.
        for uid, user in users.items():
            chat_id = str((user or {}).get("chat_id") or "").strip()
            dm_started = bool((user or {}).get("dm_started"))
            bag = (bags or {}).get(str(uid), {}) or {}
            if chat_id and dm_started and bag:
                return str(uid), user, bag
    except Exception as e:
        print(f"⚠️ DAILY16 primary Desk portfolio read failed: {e}")
    return None, {}, {}


def _daily16_full_rank(rows):
    """Rank all rows without the _intel_rank filtering that intentionally returns only candidates."""
    def key(r):
        return (
            max(_aio_num(r.get("opportunity_score"),0), _aio_num(r.get("score"),0)),
            max(_aio_num(r.get("confidence"),0), _aio_num(r.get("decision_confidence"),0)),
            abs(_atlas_visual_change(r)),
        )
    return sorted(list(rows or []), key=key, reverse=True)


def _daily16_portfolio_state_map(scoped_results):
    """Read-only state map for personal holdings, enriched from Iran Data Plane where available."""
    out = {}
    ...
    try:
        fx = _iran_dash_latest(_v3_rows("fx", 100))
        for sym in ("USD_IRR","USDT_IRR"):
            row = fx.get(sym)
            if not row: continue
            price = _iran_fx_to_toman(row, sym)
            out[sym] = {
                "coin": sym, "symbol": sym, "price": price,
                "action": "ADVISORY", "confidence": None,
                "h4_trend": "N/A", "d1_trend": "N/A",
            }
    except Exception:
        pass

    try:
        mk = _iran_dash_latest(_v3_rows("market", 120))
        for sym, row in mk.items():
            if not row: continue
            px = safe_float(row.get("price"))
            if px is None: continue
            out.setdefault(sym.upper(), {
                "coin": sym.upper(), "symbol": sym.upper(), "price": px,
                "action": "ADVISORY", "confidence": None,
                "h4_trend": "N/A", "d1_trend": "N/A",
            })
    except Exception:
...
def _daily16_row_metric(r, keys, default=None):
    for k in keys:
...
_ALLOC_PROFILES = {
    "CONSERVATIVE": {"cash": 30.0, "crypto_cap": 20.0, "metal_cap": 35.0, "tse_cap": 30.0, "single_cap": 10.0, "wait_single_cap": 4.0},
    "MODERATE":     {"cash": 20.0, "crypto_cap": 35.0, "metal_cap": 30.0, "tse_cap": 35.0, "single_cap": 12.0, "wait_single_cap": 5.0},
    "AGGRESSIVE":   {"cash": 10.0, "crypto_cap": 55.0, "metal_cap": 25.0, "tse_cap": 40.0, "single_cap": 15.0, "wait_single_cap": 6.0},
}
...
def _alloc_pct_rank(values, x):
    ...
    import bisect
    i=bisect.bisect_right(vals,float(x))
...
def _alloc_weights(cands, profile):
    ...
    readiness=max(0.0,min(1.0,(top_avg-55.0)/25.0))
    confirmed=sum(1 for c in ranked if c.get("signal")=="BUY")
    readiness=min(1.0,readiness + min(0.22,confirmed*0.055))
    ...
    return rows,cash,{"readiness":round(readiness,3),"deploy_target_pct":round(deploy_target,2),"top_avg_score":round(top_avg,2),"confirmed_buy_count":confirmed}
...
def _alloc_cash_state(bag):
    ...
    for k,v in bag.items():
        ...
        if key in {"IRR","USD","USDT","EUR","THB"}:
            cash[key]=cash.get(key,0.0)+qty

    fx=_iran_dash_latest(_v3_rows("fx",500))
    def rate_irr(aliases):
        for a in aliases:
            r=fx.get(a)
            if r and safe_float(r.get("price")) is not None:
                p=float(r["price"])
                pay=r.get("payload") if isinstance(r.get("payload"),dict) else {}
                uc=str(pay.get("unit_canonical") or "").upper()
                ur=str(pay.get("unit_raw") or pay.get("unit") or "").upper()
                # Canonical unit wins over provider/raw unit. The collector may preserve
                # unit_raw=TOMAN while already storing price canonically in IRR.
                if "IRR" in uc: return p
                if "TOMAN" in uc: return p*10.0
                if "TOMAN" in ur or "تومان" in str(pay.get("unit_raw") or pay.get("unit") or ""): return p*10.0
                if "IRR" in ur: return p
                # Conservative legacy fallback for USD/USDT-scale Iran quotes.
                return p if p>=1_000_000 else p*10.0
        return None

    rates={
        "USD":rate_irr(("USD_IRR","USD","USD_FREE","DOLLAR")),
        "USDT":rate_irr(("USDT_IRR","USDT")),
        "EUR":rate_irr(("EUR_IRR","EUR","EURO")),
        "THB":rate_irr(("THB_IRR","THB","BAHT")),
        "IRR":1.0,
    }
    values={}
    total=0.0
    for cur,qty in cash.items():
        ...
        values[cur]={"qty":qty,"rate_irr":rate,"value_irr":val}
...
def _alloc_snapshot_for_user(uid, user, bag, scoped_results):
    ...
    targets,cash_pct,diag=_alloc_weights(candidates,profile)
    ...
    if deployable_irr>0:
        ...
        cash_target_irr=round(deployable_irr*cash_pct/100.0)
    else:
        cash_target_irr=None
    ...
    payload={
        ...
        "diagnostics":diag,
        "method":"quality_gate_cross_asset_dynamic_cash_v3_fx_wait_caps",
...
def _daily16_management_png(kind, results, scoped_results, portfolio_risk=None, filename=None):
    ...
    1) Full MARKET 4H + Top10/Personal Deep Analysis (all scoped rows)
    2) Best Watch + Opportunity Ranking + canonical Trade Plans
    3) Public ATLAS Personal Universe + Signal Lifecycle (never Desk holdings)
    """
    try:
        import matplotlib.pyplot as plt
        from matplotlib.patches import Rectangle
        _iran_set_hq_fonts(plt)
        bg="#071522"; panel="#102b40"; fg="#edf6fb"; muted="#9eb3c2"
        green="#36c98f"; red="#f0646d"; amber="#f0b44d"; blue="#4bb7e8"
        dt=now_tehran()
        full_ranked=_daily16_full_rank(scoped_results or results or [])
        crypto_ranked=_daily16_full_rank(results or [])

        if kind == 1:
            # Two compact tables so every 27-row MARKET 4H item can be shown legibly.
            title="ATLAS • MARKET 4H + TOP10 / PERSONAL DEEP ANALYSIS"
            fig=plt.figure(figsize=(19,12.5),facecolor=bg)
            ax=fig.add_axes([.02,.055,.96,.87]); ax.axis("off")
            adv=sum(1 for r in scoped_results or [] if _atlas_visual_change(r)>.05)
            dec=sum(1 for r in scoped_results or [] if _atlas_visual_change(r)<-.05)
            flat=max(0,len(scoped_results or [])-adv-dec)
            btc=next((r for r in (scoped_results or []) if _aio_symbol(r)=="BTC"),{})
            ax.text(.01,.97,
                    f"Universe {len(scoped_results or [])} • Breadth {adv}↑ {dec}↓ {flat}→ • "
                    f"BTC H4 {_daily16_row_metric(btc,('h4_trend','trend_h4'),'N/A')} / "
                    f"D1 {_daily16_row_metric(btc,('d1_trend','trend_d1'),'N/A')}",
                    transform=ax.transAxes,color=fg,fontsize=14,weight="bold",va="top")

            left=full_ranked[:14]; right=full_ranked[14:28]
            headers=["ASSET","DEC","PRICE","H4/D1","RSI","MTF","CONF","OPP","VOL"]
            rel=[0.00,.12,.24,.38,.52,.62,.72,.82,.91]
            for block_x, block_rows in ((.01,left),(.505,right)):
                width=.475
                for rx,h in zip(rel,headers):
                    ax.text(block_x+rx*width,.915,h,transform=ax.transAxes,
                            color=blue,fontsize=8.4,weight="bold")
                y=.88
                for i,r in enumerate(block_rows):
                    sym=_aio_symbol(r); sig=_atlas_public_signal(r)
                    px=safe_float(r.get("price"))
                    h4=str(_daily16_row_metric(r,("h4_trend","trend_h4"),"N/A"))
                    d1=str(_daily16_row_metric(r,("d1_trend","trend_d1"),"N/A"))
                    rsi=safe_float(_daily16_row_metric(r,("rsi14","rsi","rsi_14"),None))
                    mtf=_aio_num(_daily16_row_metric(r,("mtf_agreement_pct","mtf_pct","mtf"),0),0)
                    conf=max(_aio_num(r.get("confidence"),0),_aio_num(r.get("decision_confidence"),0))
                    opp=max(_aio_num(r.get("opportunity_score"),0),_aio_num(r.get("score"),0))
                    vr=safe_float(_daily16_row_metric(r,("volume_ratio","vol_ratio","volume_ratio_4h"),None))
                    vals=[
                        sym,sig,fmt(px) if px is not None else "N/A",
                        f"{h4[:4]}/{d1[:4]}",
                        f"{rsi:.0f}" if rsi is not None else "N/A",
                        f"{mtf:.0f}%",f"{conf:.0f}%",f"{opp:.0f}",
                        f"{vr:.2f}x" if vr is not None else "N/A"
                    ]
                    ax.add_patch(Rectangle((block_x-.005,y-.021),width+.005,.043,
                                           transform=ax.transAxes,
                                           facecolor=panel if i%2==0 else bg,
                                           edgecolor="#19384e",linewidth=.5))
                    for rx,v in zip(rel,vals):
                        c=(green if sig=="BUY" else red if sig=="SELL" else amber) if rx==rel[1] else fg
                        ax.text(block_x+rx*width,y,str(v),transform=ax.transAxes,
                                color=c,fontsize=7.6,va="center",
                                weight="bold" if rx in (rel[0],rel[1]) else "normal")
                    y-=.047

            # Top blockers are valuable but do not consume one full column per row.
            blockers=[]
            for r in full_ranked:
                why=why_not_trade(r)[:1]
                if why:
                    blockers.append(f"{_aio_symbol(r)}: {why[0]}")
                if len(blockers)>=6: break
            ax.text(.01,.16,"TOP WHY-NOT / GATES",transform=ax.transAxes,color=blue,fontsize=11.5,weight="bold")
            by=.125
            for b in blockers:
                ax.text(.02,by,"• "+b,transform=ax.transAxes,color=muted,fontsize=8.3)
                by-=.025
            footer="All scoped MARKET 4H rows shown • full evidence/history + CSV remain queryable in Supabase"

        elif kind == 2:
            title="ATLAS • BEST WATCH + OPPORTUNITY RANKING + TRADE PLANS"
            fig=plt.figure(figsize=(18,10.8),facecolor=bg)
            ax=fig.add_axes([.025,.06,.95,.86]); ax.axis("off")
            confirmed=[r for r in crypto_ranked if _atlas_public_signal(r) in ("BUY","SELL")][:5]
            watch=[r for r in crypto_ranked if _atlas_public_signal(r)=="WAIT"][:10]
            plans=_daily16_trade_plan_rows(results,8)

            ax.text(.01,.955,"CONFIRMED ACTION",transform=ax.transAxes,color=blue,fontsize=13,weight="bold")
            if confirmed:
                x=.02
                for r in confirmed:
                    sig=_atlas_public_signal(r); col=green if sig=="BUY" else red
                    ax.text(x,.91,f"{_aio_symbol(r)} • {sig} • C {max(_aio_num(r.get('confidence'),0),_aio_num(r.get('decision_confidence'),0)):.0f}%",
                            transform=ax.transAxes,color=col,fontsize=10.8,weight="bold")
                    x+=.19
            else:
                ax.text(.02,.91,"No confirmed BUY/SELL — ranking remains decision-useful",
                        transform=ax.transAxes,color=muted,fontsize=10.3)

            ax.text(.01,.85,"BEST WATCH / OPPORTUNITY RANKING",transform=ax.transAxes,color=blue,fontsize=13,weight="bold")
            hdr=["#","ASSET","DEC","OPP","CONF","MTF","SETUP","TRIGGER / WHY NOT"]
            xs=[.012,.052,.13,.20,.26,.33,.40,.55]
            for x,h in zip(xs,hdr):
                ax.text(x,.812,h,transform=ax.transAxes,color=muted,fontsize=8.8,weight="bold")
            y=.772
            for i,r in enumerate(watch,1):
                score=max(_aio_num(r.get("opportunity_score"),0),_aio_num(r.get("score"),0))
                conf=max(_aio_num(r.get("confidence"),0),_aio_num(r.get("decision_confidence"),0))
                mtf=_aio_num(r.get("mtf_agreement_pct"),0)
                setup=str(_daily16_row_metric(r,("setup","setup_type","strategy"),"N/A"))
                trigger=str(_daily16_row_metric(r,("trigger","entry_trigger","confirmation_trigger"),""))
                if not trigger:
                    why=why_not_trade(r)[:1]
                    trigger=str(why[0]) if why else "Await confirmed trigger + acceptable RR"
                vals=[i,_aio_symbol(r),_atlas_public_signal(r),f"{score:.0f}",f"{conf:.0f}%",
                      f"{mtf:.0f}%",setup,trigger]
                ax.add_patch(Rectangle((.005,y-.022),.99,.046,transform=ax.transAxes,
                                       facecolor=panel if i%2 else bg,edgecolor="#19384e",linewidth=.5))
                for x,v in zip(xs,vals):
                    ax.text(x,y,str(v),transform=ax.transAxes,
                            color=fg if x<.55 else muted,fontsize=8.2,va="center")
                y-=.048
            if not watch:
                ax.text(.02,.77,"No WAIT candidates in current canonical scope",transform=ax.transAxes,color=muted,fontsize=10)

            py=.245
            ax.text(.01,py,"CANONICAL EXECUTABLE TRADE PLANS",transform=ax.transAxes,color=blue,fontsize=13,weight="bold")
            py-=.048
            if not plans:
                ax.text(.02,py,"No executable BUY/SELL Entry / SL / TP plan. WAIT levels are never promoted.",
                        transform=ax.transAxes,color=muted,fontsize=10)
            else:
                for r in plans:
                    sig=_atlas_public_signal(r); col=green if sig=="BUY" else red
                    rr=_aio_num(_daily16_row_metric(r,("rr","risk_reward","risk_reward_ratio"),0),0)
                    ax.text(.02,py,
                            f"{_aio_symbol(r)} • {sig} | Entry {fmt(r.get('entry'))} | "
                            f"SL {fmt(r.get('sl'))} | TP1 {fmt(r.get('tp1'))} | RR {rr:.2f}",
                            transform=ax.transAxes,color=col,fontsize=10,weight="bold")
                    py-=.042
            footer="Best Watch + Opportunity Ranking + canonical Trade Plans • full ranking history stays in Supabase"

        else:
            # PUBLIC Personal Universe = ATLAS_PERSONAL_ASSETS, not any user's Desk holdings.
            # This board is safe for the supergroup and intentionally contains no qty/equity/user identity.
            title="ATLAS • PERSONAL UNIVERSE 4H + SIGNAL LIFECYCLE"
            public_order=[]
            for sym in list(ATLAS_PERSONAL_ASSETS):
                key=str(sym).upper()
                if key not in public_order:
                    public_order.append(key)
            by={_aio_symbol(r).upper():r for r in (results or []) if _aio_symbol(r)}
            public_rows=[by[k] for k in public_order if k in by]
            fig=plt.figure(figsize=(18,10.8),facecolor=bg)
            ax=fig.add_axes([.025,.06,.95,.86]); ax.axis("off")

            ax.text(.01,.955,f"ATLAS PERSONAL UNIVERSE • {len(public_rows)}/{len(public_order)} resolved",
                    transform=ax.transAxes,color=blue,fontsize=13,weight="bold")
            hdr=["ASSET","DEC","PRICE","H4 / D1","RSI","MTF","CONF","OPP","LIFECYCLE"]
            xs=[.012,.095,.17,.28,.39,.46,.54,.62,.70]
            for x,h in zip(xs,hdr):
                ax.text(x,.91,h,transform=ax.transAxes,color=muted,fontsize=8.8,weight="bold")
            y=.87
            shown=0
            for r in public_rows[:18]:
                sym=_aio_symbol(r); sig=_atlas_public_signal(r)
                px=safe_float(r.get("price"))
                h4=str(_daily16_row_metric(r,("h4_trend","trend_h4"),"N/A"))
                d1=str(_daily16_row_metric(r,("d1_trend","trend_d1"),"N/A"))
                ...
                if not life:
                    life="ACTIVE" if sig in ("BUY","SELL") else "WAITING"
                vals=[sym,sig,fmt(px) if px is not None else "N/A",f"{h4[:4]} / {d1[:4]}",
                      f"{rsi:.0f}" if rsi is not None else "N/A",f"{mtf:.0f}%",
                      f"{conf:.0f}%",f"{opp:.0f}",life]
                ax.add_patch(Rectangle((.005,y-.022),.99,.046,transform=ax.transAxes,
                                       facecolor=panel if shown%2==0 else bg,edgecolor="#19384e",linewidth=.5))
                for x,v in zip(xs,vals):
                    col=(green if sig=="BUY" else red if sig=="SELL" else amber) if x==xs[1] else fg
                    ax.text(x,y,str(v),transform=ax.transAxes,color=col,fontsize=8.0,va="center",
                            weight="bold" if x in (xs[0],xs[1]) else "normal")
                y-=.048; shown+=1
            if not shown:
                ax.text(.02,.86,"No ATLAS Personal Universe rows resolved in this cycle.",
                        transform=ax.transAxes,color=muted,fontsize=10.5)

            ax.text(.01,.055,
                    "PUBLIC RESEARCH UNIVERSE • no Desk holdings • no user quantities • no personal equity",
                    transform=ax.transAxes,color=amber,fontsize=8.8,weight="bold")
            footer="ATLAS Personal Universe 4H + lifecycle • user portfolios remain private in each user's DM"

        fig.text(.025,.965,title,color=fg,fontsize=20,weight="bold",va="top")
        fig.text(.975,.965,f"{dt.strftime('%Y-%m-%d %H:%M')} Tehran",
                 ha="right",color=muted,fontsize=9.5,va="top")
        fig.text(.025,.018,footer,color=amber,fontsize=8.4,weight="bold")
        filename=filename or f"atlas_daily16_management_{kind}.png"
        plt.savefig(filename,dpi=240,bbox_inches="tight",
                    facecolor=fig.get_facecolor(),pad_inches=.08)
        plt.close(fig)
        return filename
    except Exception as e:
        print(f"⚠️ DAILY16 management PNG {kind} failed: {e}")
        return None

def send_daily16_management_pngs(results, scoped_results, portfolio_risk=None):
    """Send the three public research PNGs to SUPERGROUP only.

    These boards contain ATLAS market/research universes only. User Desk holdings
    remain exclusively on the existing per-user private-dashboard path.
    """
    sent=0; errs=[]
    captions={
        1:"📊 ATLAS AI | MARKET 4H • Top10 + Personal Deep Analysis",
        2:"👀 ATLAS | Best Watch • Opportunity Ranking • Trade Plans",
        3:"🔄 ATLAS | Personal Universe 4H • Signal Lifecycle",
    }
    group_destinations=[TELEGRAM_GROUP_CHAT_ID] if TELEGRAM_GROUP_CHAT_ID else []
    if not group_destinations:
        return 0,["MANAGEMENT_PNG: TELEGRAM_GROUP_CHAT_ID is empty"]

    for kind in (1,2,3):
        fn=_daily16_management_png(kind,results,scoped_results,portfolio_risk,
                                   f"atlas_daily16_public_{kind}.png")
        if not fn:
            errs.append(f"MANAGEMENT_PNG_{kind}: generation failed")
            continue
        for c in group_destinations:
            try:
                if _telegram_send_photo_file(c,fn,captions[kind]):
                    sent+=1
                else:
                    errs.append(f"MANAGEMENT_PNG_{kind}[{c}]: send returned false")
            except Exception as e:
                errs.append(f"MANAGEMENT_PNG_{kind}[{c}]: {e}")
        try: os.unlink(fn)
        except Exception: pass
    return sent,errs
...
def _phase310_send_full_daily16(results, scoped_results, top10, macro, news, btc_regime, portfolio_risk, metal_results=None):
    ...
    Full analytical state continues to be computed/persisted by the existing
    Analysis Plane. Telegram receives only decision-useful summaries. The three
    canonical end dashboards are sent later by main() and are intentionally not
    touched here.
    """
    ...
    sent_total=0; errors=[]; parts=0

    # A) Compact snapshot. USD/USDT are read only from Iran Data Plane.
    try:
        txt=build_daily16_compact_snapshot(scoped_results)
        p,s,e=send_report(txt); parts+=p; sent_total+=s; errors.extend(e)
        _daily16_diag("COMPACT_SNAPSHOT",s,e)
    except Exception as ex:
        ee=f"DAILY16_COMPACT_SNAPSHOT: {ex}"; errors.append(ee); _daily16_diag("COMPACT_SNAPSHOT",0,[ee])

    # B) One executive brief replaces duplicated Intelligence/Comprehensive blocks.
    try:
        txt=build_daily16_executive_brief(results,btc_regime=btc_regime,news=news)
        p,s,e=send_report(txt); parts+=p; sent_total+=s; errors.extend(e)
    ...
    # PRE-TRIGGER information is already represented here + Executive Brief.
    ...
    try:
        ...
        ee=f"DECISION_BOARD_DAILY16: {ex}"; errors.append(ee); _daily16_diag("DECISION_BOARD",0,[ee])

    # D) Three PUBLIC research PNGs -> SUPERGROUP only. They contain Top10 +
    #    ATLAS Personal Universe, never per-user Desk holdings. They are additive
    #    and do not replace the three mandatory core dashboards sent later by main().
    try:
        s,e=send_daily16_management_pngs(results,scoped_results,portfolio_risk)
        sent_total+=s; errors.extend(e)
        _daily16_diag("MANAGEMENT_PNG_1_2_3",s,e)
    except Exception as ex:
        ee=f"MANAGEMENT_PNG_DAILY16: {ex}"; errors.append(ee); _daily16_diag("MANAGEMENT_PNG_1_2_3",0,[ee])

    # E) Strong-signal PNG is event-timed outside DAILY16:
    #    Crypto/Metals on Deep4H closed-candle cycle; TSE only in-session.
    #    DAILY16 remains a recap and the 3 mandatory dashboards remain untouched.

    # F) Voice stays available but uses the same already-computed state.
    try:
        if ENABLE_VOICE_REPORT and AUTO_SEND_VOICE:
            audio_file=generate_audio_report(scoped_results,news,btc_regime)
            if audio_file:
                audio_ok=send_audio_report(audio_file,"🎤 ATLAS Daily16 • Executive Audio")
                if not audio_ok: errors.append("VOICE_DAILY16: audio not sent")
                try: os.unlink(audio_file)
                except Exception: pass
    except Exception as ex:
        errors.append(f"VOICE_DAILY16: {ex}")

    # Intentionally NOT sent to Telegram on DAILY16:
    # - MARKET/PERSONAL CSV documents
    # - duplicate Opportunity Board
    # - full Intelligence v2 text
    # - full Iran v3 text / long fund list
    # - full Why-Not-Trade document
    # - Phase1 verbose documents / Portfolio Risk dump
    # - Backtest dashboard
    # - long Signal Lifecycle dump
    # Their underlying state/history remains in the existing persistence/research
    # layers; no canonical signal, Entry/SL/TP, Backtest Gate or Execution logic
    # is modified by this delivery policy.
    print("🗃 DAILY16 storage-first details: verbose research/report surfaces suppressed from Telegram")
...
# === ATLAS MULTI-ASSET v3: IRAN + METALS ===
IRAN_MULTI_ASSET_ENABLED=os.getenv("ATLAS_IRAN_MULTI_ASSET_ENABLED","1")=="1"
...
def _v3_series(rows,aliases):
    """Historical series in one canonical display scale.

    FX snapshots have existed in both provider-Toman and canonical-IRR forms.
    Normalize USD/USDT to Toman *before* delta/RSI/support/resistance math so a
    unit migration can never appear as a synthetic +900% market move.
    """
    aliases={str(x).upper() for x in aliases};out=[]
    for row in reversed(rows or []):
        ...
        sym=str(row.get("symbol") or row.get("asset") or p.get("symbol") or p.get("asset") or p.get("coin") or "").upper()
        if sym not in aliases:continue
        if sym in ("USD","USD_IRR","USD_FREE","DOLLAR","USDT","USDT_IRR"):
            v=_iran_fx_to_toman(row, "USDT_IRR" if "USDT" in sym else "USD_IRR")
        else:
            v=safe_float(row.get("price") or row.get("value") or p.get("price") or p.get("value") or p.get("close"))
        if v is not None:out.append(v)
    return out


def _v3_change_pct(series, lookback=1):
    """Percent change from stored snapshots only. None means insufficient history."""
    vals=[safe_float(v) for v in (series or [])]
    vals=[v for v in vals if v is not None]
    if len(vals)<=lookback:return None
    prev=vals[-1-lookback];cur=vals[-1]
    if not prev:return None
    return (cur/prev-1.0)*100.0

def _v3_history_change(rows, aliases):
    """Return latest stored-snapshot delta; never manufacture +0.00% for missing history."""
    return _v3_change_pct(_v3_series(rows,aliases),1)

def _iran_flow_fmt(v):
    x=safe_float(v)
    if x is None:return "—"
    ax=abs(x)
    ...
    return f"{x:+.0f}"
def _iran_fx_to_toman(row, symbol):
    """Render canonical FX safely in Toman without double-dividing legacy Toman snapshots."""
    p=safe_float((row or {}).get("price"))
    if p is None:return None
    pay=(row or {}).get("payload") or {}
    uc=str(pay.get("unit_canonical") or "").upper()
    ur=str(pay.get("unit_raw") or pay.get("unit") or "").upper()
    # New canonical collector: FX is stored in IRR.
    if "IRR" in uc:return p/10.0
    # Legacy/provider snapshots explicitly quoted in Toman must not be divided again.
    if "TOMAN" in ur or "تومان" in str(pay.get("unit_raw") or pay.get("unit") or ""):
        return p
    # Safe fallback for USD/USDT naming: IRR-scale quotes are ~10x Toman-scale quotes.
    if symbol in ("USD_IRR","USDT_IRR") and p>=1_000_000:return p/10.0
    return p

def _iran_set_hq_fonts(plt):
    """Select an actually installed Persian-capable font; never request a missing family."""
    try:
...
def _v3_ma(x,n):
    x=[safe_float(v) for v in x];x=[v for v in x if v is not None]
...
def _v3_rsi(x,n=14):
    ...
    g=sum(max(v,0) for v in d)/n;l=sum(max(-v,0) for v in d)/n
    return 100. if l==0 else 100-100/(1+g/l)

def _v3_asset(name,s):
    if not s:return {"asset":name,"status":"DATA_MISSING","decision":"WAIT","confidence":0}
    p=s[-1];m20=_v3_ma(s,20);m50=_v3_ma(s,50);m200=_v3_ma(s,200);r=_v3_rsi(s);score=0.;pts=0
    for m,w in ((m20,.7),(m50,1.),(m200,1.3)):
        if m is not None:score+=w if p>m else -w;pts+=1
    if r is not None:
        pts+=1
        if r<30:score+=.5
        elif r>72:score-=.7
        elif 40<=r<=65:score+=.4
    recent=s[-20:];sup=min(recent) if len(recent)>=3 else None;res=max(recent) if len(recent)>=3 else None
    if sup and abs(p-sup)/p<=.02:score+=.8
    dec="BUY" if score>=3.2 else "ACCUMULATE" if score>=1 else "HOLD" if score>=-.5 else "WAIT" if score>=-2.2 else "REDUCE"
    conf=int(max(48,min(94,54+abs(score)*6+pts*2)))
    ch=_v3_change_pct(s,1)
    advisory=name in ("XAUUSD","XAGUSD","COPPER","GOLD18","COIN_EMAMI","SILVER999","SILVER999_FAIR","USD_IRR","USDT_IRR")
    return {"asset":name,"price":p,"decision":dec,"confidence":conf,"score":round(score,2),"rsi14":r,
            "ma20":m20,"ma50":m50,"ma200":m200,"support":sup,"resistance":res,
            "change_pct":ch,"signal_scope":"ADVISORY" if advisory else "MARKET_INTELLIGENCE"}

def build_iran_multi_asset_snapshot_v3(metal_results=None):
    fx=_v3_rows("fx");tse=_v3_rows("tse");funds=_v3_rows("funds");metals=_v3_rows("metals");rows=fx+tse+funds+metals
    specs=[("USD_IRR",("USD","USD_IRR","USD_FREE","DOLLAR")),("USDT_IRR",("USDT","USDT_IRR")),
      ("GOLD18",("GOLD18","18K","GOLD_18")),("COIN_EMAMI",("COIN_EMAMI","EMAMI","SEKE")),
      ("SILVER999",("SILVER999","XAG_IRR","SILVER_999")),("SILVER999_FAIR",("SILVER999_FAIR",)),("XAUUSD",("XAU","XAUUSD","GOLD")),
      ("XAGUSD",("XAG","XAGUSD","SILVER")),("COPPER",("COPPER","HG","XCUUSD")),
      ("TSE_MARKET",("TSE_MARKET",))]
    assets=[_v3_asset(n,_v3_series(rows,a)) for n,a in specs];by={x["asset"]:x for x in assets}
    usd=safe_float(by.get("USD_IRR",{}).get("price"));usdt=safe_float(by.get("USDT_IRR",{}).get("price"))
    if usd and usdt:by["USDT_IRR"]["premium_pct"]=round((usdt/usd-1)*100,2)
    return {"assets":list(by.values()),"source_counts":{"fx":len(fx),"tse":len(tse),"funds":len(funds),"metals":len(metals)}}

def _v3_funds():
    latest={}
    for row in _v3_rows("funds",500):
        ...
        p=p if isinstance(p,dict) else {}
        sym=str(row.get("symbol") or p.get("symbol") or "").strip()
        if not sym or sym in latest:continue
        price=safe_float(row.get("price") if row.get("price") is not None else p.get("price"))
        nav=safe_float(row.get("nav") if row.get("nav") is not None else p.get("nav"))
        latest[sym]={
            "symbol":sym,
            "name":row.get("name") or p.get("name") or sym,
            "fund_type":row.get("fund_type") or p.get("fund_type") or "ETF",
            "price":price,
            "nav":nav,
            "nav_status":p.get("nav_status") or ("AVAILABLE" if nav else "UNAVAILABLE"),
            "yield":safe_float(p.get("effective_yield") or p.get("yield_effective")),
            "premium":((price/nav)-1)*100 if price and nav else None,
            "liquidity":safe_float(p.get("trade_value") or row.get("value")),
            "pct":safe_float(p.get("pct")),
            "captured_at":row.get("captured_at")
        }
    return list(latest.values())

def _iran_fmt_num(v, decimals=2):
    x=safe_float(v)
    if x is None:return "N/A"
    return f"{x:,.{decimals}f}"

def _iran_fmt_price(v):
    x=safe_float(v)
    if x is None:return "N/A"
    ax=abs(x)
    if ax>=1000:return f"{x:,.0f}"
    if ax>=1:return f"{x:,.2f}"
    if ax>=0.01:return f"{x:,.4f}"
    return f"{x:,.8f}"

def build_iran_markets_report_v3(metal_results=None):
    assets=build_iran_multi_asset_snapshot_v3(metal_results)["assets"]
    lines=["🇮🇷 ATLAS IRAN & METALS v3 — "+now_tehran().strftime("%Y-%m-%d %H:%M"),"","🎯 تصمیم‌ها"]
    for a in assets:
        if a.get("status")=="DATA_MISSING":
            lines.append(f"{a['asset']}: WAIT | DATA_MISSING — no fabricated signal");continue
        scope=a.get("signal_scope") or "MARKET_INTELLIGENCE"
        ch=a.get("change_pct")
        delta=f"{ch:+.2f}%" if ch is not None else "—"
        label="ADVISORY" if scope=="ADVISORY" else scope
        lines.append(f"{a['asset']}: {label} • {a['decision']} | P={_iran_fmt_price(a.get('price'))} | Δ={delta} | RSI={_iran_fmt_num(a.get('rsi14'),1)} | S={_iran_fmt_price(a.get('support'))} R={_iran_fmt_price(a.get('resistance'))} | C={a['confidence']}%")
    lines+=["","🏦 صندوق‌ها"];fs=_v3_funds()
    if not fs:
        lines.append("ETF market data missing — WAIT; no fabricated fund data.")
    else:
        fs=sorted(fs,key=lambda x:(x.get("liquidity") or 0),reverse=True)
        nav_available=sum(1 for x in fs if x.get("nav") is not None)
        lines.append(f"ETF market snapshots: {len(fs)} | NAV available: {nav_available}/{len(fs)}")
        for x in fs[:8]:
            nav_txt=(f"NAV={_iran_fmt_price(x.get('nav'))} | premium={_iran_fmt_num(x.get('premium'),2)}%"
                     if x.get("nav") is not None else
                     f"NAV={x.get('nav_status') or 'UNAVAILABLE'}")
            lines.append(f"{x['name']}: P={_iran_fmt_price(x.get('price'))} | {nav_txt} | liquidity={_iran_fmt_num(x.get('liquidity'),0)}")
    silver=next((x for x in assets if x["asset"]=="XAGUSD" and x.get("status")!="DATA_MISSING"),None)
    if silver:
        sw=int(max(10,min(90,50+(safe_float(silver.get("score")) or 0)*7)));lines+=["",f"🥈 SILVER WEATHER: {sw}/100 | {silver['decision']}"]
    lines+=["","⚠️ Fail-closed: missing Iran feeds => DATA_MISSING; ATLAS never invents price/RSI/signal."]
    return "\n".join(lines)

def send_iran_markets_report_v3(metal_results=None):
    if not IRAN_MULTI_ASSET_ENABLED:return False
    try:
        _p,_s,_e=send_report(build_iran_markets_report_v3(metal_results));return _e==0
    except Exception as ex:
        append_changelog("IRAN_MULTI_ASSET_V3",None,None,f"send failed: {ex}");return False
# === END ATLAS MULTI-ASSET v3 ===


# =============================================================================
# ATLAS IRAN VISUAL DASHBOARD v4
# Read-only PNG renderer fed exclusively by Supabase Iran Data Plane snapshots.
# =============================================================================
ATLAS_IRAN_DASHBOARD_ENABLED=os.getenv("ATLAS_IRAN_DASHBOARD_ENABLED","1")=="1"
ATLAS_IRAN_DASHBOARD_NIGHTLY=os.getenv("ATLAS_IRAN_DASHBOARD_NIGHTLY","1")=="1"

def _iran_dash_latest(rows):
    def _ts(r):
        return str((r or {}).get("captured_at") or "")
    out={}
    for row in sorted((rows or []), key=_ts, reverse=True):
        if not isinstance(row,dict):continue
        p=row.get("payload",{})
        if isinstance(p,str):
            try:p=json.loads(p)
        ...
        p=p if isinstance(p,dict) else {}
        s=str(row.get("symbol") or p.get("symbol") or "").upper()
        if s and s not in out:
            out[s]={"symbol":s,
                    "price":safe_float(row.get("price") if row.get("price") is not None else p.get("price")),
                    "value":safe_float(row.get("value") if row.get("value") is not None else p.get("trade_value")),
                    "payload":p,"captured_at":row.get("captured_at")}
    return out

def build_iran_visual_dashboard(filename="atlas_iran_markets.png"):
    if not ATLAS_IRAN_DASHBOARD_ENABLED:return None
    try:
        import matplotlib.pyplot as plt
        from matplotlib.patches import Rectangle

        fx=_v3_rows("fx",500)
        tse=_v3_rows("tse",600)
        funds=_v3_rows("funds",500)
        metals=_v3_rows("metals",500)
        latest=_iran_dash_latest(fx+tse+metals)
        tse_latest=_iran_dash_latest(tse)
        market=tse_latest.get("TSE_MARKET",{}).get("payload",{})
        fund_rows=_v3_funds()
        snap=build_iran_multi_asset_snapshot_v3()
        decisions={x.get("asset"):x for x in snap.get("assets",[])}

        bg="#071522"; panel="#0d2233"; edge="#25455c"; fg="#edf6fb"; muted="#9eb3c2"
        green="#36c98f"; red="#f0646d"; amber="#f0b44d"
        fig=plt.figure(figsize=(19.2,10.8),facecolor=bg)
        gs=fig.add_gridspec(20,30,left=.018,right=.987,top=.91,bottom=.045,wspace=.72,hspace=.95)

        def pnl(spec,title):
            ax=fig.add_subplot(spec);ax.set_facecolor(panel)
            for s in ax.spines.values():s.set_color(edge)
            ax.tick_params(colors=muted,labelsize=7)
            ax.set_title(title,loc="left",color=fg,fontsize=10.5,weight="bold",pad=7)
            return ax

        dt=now_tehran()
        fig.text(.025,.968,"ATLAS  |  IRAN MACRO + TSE INTELLIGENCE",color=fg,fontsize=24,weight="bold",va="top")
        fig.text(.985,.968,f"{shamsi(dt)}  •  {dt.strftime('%H:%M')} Tehran  •  SUPABASE DATA PLANE",
                 ha="right",color=muted,fontsize=9,va="top")

        # 9 macro cards in a real 5x2 grid; none can be clipped off-canvas.
        ac=pnl(gs[0:7,0:15],"FX • GOLD • SILVER • COPPER")
        ac.axis("off")
        specs=[("USD","USD_IRR"),("USDT • BRSAPI","USDT_IRR"),("GOLD 18K","GOLD18"),
               ("EMAMI COIN","COIN_EMAMI"),("SILVER 999","SILVER999"),
               ("SILVER FAIR","SILVER999_FAIR"),("GOLD XAU","XAUUSD"),
               ("SILVER XAG","XAGUSD"),("COPPER","COPPER")]
        for i,(label,sym) in enumerate(specs):
            rr,cc=divmod(i,5); x=.012+cc*.197; y=.54-rr*.43; w=.185; h=.36
            a=latest.get(sym,{})
            p=a.get("price"); pay=a.get("payload",{})
            ch=safe_float(pay.get("change_percent") if pay.get("change_percent") is not None else pay.get("pct"))
            ac.add_patch(Rectangle((x,y),w,h,transform=ac.transAxes,facecolor="#102b40",edgecolor=edge,lw=1))
            ac.text(x+.012,y+.25,label,transform=ac.transAxes,color=muted,fontsize=7.5,weight="bold")
            ac.text(x+.012,y+.13,_iran_fmt_price(p) if p is not None else "DATA MISSING",
                    transform=ac.transAxes,color=fg,fontsize=10.2,weight="bold")
            if ch is not None:
                ac.text(x+.012,y+.045,f"{ch:+.2f}%",transform=ac.transAxes,
                        color=green if ch>=0 else red,fontsize=7.8,weight="bold")

        # Macro decisions: explicit analysis for USD / USDT / gold / silver / copper.
        ad=pnl(gs[0:7,15:22],"ATLAS MACRO DECISIONS");ad.axis("off")
        macro_order=["USD_IRR","USDT_IRR","GOLD18","SILVER999","XAUUSD","XAGUSD","COPPER"]
        for i,key in enumerate(macro_order):
            x=decisions.get(key,{})
            y=.90-i*.125
            d=str(x.get("decision") or "WAIT")
            col=green if d in ("BUY","ACCUMULATE") else red if d in ("SELL","REDUCE") else amber
            ad.text(.04,y,key,transform=ad.transAxes,color=fg,fontsize=8)
            ad.text(.60,y,d,transform=ad.transAxes,color=col,fontsize=8,weight="bold")
            ad.text(.96,y,f"RSI {_iran_fmt_num(x.get('rsi14'),1)}",ha="right",
                    transform=ad.transAxes,color=muted,fontsize=7.5)

        # Fixed-income fund candidates are separated from the broad ETF universe.
        afix=pnl(gs[0:7,22:30],"FIXED-INCOME FUNDS");afix.axis("off")
        fixed_words=("درآمد ثابت","درآمدثابت","ثابت","fixed income")
        fixed=[x for x in fund_rows if any(w in str(x.get("name") or "").lower() for w in fixed_words)]
        fixed=sorted(fixed,key=lambda x:x.get("liquidity") or 0,reverse=True)[:8]
        if not fixed:
            afix.text(.5,.58,"NO VERIFIED FIXED-INCOME SUBTYPE",ha="center",
                      transform=afix.transAxes,color=amber,fontsize=9,weight="bold")
            afix.text(.5,.42,"ETF market data exists; subtype is not fabricated.",
                      ha="center",transform=afix.transAxes,color=muted,fontsize=7.5)
        else:
            for i,x in enumerate(fixed):
                y=.91-i*.105
                afix.text(.03,y,str(x.get("name"))[:19],transform=afix.transAxes,color=fg,fontsize=7.2)
                afix.text(.97,y,_iran_fmt_price(x.get("price")),ha="right",
                          transform=afix.transAxes,color=fg,fontsize=7.2)

        # TSE heatmap
        ah=pnl(gs[7:15,0:17],"TEHRAN STOCK EXCHANGE • LIQUID LEADERS")
        ah.axis("off")
        stocks=[v for k,v in tse_latest.items() if k!="TSE_MARKET"]
        stocks=sorted(stocks,key=lambda x:x.get("value") or 0,reverse=True)[:35]
        cols=7
        for i,r in enumerate(stocks):
            rr,cc=divmod(i,cols); x=.01+cc*.141; y=.81-rr*.19
            pc=safe_float(r.get("payload",{}).get("pct")) or 0
            face="#14583f" if pc>.05 else "#6a2c35" if pc<-.05 else "#334657"
            ah.add_patch(Rectangle((x,y),.132,.16,transform=ah.transAxes,facecolor=face,edgecolor=edge,lw=.7))
            ah.text(x+.008,y+.09,r["symbol"][:12],transform=ah.transAxes,color=fg,fontsize=7.1,weight="bold")
            ah.text(x+.008,y+.027,f"{pc:+.2f}%",transform=ah.transAxes,
                    color=green if pc>=0 else red,fontsize=7.0,weight="bold")

        # Explicit TSE analysis, which was missing from the old image.
        at=pnl(gs[7:15,17:24],"TSE ANALYSIS");at.axis("off")
        breadth=safe_float(market.get("breadth"))
        adv=safe_float(market.get("advance")) or 0
        dec=safe_float(market.get("decline")) or 0
        unchanged=safe_float(market.get("unchanged")) or 0
        tv=safe_float(market.get("trade_value"))
        vol=safe_float(market.get("volume"))
        real_net=safe_float(market.get("market_real_net_volume"))
        real_power=safe_float(market.get("market_real_buyer_power"))
        real_dir=str(market.get("market_real_flow_direction") or "—")
        if breadth is None:
            regime="DATA MISSING"; rcol=amber
        elif breadth>=60:
            regime="BROAD STRENGTH"; rcol=green
        elif breadth<=40:
            regime="BROAD WEAKNESS"; rcol=red
        else:
            regime="MIXED / NEUTRAL"; rcol=amber
        at.text(.04,.88,regime,transform=at.transAxes,color=rcol,fontsize=13,weight="bold")
        tse_lines=[
            f"Breadth: {_iran_fmt_num(breadth,1)}%" if breadth is not None else "Breadth: N/A",
            f"Advance / Decline: {int(adv)} / {int(dec)}",
            f"Unchanged: {int(unchanged)}",
            f"Trade value: {_iran_fmt_num(tv,0)}",
            f"Real flow: {real_dir} • {_iran_flow_fmt(real_net)}",
            f"Real buyer power: {_iran_fmt_num(real_power,2)}x" if real_power is not None else "Real buyer power: —",
            "Interpretation: breadth + real-money; no invented index signal."
        ]
        for i,line in enumerate(tse_lines):
            at.text(.04,.72-i*.105,line,transform=at.transAxes,color=fg if i<5 else muted,fontsize=8)

        # Movers
        am=pnl(gs[7:15,24:30],"TOP MOVERS");am.axis("off")
        movers=sorted(stocks,key=lambda x:safe_float(x.get("payload",{}).get("pct")) or 0,reverse=True)
        for i,r in enumerate((movers[:5]+movers[-5:])[:10]):
            pc=safe_float(r.get("payload",{}).get("pct")) or 0;y=.91-i*.087
            am.text(.05,y,r["symbol"][:13],transform=am.transAxes,color=fg,fontsize=7.8,weight="bold")
            am.text(.94,y,f"{pc:+.2f}%",ha="right",transform=am.transAxes,
                    color=green if pc>=0 else red,fontsize=7.8,weight="bold")

        # Breadth chart
        abr=pnl(gs[15:20,0:8],"TSE BREADTH")
        abr.bar(["Advance","Flat","Decline"],[adv,unchanged,dec])
        abr.grid(axis="y",alpha=.12);abr.set_ylabel("Symbols",color=muted,fontsize=8)

        # Broad ETF panel
        au=pnl(gs[15:20,8:18],"ETF FUNDS • MARKET DATA");au.axis("off")
        fs=sorted(fund_rows,key=lambda x:x.get("liquidity") or 0,reverse=True)[:7]
        if not fs:
            au.text(.5,.5,"ETF MARKET DATA MISSING",ha="center",transform=au.transAxes,color=amber,fontsize=10)
        else:
            for i,x in enumerate(fs):
                y=.88-i*.12
                au.text(.03,y,str(x.get("name"))[:19],transform=au.transAxes,color=fg,fontsize=7.2)
                au.text(.58,y,_iran_fmt_price(x.get("price")),transform=au.transAxes,color=fg,fontsize=7.1)
                pctv=safe_float(x.get("pct"))
                rhs=(f"{pctv:+.2f}% • V {_iran_fmt_num(x.get('liquidity'),0)}" if pctv is not None
                     else f"V {_iran_fmt_num(x.get('liquidity'),0)}")
                au.text(.97,y,rhs,ha="right",transform=au.transAxes,color=green if (pctv or 0)>0 else red if (pctv or 0)<0 else muted,fontsize=6.8)

        # Silver weather + summary
        asi=pnl(gs[15:20,18:24],"SILVER WEATHER");asi.axis("off")
        sx=decisions.get("XAGUSD",{})
        sw=int(max(10,min(90,50+(safe_float(sx.get("score")) or 0)*7))) if sx and sx.get("status")!="DATA_MISSING" else None
        asi.text(.5,.62,str(sw) if sw is not None else "N/A",ha="center",
                 transform=asi.transAxes,color=green if sw and sw>=60 else red if sw and sw<=40 else amber,
                 fontsize=32,weight="bold")
        asi.text(.5,.30,str(sx.get("decision") or "WAIT"),ha="center",
                 transform=asi.transAxes,color=fg,fontsize=10,weight="bold")

        ai=pnl(gs[15:20,24:30],"DATA QUALITY");ai.axis("off")
        usdt_provider=(latest.get("USDT_IRR",{}).get("payload",{}) or {}).get("provider","N/A")
        lines=[
            f"USDT provider: {usdt_provider}",
            f"TSE stored symbols: {len(stocks)} shown / 251 tracked",
            f"ETF tracked: {len(fund_rows)}",
            f"Real-money: {real_dir} • power {_iran_fmt_num(real_power,2)}x" if real_power is not None else "Real-money: —",
            "NAV enrichment unavailable on current BrsApi plan",
            "Fail-closed • no fabricated retail silver/NAV/signal"
        ]
        for i,line in enumerate(lines):
            ai.text(.05,.88-i*.16,"• "+line,transform=ai.transAxes,color=fg,fontsize=7.5,va="top")

        fig.text(.025,.015,"ATLAS Iran Data Plane → Supabase → Intelligence → Telegram  |  Analysis, not financial advice",
                 color=muted,fontsize=7.5)
        plt.savefig(filename,dpi=240,bbox_inches="tight",pad_inches=0.10,facecolor=fig.get_facecolor())
        plt.close(fig)
        return filename
    except Exception as e:
        print(f"⚠️ Iran dashboard generation failed: {e}")
        return None

def _legacy_send_iran_visual_dashboard():
    filename=build_iran_visual_dashboard()
    if not filename:return 0,["Iran dashboard generation returned no file"]
    dests=[]
    # Group first: this is the requested canonical destination. DM remains optional fallback.
    for c in (TELEGRAM_GROUP_CHAT_ID, TELEGRAM_CHAT_ID):
        c=str(c or "").strip()
        if c and c not in dests:dests.append(c)
    sent=0;errs=[]
    cap=f"🇮🇷 ATLAS Iran Markets Dashboard | {shamsi(now_tehran())} {now_tehran().strftime('%H:%M')} Tehran"
    for c in dests:
        try:
            if _telegram_send_photo_file(c,filename,cap):sent+=1
        except Exception as e:errs.append(f"{c}: {e}")
    try:os.unlink(filename)
    except Exception:pass
    return sent,errs
# =============================================================================
# ATLAS IRAN SPLIT DASHBOARDS v6
# Two read-only images: (1) Macro/Metals/Funds, (2) TSE Intelligence.
# Crypto dashboard remains the separate third image.
# =============================================================================
def _iran_data_bundle():
    fx=_v3_rows("fx",500); metals=_v3_rows("metals",500)
    tse=_v3_rows("tse",700); funds=_v3_rows("funds",500)
    return fx,metals,tse,funds

def build_iran_macro_funds_dashboard(filename="atlas_iran_macro_funds.png"):
    if not ATLAS_IRAN_DASHBOARD_ENABLED:return None
    try:
        import matplotlib.pyplot as plt
        _iran_set_hq_fonts(plt)
        from matplotlib.patches import Rectangle
        fx,metals,tse,funds=_iran_data_bundle()
        latest=_iran_dash_latest(fx+metals)
        fs=_v3_funds()
        snap=build_iran_multi_asset_snapshot_v3()
        decisions={x.get("asset"):x for x in snap.get("assets",[])}
        ...
        green="#36c98f";red="#f0646d";amber="#f0b44d"
        fig=plt.figure(figsize=(19.2,10.8),facecolor=bg)
        gs=fig.add_gridspec(18,24,left=.025,right=.98,top=.90,bottom=.055,wspace=.7,hspace=.9)
        dt=now_tehran()
        fig.text(.025,.965,"ATLAS  |  IRAN MACRO • METALS • FUNDS",color=fg,fontsize=24,weight="bold",va="top")
        fig.text(.98,.965,f"{shamsi(dt)}  •  {dt.strftime('%H:%M')} Tehran  •  SUPABASE DATA PLANE",
                 ha="right",color=muted,fontsize=9,va="top")

        ax=fig.add_subplot(gs[0:7,:]);ax.set_facecolor(panel);ax.axis("off")
        specs=[("USD • TOMAN","USD_IRR",.1),("USDT • TOMAN","USDT_IRR",.1),
               ("GOLD 18K","GOLD18",1),("EMAMI COIN","COIN_EMAMI",1),
               ("SILVER FAIR • TOMAN/g","SILVER999_FAIR",1),
               ("GOLD XAU","XAUUSD",1),("SILVER XAG","XAGUSD",1),("COPPER","COPPER",1)]
        for i,(label,sym,scale) in enumerate(specs):
            rr,cc=divmod(i,4);x=.015+cc*.247;y=.54-rr*.44
            r=latest.get(sym,{})
            p=(_iran_fx_to_toman(r,sym) if sym in ("USD_IRR","USDT_IRR")
               else safe_float(r.get("price")))
            pay=r.get("payload",{}) if r else {}
            ch=safe_float(pay.get("change_percent") if pay.get("change_percent") is not None else pay.get("pct"))
            if ch is None:
                d=decisions.get(sym,{})
                ch=safe_float(d.get("change_pct"))
            ax.add_patch(Rectangle((x,y),.23,.36,transform=ax.transAxes,facecolor="#102b40",edgecolor=edge,lw=1))
            ax.text(x+.012,y+.255,label,transform=ax.transAxes,color=muted,fontsize=10.5,weight="bold")
            ax.text(x+.012,y+.13,_iran_fmt_price(p) if p is not None else "—",
                    transform=ax.transAxes,color=fg,fontsize=12,weight="bold")
            if ch is not None:
                ax.text(x+.012,y+.045,f"{ch:+.2f}%",transform=ax.transAxes,
                        color=green if ch>=0 else red,fontsize=10.5,weight="bold")

        # Decisions: advisory only for macro/metals.
        ad=fig.add_subplot(gs[8:18,0:9]);ad.set_facecolor(panel);ad.axis("off")
        ad.set_title("ATLAS MACRO ADVISORY",loc="left",color=fg,fontsize=11,weight="bold")
        order=["USD_IRR","USDT_IRR","GOLD18","COIN_EMAMI","SILVER999_FAIR","XAUUSD","XAGUSD","COPPER"]
        for i,key in enumerate(order):
            d=decisions.get(key,{})
            y=.90-i*.105;dec=str(d.get("decision") or "WAIT")
            col=green if dec in ("BUY","ACCUMULATE") else red if dec in ("SELL","REDUCE") else amber
            ad.text(.03,y,key,transform=ad.transAxes,color=fg,fontsize=10.5,weight="bold")
            ad.text(.54,y,f"ADVISORY • {dec}",transform=ad.transAxes,color=col,fontsize=10.5,weight="bold")
            ch=safe_float(d.get("change_pct"))
            ad.text(.97,y,(f"{ch:+.2f}%" if ch is not None else "Δ —"),ha="right",
                    transform=ad.transAxes,color=muted,fontsize=10.5,weight="bold")

        # Funds: market facts only; NAV never fabricated.
        af=fig.add_subplot(gs[8:18,9:24]);af.set_facecolor(panel);af.axis("off")
        af.set_title("ETF / INVESTMENT FUNDS • MARKET DATA",loc="left",color=fg,fontsize=11,weight="bold")
        fs=sorted(fs,key=lambda x:x.get("liquidity") or 0,reverse=True)[:10]
        if not fs:
            af.text(.5,.5,"NO FRESH FUND ROWS",ha="center",transform=af.transAxes,color=amber,fontsize=12,weight="bold")
        else:
            af.text(.02,.94,"FUND",transform=af.transAxes,color=muted,fontsize=9.8,weight="bold")
            af.text(.55,.94,"PRICE",transform=af.transAxes,color=muted,fontsize=9.8,weight="bold")
            af.text(.72,.94,"CHANGE",transform=af.transAxes,color=muted,fontsize=9.8,weight="bold")
            af.text(.97,.94,"TRADE VALUE",ha="right",transform=af.transAxes,color=muted,fontsize=9.8,weight="bold")
            for i,x in enumerate(fs):
                y=.86-i*.082;pct=safe_float(x.get("pct"))
                af.text(.02,y,str(x.get("name") or x.get("symbol"))[:32],transform=af.transAxes,color=fg,fontsize=10.5,weight="bold")
                af.text(.55,y,_iran_fmt_price(x.get("price")),transform=af.transAxes,color=fg,fontsize=10.5,weight="bold")
                af.text(.72,y,f"{pct:+.2f}%" if pct is not None else "—",transform=af.transAxes,
                        color=green if (pct or 0)>0 else red if (pct or 0)<0 else muted,fontsize=10.5,weight="bold")
                af.text(.97,y,_iran_fmt_num(x.get("liquidity"),0),ha="right",transform=af.transAxes,color=muted,fontsize=9.8,weight="bold")
            af.text(.02,.035,"NAV is omitted when provider plan does not supply it • no fabricated premium/discount",
                    transform=af.transAxes,color=muted,fontsize=9.8,weight="bold")
        fig.savefig(filename,dpi=240,bbox_inches="tight",pad_inches=0.10,facecolor=fig.get_facecolor())
        plt.close(fig);return filename
    except Exception as e:
        print(f"⚠️ Iran macro/funds dashboard generation failed: {e}");return None

def build_tse_intelligence_dashboard(filename="atlas_tse_intelligence.png"):
    if not ATLAS_IRAN_DASHBOARD_ENABLED:return None
    try:
        import matplotlib.pyplot as plt
        _iran_set_hq_fonts(plt)
        from matplotlib.patches import Rectangle
        _fx,_metals,tse,_funds=_iran_data_bundle()
        tl=_iran_dash_latest(tse)
        market=(tl.get("TSE_MARKET") or {}).get("payload",{})
        stocks=[v for k,v in tl.items() if k!="TSE_MARKET"]
        stocks=sorted(stocks,key=lambda x:x.get("value") or 0,reverse=True)

        bg="#071522";panel="#0d2233";edge="#25455c";fg="#edf6fb";muted="#9eb3c2"
        green="#36c98f";red="#f0646d";amber="#f0b44d"
        fig=plt.figure(figsize=(19.2,10.8),facecolor=bg)
        gs=fig.add_gridspec(18,24,left=.025,right=.98,top=.90,bottom=.055,wspace=.75,hspace=.9)
        dt=now_tehran()
        fig.text(.025,.965,"ATLAS  |  TEHRAN STOCK EXCHANGE INTELLIGENCE",color=fg,fontsize=24,weight="bold",va="top")
        fig.text(.98,.965,f"{shamsi(dt)}  •  {dt.strftime('%H:%M')} Tehran  •  REAL-MONEY DATA",
                 ha="right",color=muted,fontsize=9,va="top")

        # Heatmap
        ah=fig.add_subplot(gs[0:10,0:16]);ah.set_facecolor(panel);ah.axis("off")
        ah.set_title("LIQUID LEADERS • PRICE ACTION",loc="left",color=fg,fontsize=11,weight="bold")
        for i,r in enumerate(stocks[:40]):
            rr,cc=divmod(i,8);x=.01+cc*.123;y=.78-rr*.18
            pc=safe_float((r.get("payload") or {}).get("pct"))
            face="#14583f" if (pc or 0)>.05 else "#6a2c35" if (pc or 0)<-.05 else "#334657"
            ah.add_patch(Rectangle((x,y),.115,.145,transform=ah.transAxes,facecolor=face,edgecolor=edge,lw=.7))
            ah.text(x+.006,y+.078,str(r.get("symbol"))[:10],transform=ah.transAxes,color=fg,fontsize=9.6,weight="bold")
            ah.text(x+.006,y+.025,f"{pc:+.2f}%" if pc is not None else "—",transform=ah.transAxes,
                    color=green if (pc or 0)>=0 else red,fontsize=9.2,weight="bold")

        breadth=safe_float(market.get("breadth"));adv=safe_float(market.get("advance")) or 0
        dec=safe_float(market.get("decline")) or 0;unch=safe_float(market.get("unchanged")) or 0
        tv=safe_float(market.get("trade_value"));rv=safe_float(market.get("market_real_net_volume"))
        rp=safe_float(market.get("market_real_buyer_power"));rd=str(market.get("market_real_flow_direction") or "—")
        ai=fig.add_subplot(gs[0:10,16:24]);ai.set_facecolor(panel);ai.axis("off")
        ai.set_title("MARKET PULSE • CLIENT-TYPE FLOW",loc="left",color=fg,fontsize=11,weight="bold")
        ai.text(.05,.82,f"{breadth:.1f}" if breadth is not None else "—",transform=ai.transAxes,color=amber,fontsize=34,weight="bold")
        ai.text(.05,.74,"BREADTH %",transform=ai.transAxes,color=muted,fontsize=10.5,weight="bold")
        facts=[
            f"Advance / Decline: {int(adv)} / {int(dec)}",
            f"Unchanged: {int(unch)}",
            f"Trade value: {_iran_fmt_num(tv,0)}",
            f"Net real volume: {rd} • {_iran_flow_fmt(rv)} shares",
            f"Real buyer power: {_iran_fmt_num(rp,2)}x" if rp is not None else "Real buyer power: —",
        ]
        for i,s in enumerate(facts):ai.text(.05,.62-i*.09,s,transform=ai.transAxes,color=fg,fontsize=11.0,weight="bold")
        if breadth is not None and rp is not None:
            regime=("BROAD + REAL DEMAND" if breadth>=55 and rp>1.05 else
                    "BROAD BUT FLOW UNCONFIRMED" if breadth>=55 else
                    "WEAK + REAL SUPPLY" if breadth<45 and rp<.95 else "MIXED / SELECTIVE")
            ai.text(.05,.12,regime,transform=ai.transAxes,color=green if "DEMAND" in regime else red if "SUPPLY" in regime else amber,
                    fontsize=10,weight="bold")

        # Top inflow/outflow by real net volume; uses actual client-type volumes.
        ar=fig.add_subplot(gs[11:18,0:12]);ar.set_facecolor(panel);ar.axis("off")
        ar.set_title("NET REAL-VOLUME LEADERS",loc="left",color=fg,fontsize=11,weight="bold")
        flow=[]
        for r in stocks:
            p=r.get("payload") or {};n=safe_float(p.get("real_net_volume"))
            if n is not None:flow.append((n,r))
        top=sorted(flow,key=lambda z:z[0],reverse=True)[:6]
        bot=sorted(flow,key=lambda z:z[0])[:6]
        ar.text(.02,.88,"INFLOW",transform=ar.transAxes,color=green,fontsize=10.5,weight="bold")
        ar.text(.52,.88,"OUTFLOW",transform=ar.transAxes,color=red,fontsize=10.5,weight="bold")
        for i,(n,r) in enumerate(top):
            y=.78-i*.11;ar.text(.02,y,str(r.get("symbol"))[:12],transform=ar.transAxes,color=fg,fontsize=10.5,weight="bold")
            ar.text(.42,y,_iran_flow_fmt(n),ha="right",transform=ar.transAxes,color=green,fontsize=10.5,weight="bold")
        for i,(n,r) in enumerate(bot):
            y=.78-i*.11;ar.text(.52,y,str(r.get("symbol"))[:12],transform=ar.transAxes,color=fg,fontsize=10.5,weight="bold")
            ar.text(.97,y,_iran_flow_fmt(n),ha="right",transform=ar.transAxes,color=red,fontsize=10.5,weight="bold")

        am=fig.add_subplot(gs[11:18,12:24]);am.set_facecolor(panel);am.axis("off")
        am.set_title("TOP MOVERS • LIQUID UNIVERSE",loc="left",color=fg,fontsize=11,weight="bold")
        movers=[]
        mover_outliers=0
        for r in stocks:
            pc=safe_float((r.get("payload") or {}).get("pct"))
            ...
            if abs(pc)>20.0:
                ...
                continue
            movers.append(r)
        movers=sorted(movers,key=lambda r:safe_float((r.get("payload") or {}).get("pct")) or 0,reverse=True)
        show=movers[:6]+movers[-6:]
        if mover_outliers:
            am.text(.97,.96,f"QC: {mover_outliers} outlier(s) excluded (>20%)",ha="right",
                    transform=am.transAxes,color=muted,fontsize=10.5,weight="bold")
        for i,r in enumerate(show[:12]):
            col=0 if i<6 else 1;j=i if i<6 else i-6;x=.03+col*.50;y=.84-j*.12
            pc=safe_float((r.get("payload") or {}).get("pct"))
            am.text(x,y,str(r.get("symbol"))[:13],transform=am.transAxes,color=fg,fontsize=10.5,weight="bold")
            am.text(x+.42,y,f"{pc:+.2f}%",ha="right",transform=am.transAxes,
                    color=green if pc>=0 else red,fontsize=10.5,weight="bold")

        fig.savefig(filename,dpi=240,bbox_inches="tight",pad_inches=0.10,facecolor=fig.get_facecolor())
        plt.close(fig);return filename
    except Exception as e:
        print(f"⚠️ TSE intelligence dashboard generation failed: {e}");return None

def send_iran_visual_dashboard():
    """Send two Iran images. Together with canonical crypto PNG => three complete dashboards."""
    files=[
        (build_tse_intelligence_dashboard(),"🇮🇷 ATLAS TSE Intelligence"),
        (build_iran_macro_funds_dashboard(),"💱 ATLAS Iran Macro • Metals • Funds"),
    ]
    dests=[]
    for c in (TELEGRAM_GROUP_CHAT_ID,TELEGRAM_CHAT_ID):
        c=str(c or "").strip()
    ...
    for filename,title in files:
        if not filename:
            errs.append(f"{title}: generation returned no file");continue
        cap=f"{title} | {shamsi(now_tehran())} {now_tehran().strftime('%H:%M')} Tehran"
        for c in dests:
            try:
                if _telegram_send_photo_file(c,filename,cap):sent+=1
                else:errs.append(f"{title} -> {c}: send returned false")
            except Exception as e:errs.append(f"{title} -> {c}: {e}")
        try:os.unlink(filename)
        except Exception:pass
    return sent,errs
...
def main():
    ...
    try:
        ...
        # Chat plane owns Telegram ingest via Supabase Edge Function (hyper-endpoint).
        # GitHub Analysis Plane must never call getUpdates in production.
        print("ℹ️ Atlas Desk ingest disabled; owner=atlas-desk-inbox")
        ...
        globals()["_LAST_MARKET_INFO"] = market_info
        ...
        with _AtlasTimer("Evidence + MTF + Risk + Lifecycle"):
            ...
            # ATLAS Intelligence v2: advisory portfolio language only.
            # Canonical decision_state/action and executable trade math remain untouched.
            results = apply_portfolio_layer(
                results, btc_regime, breadth, market_info, news, macro
            )
        ...
        # Domain-timed STRONG signal delivery. This is additive and rare:
        # - CRYPTO/METALS only on the existing Deep4H closed-candle cycle.
        # - TSE only while Tehran regular session is open and Data Plane is fresh.
        # It does not alter scheduler mode flags, canonical signals, Desk or Execution.
        try:
            with _AtlasTimer("DOMAIN STRONG SIGNAL TIMING"):
                _ss,_se=send_strong_signals_for_cycle(
                    results, metal_results,
                    deep_cycle=bool(deep_cycle),
                    tse_check=True,
                )
            total_sent += _ss
            all_errors.extend(_se)
        except Exception as e:
            append_changelog("DOMAIN_STRONG_SIGNAL_TIMING",None,None,str(e))
            print(f"⚠️ Domain strong-signal timing failed non-fatally: {e}")
        ...
        if daily_cycle:
            ...
            with _AtlasTimer("PHASE310 FULL DAILY16 DELIVERY"):
                sent, errors, parts = _phase310_send_full_daily16(
                    results, scoped_results, top10, macro, news, btc_regime, portfolio_risk, metal_results=metal_results
                )
            total_sent += sent
            ...
            # MANDATORY CORE DASHBOARD 1/3 — Crypto. Never delete/merge from DAILY16.
            ...
            # MANDATORY CORE DASHBOARDS 2/3 + 3/3 — TSE and Iran Macro/Metals/Funds.
            # Never delete/merge from DAILY16; generated from the fresh Data Plane.
            try:
                if IRAN_MULTI_ASSET_ENABLED and ATLAS_IRAN_DASHBOARD_ENABLED:
                    with _AtlasTimer("ATLAS IRAN VISUAL DASHBOARD"):
                        _is, _ie = send_iran_visual_dashboard()
                    print(f"🇮🇷 Iran dashboard: sent={_is}, errors={len(_ie)}")
                    for _err in _ie[:3]: print(f"⚠️ Iran dashboard: {_err}")
            except Exception as e:
            ...
            try:
                with _AtlasTimer("ATLAS PERSONAL PORTFOLIO DASHBOARDS"):
                ...
                for _err in (personal_dash.get("errors") or [])[:3]:
                    print(f"⚠️ Personal dashboard: {_err}")
            except Exception as e:
                append_changelog("PERSONAL_DASHBOARDS", None, None, str(e))
                print(f"⚠️ Personal dashboards failed non-fatally: {e}")
        elif nightly_cycle and ATLAS_NIGHTLY_BRIEF_ENABLED:
            try:
                telegram_preflight()
            except Exception as e:
                print(f"⚠️ Telegram preflight failed; nightly send will still retry: {e}")
            with _AtlasTimer("ATLAS INTELLIGENCE V2 NIGHTLY23"):
                _intel_prev = load_previous_intelligence_report_results(
                    "NIGHTLY23", current_results=results, lookback_hours=72
                )
                intel_v2_text = build_intelligence_report_v2(
                    results, btc_regime=btc_regime, breadth=breadth, macro=macro, news=news,
                    market_info=market_info, previous_results=_intel_prev, cycle="NIGHTLY23"
                )
                _iparts, _isent, _ierrors = send_report(intel_v2_text)
                if IRAN_MULTI_ASSET_ENABLED and IRAN_REPORT_NIGHTLY23:
                    send_iran_markets_report_v3(metal_results)
                    if ATLAS_IRAN_DASHBOARD_NIGHTLY:
                        _nis, _nie = send_iran_visual_dashboard()
                        print(f"🇮🇷 Iran Nightly dashboard: sent={_nis}, errors={len(_nie)}")
                total_sent += _isent
                all_errors.extend(_ierrors)
                print(f"🧠 Intelligence v2 Nightly23: parts={_iparts}, sent={_isent}, errors={len(_ierrors)}")
        ...
        # Personal Allocation is an Analysis-Plane persistence task.
        # Run on every DEEP4H cycle (including DAILY16), independent of Telegram/book-scan delivery.
        # DAILY16 already runs the same cycle in its delivery block above, so avoid a duplicate write there.
        if deep_cycle and not daily_cycle:
            try:
                with _AtlasTimer("ATLAS PERSONAL ALLOCATION ENGINE DEEP4H"):
                    _alloc = run_personal_allocation_cycle(scoped_results)
                print("🧠 Personal allocation DEEP4H:", "users=", _alloc.get("users"), "written=", _alloc.get("written"), "errors=", len(_alloc.get("errors") or []))
                for _err in (_alloc.get("errors") or [])[:3]:
                    print(f"⚠️ Personal allocation DEEP4H: {_err}")
            except Exception as e:

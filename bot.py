...
def _iran_dash_latest(rows):
    ...
    for row in rows or []:
        ...
        if s and s not in out:
            out[s]={"symbol":s,"price":safe_float(row.get("price") or p.get("price")),
                    "value":safe_float(row.get("value") or p.get("trade_value")),
...
def build_iran_visual_dashboard(filename="atlas_iran_markets.png"):
    ...
    try:
        import matplotlib.pyplot as plt
        from matplotlib.patches import Rectangle
        fx=_v3_rows("fx",400); tse=_v3_rows("tse",500); funds=_v3_rows("funds",500); metals=_v3_rows("metals",500)
        ...
        green="#36c98f"; red="#f0646d"; amber="#f0b44d"; blue="#4da3ff"
        fig=plt.figure(figsize=(19.2,10.8),facecolor=bg)
        gs=fig.add_gridspec(18,30,left=.018,right=.987,top=.91,bottom=.045,wspace=.65,hspace=.85)
        def pnl(spec,title):
            ax=fig.add_subplot(spec);ax.set_facecolor(panel)
            for s in ax.spines.values():s.set_color(edge)
            ax.tick_params(colors=muted,labelsize=7);ax.set_title(title,loc="left",color=fg,fontsize=11,weight="bold",pad=7)
            return ax
        dt=now_tehran()
        fig.text(.025,.968,"ATLAS  |  IRAN MARKETS OVERVIEW",color=fg,fontsize=25,weight="bold",va="top")
        fig.text(.985,.968,f"{shamsi(dt)}  •  {dt.strftime('%H:%M')} Tehran  •  LIVE DATA PLANE",ha="right",color=muted,fontsize=9,va="top")

        # Top market cards
        ax=pnl(gs[0:4,0:30],"MARKET SNAPSHOT");ax.axis("off")
        specs=[("USD","USD_IRR"),("USDT","USDT_IRR"),("GOLD 18K","GOLD18"),("EMAMI COIN","COIN_EMAMI"),
               ("SILVER 999","SILVER999"),("SILVER FAIR","SILVER999_FAIR"),("GOLD XAU","XAUUSD"),("SILVER XAG","XAGUSD"),("COPPER","COPPER")]
        for i,(label,sym) in enumerate(specs):
            x=.012+i*.1235; w=.116
            a=latest.get(sym,{})
            p=a.get("price"); pay=a.get("payload",{}); ch=safe_float(pay.get("pct") or pay.get("change"))
            ax.add_patch(Rectangle((x,.16),w,.68,transform=ax.transAxes,facecolor="#102b40",edgecolor=edge,lw=1))
            ax.text(x+.01,.68,label,transform=ax.transAxes,color=muted,fontsize=8,weight="bold")
            ax.text(x+.01,.43,_iran_fmt_price(p) if p is not None else "DATA MISSING",transform=ax.transAxes,color=fg,fontsize=11,weight="bold")
            if ch is not None:ax.text(x+.01,.24,f"{ch:+.2f}%",transform=ax.transAxes,color=green if ch>=0 else red,fontsize=9,weight="bold")

        # TSE heatmap from most liquid stored symbols
        ah=pnl(gs[4:12,0:18],"TEHRAN STOCK EXCHANGE • LIQUID LEADERS HEATMAP");ah.axis("off")
        ...
        for i,r in enumerate(stocks):
            ...
            ah.text(x+.008,y+.09,r["symbol"][:12],transform=ah.transAxes,color=fg,fontsize=7.4,weight="bold")
            ah.text(x+.008,y+.027,f"{pc:+.2f}%",transform=ah.transAxes,color=green if pc>=0 else red,fontsize=7.2,weight="bold")

        # Market pulse
        ap=pnl(gs[4:8,18:24],"TSE MARKET PULSE");ap.axis("off")
        breadth=safe_float(market.get("breadth")); flow=safe_float(market.get("real_money_flow"))
        pulse=50 if breadth is None else max(0,min(100,breadth))
        ap.text(.5,.58,f"{pulse:.0f}",ha="center",transform=ap.transAxes,color=green if pulse>=55 else red if pulse<=45 else amber,fontsize=35,weight="bold")
        ap.text(.5,.30,"BREADTH",ha="center",transform=ap.transAxes,color=muted,fontsize=9)
        ap.text(.5,.12,f"Real money: {_iran_fmt_num(flow,0) if flow is not None else 'N/A'}",ha="center",transform=ap.transAxes,color=fg,fontsize=8)

        # Movers
        am=pnl(gs[4:12,24:30],"TOP MOVERS");am.axis("off")
        ...
        for i,r in enumerate((movers[:5]+movers[-5:])[:10]):
            ...
            am.text(.05,y,r["symbol"][:13],transform=am.transAxes,color=fg,fontsize=8,weight="bold")
            am.text(.94,y,f"{pc:+.2f}%",ha="right",transform=am.transAxes,color=green if pc>=0 else red,fontsize=8,weight="bold")

        # Money flow / breadth bar
        af=pnl(gs[12:18,0:9],"MARKET FLOW & BREADTH")
        adv=safe_float(market.get("advance")) or 0; dec=safe_float(market.get("decline")) or 0; unchanged=safe_float(market.get("unchanged")) or 0
        vals=[adv,unchanged,dec]; names=["Advance","Flat","Decline"]
        af.bar(names,vals);af.grid(axis="y",alpha=.12);af.set_ylabel("Symbols",color=muted,fontsize=8)
        af.text(.98,.92,f"Trade value: {_iran_fmt_num(market.get('trade_value'),0)}",ha="right",transform=af.transAxes,color=fg,fontsize=8)
        af.text(.98,.83,f"Volume: {_iran_fmt_num(market.get('volume'),0)}",ha="right",transform=af.transAxes,color=fg,fontsize=8)

        # Funds: real ETF market snapshots. NAV is optional enrichment.
        au=pnl(gs[12:18,9:19],"ETF FUNDS • MARKET DATA");au.axis("off")
        fs=sorted(fund_rows,key=lambda x:x.get("liquidity") or 0,reverse=True)[:9]
        if not fs:
            au.text(.5,.5,"ETF MARKET DATA MISSING",ha="center",transform=au.transAxes,color=amber,fontsize=11,weight="bold")
        else:
            for i,x in enumerate(fs):
                y=.90-i*.095
                au.text(.03,y,str(x.get("name"))[:17],transform=au.transAxes,color=fg,fontsize=7.5)
                au.text(.58,y,_iran_fmt_price(x.get("price")),transform=au.transAxes,color=fg,fontsize=7.3)
                if x.get("nav") is not None:
                    au.text(.97,y,f"P/N {_iran_fmt_num(x.get('premium'),2)}%",ha="right",transform=au.transAxes,color=amber,fontsize=7.3)
                else:
                    au.text(.97,y,"NAV N/A",ha="right",transform=au.transAxes,color=muted,fontsize=7.3)

        # Relative returns / decisions
        ar=pnl(gs[12:18,19:25],"ATLAS MULTI-ASSET DECISIONS");ar.axis("off")
        snap=build_iran_multi_asset_snapshot_v3()
        valid=[x for x in snap.get("assets",[]) if x.get("status")!="DATA_MISSING"][:9]
        for i,x in enumerate(valid):
            y=.90-i*.095; d=str(x.get("decision") or "WAIT")
            col=green if d in ("BUY","ACCUMULATE") else red if d in ("SELL","REDUCE") else amber
            ar.text(.03,y,x["asset"][:13],transform=ar.transAxes,color=fg,fontsize=7.5)
            ar.text(.97,y,d,ha="right",transform=ar.transAxes,color=col,fontsize=7.5,weight="bold")

        # Summary
        ai=pnl(gs[12:18,25:30],"ATLAS SUMMARY");ai.axis("off")
        lines=[
          f"Breadth: {_iran_fmt_num(breadth,1)}%" if breadth is not None else "Breadth: DATA MISSING",
          f"Real money: {_iran_fmt_num(flow,0)}" if flow is not None else "Real money: DATA MISSING",
          f"ETF funds tracked: {len(fund_rows)}",
          f"NAV available: {sum(1 for x in fund_rows if x.get('nav') is not None)}/{len(fund_rows)}",
          "Fail-closed • no fabricated NAV",
          "Daily16 + Nightly23 PNG"
        ]
        for i,line in enumerate(lines):ai.text(.05,.88-i*.14,"• "+line,transform=ai.transAxes,color=fg,fontsize=8,va="top")
        fig.text(.025,.015,"ATLAS Iran Data Plane → Supabase → Intelligence → PNG → Telegram Group  |  Not financial advice",color=muted,fontsize=7.5)
        plt.savefig(filename,dpi=170,bbox_inches="tight",facecolor=fig.get_facecolor());plt.close(fig)
        return filename
    except Exception as e:
        print(f"⚠️ Iran dashboard generation failed: {e}");return None

...
def _alloc_weights(cands, profile):
    ...
    for g,items in groups.items():
        ...
        for c,raw in zip(active,weights):
            w=min(cfg["single_cap"],budgets[g]*raw/den)
            if w>=0.75:
    ...
    def rate_irr(aliases):
        for a in aliases:
            ...
            if r and safe_float(r.get("price")) is not None:
                ...
                if "TOMAN" in ur: return p*10.0
                if "IRR" in uc: return p
                # FX fallback: large quote is IRR, smaller quote is Toman.
...
def _alloc_snapshot_for_user(uid, user, bag, scoped_results):
    ...
    payload={
        ...
        "diagnostics":diag,
        "method":"quality_gate_cross_asset_dynamic_cash_v2",

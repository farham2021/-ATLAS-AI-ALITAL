...
def build_iran_visual_dashboard(filename="atlas_iran_markets.png"):
    ...
    try:
        ...
        plt.savefig(filename,dpi=220,bbox_inches="tight",pad_inches=0.10,facecolor=fig.get_facecolor())
        plt.close(fig)
        return filename
    except Exception as e:
...
def build_iran_macro_funds_dashboard(filename="atlas_iran_macro_funds.png"):
    ...
    try:
        ...
        for i,(label,sym,scale) in enumerate(specs):
            ...
            # Canonical FX is IRR; user-facing cards are explicitly Toman.
            if p is not None and sym in ("USD_IRR","USDT_IRR"):p/=10.0
            ...
            ax.text(x+.012,y+.255,label,transform=ax.transAxes,color=muted,fontsize=9.5,weight="bold")
            ...
            if ch is not None:
                ax.text(x+.012,y+.045,f"{ch:+.2f}%",transform=ax.transAxes,
                        color=green if ch>=0 else red,fontsize=9.5,weight="bold")
        ...
        for i,key in enumerate(order):
            ...
            ad.text(.03,y,key,transform=ad.transAxes,color=fg,fontsize=9.5,weight="bold")
            ad.text(.54,y,f"ADVISORY • {dec}",transform=ad.transAxes,color=col,fontsize=9.5,weight="bold")
            ch=safe_float(d.get("change_pct"))
            ad.text(.97,y,(f"{ch:+.2f}%" if ch is not None else "Δ —"),ha="right",
                    transform=ad.transAxes,color=muted,fontsize=9.0)
        ...
        fs=sorted(fs,key=lambda x:x.get("liquidity") or 0,reverse=True)[:12]
        if not fs:
            af.text(.5,.5,"NO FRESH FUND ROWS",ha="center",transform=af.transAxes,color=amber,fontsize=12,weight="bold")
        else:
            af.text(.02,.94,"FUND",transform=af.transAxes,color=muted,fontsize=8.5,weight="bold")
            af.text(.55,.94,"PRICE",transform=af.transAxes,color=muted,fontsize=8.5,weight="bold")
            af.text(.72,.94,"CHANGE",transform=af.transAxes,color=muted,fontsize=8.5,weight="bold")
            af.text(.97,.94,"TRADE VALUE",ha="right",transform=af.transAxes,color=muted,fontsize=8.5,weight="bold")
            for i,x in enumerate(fs):
                y=.87-i*.066;pct=safe_float(x.get("pct"))
                af.text(.02,y,str(x.get("name") or x.get("symbol"))[:32],transform=af.transAxes,color=fg,fontsize=9.0)
                af.text(.55,y,_iran_fmt_price(x.get("price")),transform=af.transAxes,color=fg,fontsize=9.0)
                af.text(.72,y,f"{pct:+.2f}%" if pct is not None else "—",transform=af.transAxes,
                        color=green if (pct or 0)>0 else red if (pct or 0)<0 else muted,fontsize=9.0)
                af.text(.97,y,_iran_fmt_num(x.get("liquidity"),0),ha="right",transform=af.transAxes,color=muted,fontsize=8.5,weight="bold")
            af.text(.02,.035,"NAV is omitted when provider plan does not supply it • no fabricated premium/discount",
                    transform=af.transAxes,color=muted,fontsize=8.5,weight="bold")
        fig.savefig(filename,dpi=220,bbox_inches="tight",pad_inches=0.10,facecolor=fig.get_facecolor())
...
def build_tse_intelligence_dashboard(filename="atlas_tse_intelligence.png"):
    ...
    try:
        ...
        for i,r in enumerate(stocks[:40]):
            ...
            ah.text(x+.006,y+.078,str(r.get("symbol"))[:10],transform=ah.transAxes,color=fg,fontsize=8.2,weight="bold")
            ah.text(x+.006,y+.025,f"{pc:+.2f}%" if pc is not None else "—",transform=ah.transAxes,
                    color=green if (pc or 0)>=0 else red,fontsize=8.0,weight="bold")
        ...
        ai.set_title("MARKET PULSE • REAL MONEY",loc="left",color=fg,fontsize=11,weight="bold")
        ai.text(.05,.82,f"{breadth:.1f}" if breadth is not None else "—",transform=ai.transAxes,color=amber,fontsize=34,weight="bold")
        ai.text(.05,.74,"BREADTH %",transform=ai.transAxes,color=muted,fontsize=9.5,weight="bold")
        facts=[
            f"Advance / Decline: {int(adv)} / {int(dec)}",
            f"Unchanged: {int(unch)}",
            f"Trade value: {_iran_fmt_num(tv,0)}",
            f"Real flow: {rd} • {_iran_flow_fmt(rv)}",
            f"Real buyer power: {_iran_fmt_num(rp,2)}x" if rp is not None else "Real buyer power: —",
        ]
        for i,s in enumerate(facts):ai.text(.05,.62-i*.09,s,transform=ai.transAxes,color=fg,fontsize=10.0)
        ...
        ar.set_title("REAL-MONEY LEADERS",loc="left",color=fg,fontsize=11,weight="bold")
        flow=[]
        for r in stocks:
        ...
        ar.text(.02,.88,"INFLOW",transform=ar.transAxes,color=green,fontsize=9.5,weight="bold")
        ar.text(.52,.88,"OUTFLOW",transform=ar.transAxes,color=red,fontsize=9.5,weight="bold")
        for i,(n,r) in enumerate(top):
            y=.78-i*.11;ar.text(.02,y,str(r.get("symbol"))[:12],transform=ar.transAxes,color=fg,fontsize=9.5,weight="bold")
            ar.text(.42,y,_iran_flow_fmt(n),ha="right",transform=ar.transAxes,color=green,fontsize=9.5,weight="bold")
        for i,(n,r) in enumerate(bot):
            y=.78-i*.11;ar.text(.52,y,str(r.get("symbol"))[:12],transform=ar.transAxes,color=fg,fontsize=9.5,weight="bold")
            ar.text(.97,y,_iran_flow_fmt(n),ha="right",transform=ar.transAxes,color=red,fontsize=9.5,weight="bold")
        ...
        movers=[r for r in stocks if safe_float((r.get("payload") or {}).get("pct")) is not None]
        ...
        for i,r in enumerate(show[:12]):
            ...
            pc=safe_float((r.get("payload") or {}).get("pct"))
            am.text(x,y,str(r.get("symbol"))[:13],transform=am.transAxes,color=fg,fontsize=9.5,weight="bold")
            am.text(x+.42,y,f"{pc:+.2f}%",ha="right",transform=am.transAxes,
                    color=green if pc>=0 else red,fontsize=9.5,weight="bold")

...
def build_iran_markets_report_v3(metal_results=None):
    ...
    for a in assets:
        ...
        lines.append(f"{a['asset']}: {a['decision']} | P={_iran_fmt_price(a.get('price'))} | RSI={_iran_fmt_num(a.get('rsi14'),1)} | S={_iran_fmt_price(a.get('support'))} R={_iran_fmt_price(a.get('resistance'))} | C={a['confidence']}%")
...
def build_iran_visual_dashboard(filename="atlas_iran_markets.png"):
    ...
    try:
        ...
        tse_lines=[
            ...
            f"Volume: {_iran_fmt_num(vol,0)}",
            "Interpretation: breadth-based; no invented index signal."
        ]
        ...
        if not fs:
            ...
            for i,x in enumerate(fs):
                ...
                au.text(.62,y,_iran_fmt_price(x.get("price")),transform=au.transAxes,color=fg,fontsize=7.1)
                au.text(.97,y,"NAV N/A" if x.get("nav") is None else f"P/N {_iran_fmt_num(x.get('premium'),2)}%",
                        ha="right",transform=au.transAxes,color=muted if x.get("nav") is None else amber,fontsize=7.1)
        ...
        lines=[
            ...
            f"Fixed-income verified: {len(fixed)}",
            "Fail-closed • no fabricated NAV/signal"
        ]

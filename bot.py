...
def _daily16_management_png(kind, results, scoped_results, portfolio_risk=None, filename=None):
    """Three compact management PNGs. They are additive to the 3 mandatory dashboards."""
    try:
        import matplotlib.pyplot as plt
        from matplotlib.patches import Rectangle
        _iran_set_hq_fonts(plt)
        ...
        dt=now_tehran()
        ranked=_atlas_visual_rank(results or [])
        fig=plt.figure(figsize=(16,10),facecolor=bg)
        ax=fig.add_axes([.035,.07,.93,.84]);ax.axis("off")

        if kind==1:
            title="ATLAS • MARKET + DEEP ANALYSIS"
            ...
            ax.text(.02,.94,f"Breadth  {adv}↑  {dec}↓  {flat}→",transform=ax.transAxes,color=fg,fontsize=18,weight="bold")
            y=.84
            for r in ranked[:10]:
                sym=_aio_symbol(r);sig=_atlas_public_signal(r)
                conf=max(_aio_num(r.get("confidence"),0),_aio_num(r.get("decision_confidence"),0))
                h4=str(r.get("h4_trend") or "UNKNOWN");d1=str(r.get("d1_trend") or "UNKNOWN")
                reasons=why_not_trade(r)[:2] if sig=="WAIT" else []
                ax.add_patch(Rectangle((.01,y-.055),.98,.08,transform=ax.transAxes,facecolor=panel,edgecolor="#25455c"))
                col=green if sig=="BUY" else red if sig=="SELL" else amber
                ax.text(.03,y,f"{sym} • {sig}",transform=ax.transAxes,color=col,fontsize=12.5,weight="bold",va="center")
                ax.text(.22,y,f"C {conf:.0f}%  | H4 {h4} | D1 {d1}",transform=ax.transAxes,color=fg,fontsize=10.5,va="center")
                if reasons:ax.text(.98,y," • ".join(reasons),ha="right",transform=ax.transAxes,color=muted,fontsize=8.5,va="center")
                y-=.085
            footer="Top10 + Personal deep view • full evidence/history stored in Supabase"

        elif kind==2:
            title="ATLAS • DECISION + OPPORTUNITY + TRADE PLANS"
            confirmed=[r for r in ranked if _atlas_public_signal(r) in ("BUY","SELL")][:5]
            watch=_daily16_best_watch(results,5)
            plans=_daily16_trade_plan_rows(results,5)
            y=.91
            ax.text(.02,y,"CONFIRMED ACTION",transform=ax.transAxes,color=blue,fontsize=14,weight="bold");y-=.055
            if not confirmed:
                ax.text(.04,y,"No confirmed BUY/SELL",transform=ax.transAxes,color=muted,fontsize=11);y-=.07
            else:
                for r in confirmed:
                    ax.text(.04,y,_atlas_market_board_line(r),transform=ax.transAxes,color=fg,fontsize=10.5);y-=.055
            ax.text(.02,y,"BEST WATCH / PRE-TRIGGER",transform=ax.transAxes,color=blue,fontsize=14,weight="bold");y-=.055
            if not watch:
                ax.text(.04,y,"No qualified WAIT",transform=ax.transAxes,color=muted,fontsize=11);y-=.07
            else:
                for r in watch:
                    score=max(_aio_num(r.get("opportunity_score"),0),_aio_num(r.get("score"),0))
                    ax.text(.04,y,f"{_aio_symbol(r)} • WAIT • O {score:.0f} • MTF {_aio_num(r.get('mtf_agreement_pct'),0):.0f}%",
                            transform=ax.transAxes,color=fg,fontsize=10.5);y-=.055
            ax.text(.02,y,"EXECUTABLE TRADE PLANS",transform=ax.transAxes,color=blue,fontsize=14,weight="bold");y-=.055
            if not plans:
                ax.text(.04,y,"No executable Entry / SL / TP plan",transform=ax.transAxes,color=muted,fontsize=11)
            else:
                for r in plans:
                    ax.text(.04,y,f"{_aio_symbol(r)} • {_atlas_public_signal(r)} | E {fmt(r.get('entry'))} | SL {fmt(r.get('sl'))} | TP1 {fmt(r.get('tp1'))} | RR {_aio_num(r.get('rr'),0):.2f}",
                            transform=ax.transAxes,color=fg,fontsize=10.5);y-=.06
            footer="WAIT is never promoted to BUY/SELL • canonical Entry/SL/TP only"

        else:
            title="ATLAS • PERSONAL PORTFOLIO + SIGNAL LIFECYCLE"
            holdings={}
            try:
                if callable(globals().get("desk_load_holdings")):
                    holdings=desk_load_holdings(TELEGRAM_CHAT_ID) or {}
            except Exception:
                holdings={}
            y=.91
            ax.text(.02,y,"PERSONAL PORTFOLIO",transform=ax.transAxes,color=blue,fontsize=14,weight="bold");y-=.06
            if holdings:
                by={_aio_symbol(r):r for r in scoped_results or []}
                for sym,qty in list(holdings.items())[:10]:
                    r=by.get(str(sym).upper(),{})
                    px=safe_float(r.get("price"))
                    sig=_atlas_public_signal(r) if r else "N/A"
                    val=(safe_float(qty) or 0)*(px or 0)
                    ax.text(.04,y,f"{sym}: {fmt(qty)} | {sig} | Price {fmt(px) if px is not None else 'N/A'} | Value {fmt(val) if px is not None else 'N/A'}",
                            transform=ax.transAxes,color=fg,fontsize=10.2);y-=.055
            else:
                ax.text(.04,y,"Portfolio holdings: unavailable in this analysis context",transform=ax.transAxes,color=muted,fontsize=10.5);y-=.07
            ax.text(.02,y,"SIGNAL LIFECYCLE",transform=ax.transAxes,color=blue,fontsize=14,weight="bold");y-=.06
            for sym,sig,life in _daily16_signal_lifecycle_summary(results,6):
                col=green if sig=="BUY" else red if sig=="SELL" else amber
                ax.text(.04,y,f"{sym} • {sig} • {life}",transform=ax.transAxes,color=col,fontsize=10.5,weight="bold");y-=.055
            # Risk: only compact factual headline if already computed.
            if portfolio_risk:
                ax.text(.02,max(.08,y-.015),"RISK CONTEXT",transform=ax.transAxes,color=blue,fontsize=13,weight="bold")
                risk_txt=str(portfolio_risk)
                if len(risk_txt)>180:risk_txt=risk_txt[:177]+"..."
                ax.text(.04,max(.03,y-.07),risk_txt,transform=ax.transAxes,color=muted,fontsize=8.8,wrap=True)
            footer="Portfolio state + lifecycle summary • detailed risk/lifecycle history stays in Supabase"

        fig.text(.035,.965,title,color=fg,fontsize=22,weight="bold",va="top")
        fig.text(.965,.965,f"{dt.strftime('%Y-%m-%d %H:%M')} Tehran",ha="right",color=muted,fontsize=10,va="top")
        fig.text(.035,.02,footer,color=amber,fontsize=9,weight="bold")

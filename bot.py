...
EVENT_DEDUP_ENABLED = os.environ.get("ATLAS_CANDLE_EVENT_DEDUP", "0").strip() != "0" 
...
VOICE & IMAGE SETTINGS - فعال برای تولید
...
MIN_CONFIDENCE = float(os.environ.get("ATLAS_MIN_CONFIDENCE", "35")) 
...
MIN_VOLUME_RATIO = float(os.environ.get("ATLAS_MIN_VOLUME_RATIO", "0.30")) 
H4_FALLBACK_MIN_SCORE = float(os.environ.get("ATLAS_H4_FALLBACK_MIN_SCORE", "50")) 
REQUEST_SLEEP_SECONDS = float(os.environ.get("ATLAS_REQUEST_SLEEP_SECONDS", "0.50"))


TRADE GEOMETRY SETTINGS

MIN_EXECUTABLE_RR = float(os.environ.get("ATLAS_MIN_EXECUTABLE_RR", "1.0")) 
MIN_WATCH_CONFIDENCE = float(os.environ.get("ATLAS_MIN_WATCH_CONFIDENCE", "35")) 
...
def get_run_mode(): 
mode = os.environ.get("ATLAS_RUN_MODE", "BOTH").strip().upper() 
...
pnl=((exit_price-entry)/entry100) if row["direction"]"LONG" else ((entry-exit_price)/entry100) 
with sqlite_conn() as c: 
...
for dest in destinations:
    ...
    # Duplicate check is disabled by default (ATLAS_CANDLE_EVENT_DEDUP=0)
    # but we still skip if explicitly enabled
    if EVENT_DEDUP_ENABLED:
        with sqlite_conn() as c:
            already = c.execute(
...
for r in results[:5]: 
print(f"  {r.get('coin')}: action={r.get('action')}, price={r.get('price')}") 

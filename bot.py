...
def fetch_whale_activity():
    """تراکنش‌های بزرگ (نهنگ) اخیر از Whale Alert. نیازمند WHALE_ALERT_API_KEY؛
    اگر تنظیم نشده باشد، بی‌صدا لیست خالی برمی‌گرداند."""
    ...
    if NEWSAPI_KEY:
        try:
            ...
            for x in d.get("articles", []) if isinstance(d, dict) else []:
                title = x.get("title", "")
                link = x.get("url", "")
                person = keyword_hit(title) or "سایر چهره‌های تأثیرگذار"
...
def build_intelligence_briefing(macro, fed_macro, whale_items, figure_items):
    ...
    if figure_items:
        ...
        for it in figure_items[:5]:
            text_lines.append(f"   [{it.get('figure')}] {it.get('title')} ({it.get('source')})")
        voice_bits.append(
            f"همچنین {figure_items[0].get('figure')} اظهاراتی مطرح کرده که ممکن است روی بازار اثر بگذارد: "
            f"{figure_items[0].get('title')}."
        )

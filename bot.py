...
def _phase310_send_full_daily16(results, scoped_results, top10, macro, news, btc_regime, portfolio_risk, metal_results=None):
    ...
    # D) Extra strong-signal image only for verified BUY/SELL.
    # No strong signal => no extra image. It never replaces core dashboards.
    try:
        s,e=send_daily16_strong_signal_png(results,metal_results)
        sent_total+=s; errors.extend(e)
        _daily16_diag("STRONG_SIGNAL_PNG",s,e)
    except Exception as ex:
        ee=f"STRONG_SIGNAL_PNG_DAILY16: {ex}"; errors.append(ee); _daily16_diag("STRONG_SIGNAL_PNG",0,[ee])

    # E) Voice stays available but uses the same already-computed state.
    try:

...
def build_iran_markets_report_v3(metal_results=None):
    ...
    for a in assets:
        ...
        lines.append(f"{a['asset']}: {a['decision']} | P={fmt_price(a.get('price'))} | RSI={fmt_num(a.get('rsi14'),1)} | S={fmt_price(a.get('support'))} R={fmt_price(a.get('resistance'))} | C={a['confidence']}%")
    ...
    if not fs:
        ...
        for x in fs[:8]:
            nav_txt=(f"NAV={fmt_price(x.get('nav'))} | premium={fmt_num(x.get('premium'),2)}%"
                     if x.get("nav") is not None else
                     f"NAV={x.get('nav_status') or 'UNAVAILABLE'}")
            lines.append(f"{x['name']}: P={fmt_price(x.get('price'))} | {nav_txt} | liquidity={fmt_num(x.get('liquidity'),0)}")
...
def build_iran_visual_dashboard(filename="atlas_iran_markets.png"):
    ...
    try:
        ...
        for i,(label,sym) in enumerate(specs):
            ...
            ax.text(x+.01,.43,fmt_price(p) if p is not None else "DATA MISSING",transform=ax.transAxes,color=fg,fontsize=11,weight="bold")
        ...
        ap.text(.5,.12,f"Real money: {fmt_num(flow,0) if flow is not None else 'N/A'}",ha="center",transform=ap.transAxes,color=fg,fontsize=8)
        ...
        af.text(.98,.92,f"Trade value: {fmt_num(market.get('trade_value'),0)}",ha="right",transform=af.transAxes,color=fg,fontsize=8)
        af.text(.98,.83,f"Volume: {fmt_num(market.get('volume'),0)}",ha="right",transform=af.transAxes,color=fg,fontsize=8)
        ...
        if not fs:
            ...
            for i,x in enumerate(fs):
                ...
                au.text(.58,y,fmt_price(x.get("price")),transform=au.transAxes,color=fg,fontsize=7.3)
                if x.get("nav") is not None:
                    au.text(.97,y,f"P/N {fmt_num(x.get('premium'),2)}%",ha="right",transform=au.transAxes,color=amber,fontsize=7.3)
                else:
        ...
        lines=[
          f"Breadth: {fmt_num(breadth,1)}%" if breadth is not None else "Breadth: DATA MISSING",
          f"Real money: {fmt_num(flow,0)}" if flow is not None else "Real money: DATA MISSING",

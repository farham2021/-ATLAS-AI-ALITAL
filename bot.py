...
def _alloc_cash_state(bag):
    ...
    fx=_iran_dash_latest(_v3_rows("fx",800))
    def rate_info(aliases):
        for a in aliases:
            r=fx.get(a)
            if not r or safe_float(r.get("price")) is None: continue
            p=float(r["price"]); pay=r.get("payload") if isinstance(r.get("payload"),dict) else {}
            uc=str(pay.get("unit_canonical") or "").upper(); ur=str(pay.get("unit_raw") or pay.get("unit") or "").upper()
            if "IRR" in uc: rate=p
            elif "TOMAN" in uc: rate=p*10.0
            ...
            if age_h is not None and age_h>4.0: continue
            return {"rate":rate,"source":str(pay.get("provider") or "state"),"captured_at":ts,"age_h":age_h,"symbol":a}
        return None

    infos={
        "USD":rate_info(("USD_IRR","USD","USD_FREE","DOLLAR")),
        "USDT":rate_info(("USDT_IRR","USDT")),
        "EUR":rate_info(("EUR_IRR","EUR","EURO")),
        "THB":rate_info(("THB_IRR","THB","BAHT")),
...
def _alloc_snapshot_for_user(uid, user, bag, scoped_results):
    ...
    payload={
        ...
        "diagnostics":diag,
        "method":"quality_gate_cross_asset_dynamic_cash_v4_fresh_fx_diversification",

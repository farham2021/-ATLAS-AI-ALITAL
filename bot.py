...
def _phase310_send_full_daily16(results, scoped_results, top10, macro, news, btc_regime, portfolio_risk, metal_results=None):
    ...
    # 5.5) ATLAS Intelligence Report v2 — compact operational portfolio layer.
    # Additive only: does not replace any canonical report or execution surface.
    try:
        _intel_prev = load_previous_intelligence_report_results(
            "DAILY16", current_results=results, lookback_hours=72
        )
        intel_v2_text = build_intelligence_report_v2(
            results, btc_regime=btc_regime, breadth=market_breadth(results),
            macro=macro, news=news, market_info=globals().get("_LAST_MARKET_INFO", {}) or {},
            previous_results=_intel_prev, cycle="DAILY16",
        )
        _p, s, e = send_report(intel_v2_text)
        if IRAN_MULTI_ASSET_ENABLED and IRAN_REPORT_DAILY16:
            send_iran_markets_report_v3(metal_results)
        sent_total += s; errors.extend(e)
        _daily16_diag("INTELLIGENCE_V2", s, e)
    except Exception as ex:
        _e = f"INTELLIGENCE_V2_DAILY16: {ex}"
        errors.append(_e)
        _daily16_diag("INTELLIGENCE_V2", 0, [_e])
...
def main():
    ...
    try:
        ...
        if daily_cycle:
            ...
            # Iran Markets PNG Dashboard — generated from the same fresh Data Plane.
            try:

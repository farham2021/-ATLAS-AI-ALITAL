...
def _v3_rows(kind,limit=300):
    ...
    if not STORE.enabled:return []
    tabs={"fx":["atlas_iran_fx_snapshots","atlas_market_snapshots"],"tse":["atlas_tse_snapshots","atlas_market_snapshots"],
          "funds":["atlas_fund_snapshots","atlas_market_snapshots"],"metals":["atlas_hourly_signal_snapshots","atlas_market_snapshots"]}.get(kind,["atlas_market_snapshots"])
    out=[]
    for tab in tabs:
        try:
...
def build_iran_multi_asset_snapshot_v3(metal_results=None):
    ...
    specs=[("USD_IRR",("USD","USD_IRR","USD_FREE","DOLLAR")),("USDT_IRR",("USDT","USDT_IRR")),
      ...
      ("TSE_TOTAL",("TSE_TOTAL","TEDPIX","TSE")),("TSE_EQUAL",("TSE_EQUAL","EQUAL_WEIGHT"))]
...
def _v3_funds():
    out=[]
    for row in _v3_rows("funds"):
        p=row.get("payload",{}) if isinstance(row,dict) else {}
        if isinstance(p,str):
            try:p=json.loads(p)
            except Exception:p={}
        p=p if isinstance(p,dict) else {};typ=str(p.get("fund_type") or row.get("fund_type") or "").upper()
        if not any(k in typ for k in ("FIXED","GOLD","درآمد","طلا")):continue
        price=safe_float(p.get("price") or row.get("price"));nav=safe_float(p.get("nav") or row.get("nav"))
        out.append({"name":p.get("name") or row.get("name") or row.get("symbol") or "FUND",
          "yield":safe_float(p.get("effective_yield") or p.get("yield_effective")),
          "premium":((price/nav)-1)*100 if price and nav else None,"liquidity":safe_float(p.get("trade_value") or p.get("value"))})
    return out
...
def build_iran_markets_report_v3(metal_results=None):
    ...
    if not fs:lines.append("NAV/effective-yield data missing — WAIT; no fabricated yield.")
    else:
        fs=sorted(fs,key=lambda x:((x["yield"] is not None),x["yield"] or -1,x["liquidity"] or 0),reverse=True)
        for x in fs[:8]:lines.append(f"{x['name']}: yield={fmt_num(x['yield'],2)}% | premium={fmt_num(x['premium'],2)}% | liquidity={fmt_num(x['liquidity'],0)}")
...
def build_iran_visual_dashboard(filename="atlas_iran_markets.png"):
    ...
    try:
        ...
        # Funds
        au=pnl(gs[12:18,9:19],"GOLD / FIXED-INCOME FUNDS");au.axis("off")
        fs=sorted(fund_rows,key=lambda x:(x.get("yield") is not None,x.get("yield") or -1,x.get("liquidity") or 0),reverse=True)[:9]
        if not fs:au.text(.5,.5,"FUND NAV / YIELD DATA MISSING",ha="center",transform=au.transAxes,color=amber,fontsize=11,weight="bold")
        else:
            for i,x in enumerate(fs):
                y=.90-i*.095
                au.text(.03,y,str(x.get("name"))[:18],transform=au.transAxes,color=fg,fontsize=7.7)
                au.text(.62,y,f"Y {fmt_num(x.get('yield'),2)}%",transform=au.transAxes,color=green,fontsize=7.5)
                au.text(.97,y,f"P/N {fmt_num(x.get('premium'),2)}%",ha="right",transform=au.transAxes,color=amber,fontsize=7.5)
        ...
        lines=[
          ...
          f"Funds tracked: {len(fund_rows)}",
          "Fail-closed data policy",
          "No fabricated NAV / yield",

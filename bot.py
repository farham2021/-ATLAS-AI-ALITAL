# ============================================================
# ATLAS AI v11.2 — UNIFIED TWO-ENGINE DECISION ENGINE
# ============================================================
...
VERSION = "ATLAS v11.2 UNIFIED TWO-ENGINE"
...
V11_2_MAX_REPORT_CHARS = int(os.environ.get("ATLAS_MAX_REPORT_CHARS", "8000"))
...
# ============================================================
# TELEGRAM — WITH RATE LIMIT HANDLING (اصلاح شده)
# ============================================================
...
def split_telegram(text, max_chars=3900):
    ...
    if not text:
        return []
    # بررسی وجود V11_2_MAX_REPORT_CHARS
    max_report_chars = globals().get('V11_2_MAX_REPORT_CHARS', 8000)
    if len(text) > max_report_chars:
        append_changelog(
            "REPORT_SIZE",
            None,
            None,
            f"Report exceeded safety limit ({len(text)} chars); truncating safely."
        )
        text = text[:max_report_chars] + "\n\n⚠️ ATLAS v11.2: report truncated by safety limit."
...
def send_report(text):
    """ارسال گزارش به تلگرام با مدیریت خطا و لاگ دقیق"""
    print("📤 Starting send_report...")
    
    # بررسی تنظیمات اولیه
    if not TELEGRAM_TOKEN:
        error_msg = "TELEGRAM_TOKEN is not set"
        print(f"❌ {error_msg}")
        append_changelog("TELEGRAM", None, None, error_msg)
        return 0, 0, [error_msg]
    
    parts = split_telegram(text)
    print(f"📝 Report split into {len(parts)} parts")
    ...
    if TELEGRAM_CHAT_ID:
        ...
        print(f"📌 Primary chat: {TELEGRAM_CHAT_ID}")
    ...
    if TELEGRAM_GROUP_CHAT_ID and TELEGRAM_GROUP_CHAT_ID not in [d["id"] for d in destinations]:
        ...
        print(f"📌 Group chat: {TELEGRAM_GROUP_CHAT_ID}")
    
    if not destinations:
        error_msg = "No Telegram destination configured"
        print(f"❌ {error_msg}")
        append_changelog("TELEGRAM", None, None, error_msg)
        return len(parts), 0, [error_msg]
    ...
    for dest in destinations:
        ...
        for i, part in enumerate(parts, 1):
            ...
            try:
                ...
                dest_success = False
                errors.append(f"Telegram {dest_name}, part {i}: {e}")
                print(f"❌ {e}")
                break
    ...
    print(f"📊 send_report result: {sent} messages sent, {len(errors)} errors")
    return len(parts), sent, errors


# ============================================================
# تابع تست تلگرام (اضافه شده)
# ============================================================

def send_test_telegram():
    """ارسال پیام تست به تلگرام برای بررسی اتصال"""
    ...
    test_msg = (
        f"🧪 ATLAS AI v11.2 Test Message\n"
        f"Time: {now_tehran().strftime('%Y-%m-%d %H:%M:%S')}\n"
        f"Status: Bot is running successfully"
    )
    
    destinations = []
    if TELEGRAM_CHAT_ID:
        destinations.append(("PRIVATE_CHAT", TELEGRAM_CHAT_ID))
    if TELEGRAM_GROUP_CHAT_ID:
        destinations.append(("SUPERGROUP", TELEGRAM_GROUP_CHAT_ID))
    ...
    for name, chat_id in destinations:
        try:
            print(f"📤 Sending test to {name} ({chat_id})...")
            result = telegram_send_one(chat_id, test_msg)
            if result:
                print(f"✅ Test message sent to {name}")
                success = True
            else:
                print(f"❌ Failed to send test to {name}")
        except Exception as e:
            print(f"❌ Error sending test to {name}: {e}")
...
# ============================================================
# ATLAS v11.2 — SEPARATE 3H PRICE SNAPSHOT
# ============================================================
...
# ============================================================
# توابع CSV و BEST SETUP - افزوده شده به bot.py
# ============================================================
# این توابع در انتهای فایل bot.py و قبل از if __name__ == "__main__" اضافه می‌شوند
# بدون هیچ تغییری در ساختار و معماری برنامه
# ============================================================

def _best_setup_block(rows, title="🔥 BEST SETUP"):
    """
    پیدا کردن بهترین ستاپ اجرایی از بین ردیف‌های داده
    """
    executable = [
        r for r in (rows or [])
        if str(r.get("action", "")).upper() in (
            "BUY CONFIRMATION",
            "SELL CONFIRMATION",
            "EXECUTABLE",
        )
    ...
    if not executable:
        return (
            f"{title}: هیچ ستاپ اجرایی با R/R و هندسه معتبر "
            "در این اجرا تأیید نشد."
        )
    ...
    action = best.get("action", "EXECUTABLE")
    rr = best.get("rr_tp2") or best.get("rr") or 0
...
def generate_csv_report(results, top10=None, dynamic30=None):
    """
    تولید گزارش CSV از نتایج تحلیل
    """
    import csv
    import io
    import json
...
def send_csv_report(results, top10=None, dynamic30=None):
    """
    ارسال گزارش CSV به کانال‌های تلگرام
    """
    import json
    ...
    filename = (
        f"atlas_v11_2_"
        f"{shamsi(dt).replace('/', '')}_"
        f"{dt.strftime('%H%M%S')}.csv"
    )
    ...
    for chat_id in destinations:
        try:
            # استفاده از تابع _telegram_send_document اگر وجود داشته باشد
            if '_telegram_send_document' in globals():
                ...
                # ارسال فایل با استفاده از روش جایگزین
                _send_csv_as_document(chat_id, content, filename, caption)
...
def _send_csv_as_document(chat_id, content, filename, caption):
    """
    ارسال فایل CSV به عنوان سند تلگرام - تابع کمکی
    """
    import os
    import urllib.request
    import tempfile
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as response:
            ...
            data = json.loads(raw)
            if not data.get('ok'):
                raise RuntimeError(f"Telegram sendDocument failed: {data.get('description', 'Unknown error')}")
...
def send_csv_if_enabled(results, top10=None, dynamic30=None):
    """
    ارسال گزارش CSV در صورت فعال بودن متغیر محیطی ATLAS_SEND_CSV
    """
    send_csv = os.environ.get("ATLAS_SEND_CSV", "0").strip().lower() in ("1", "true", "yes")
    
    if not send_csv:
        return 0, 0, ["CSV sending disabled"]
    
    return send_csv_report(results, top10, dynamic30)


# ============================================================
# تابع کمکی برای استفاده در main (اختیاری)
# ============================================================

def _send_csv_optional(results, top10, dynamic30):
    """
    ارسال CSV در صورت فعال بودن - برای استفاده در main
    """
    send_csv = os.environ.get("ATLAS_SEND_CSV", "0").strip().lower() in ("1", "true", "yes")
    
    if not send_csv:
        print("📊 CSV report sending disabled (ATLAS_SEND_CSV=0)")
        return 0, 0, ["CSV sending disabled"]
...
V11_2_HEARTBEAT_ON_IDLE = os.environ.get("ATLAS_HEARTBEAT_ON_IDLE", "0").strip().lower() in ("1", "true", "yes")
...
def main():
    try:
        ...
        if do_analysis:
            ...
            # ارسال CSV در صورت فعال بودن
            csv_parts, csv_sent, csv_errors = _send_csv_optional(results, top10, dynamic30)
            total_sent += csv_sent
        ...
        if not do_analysis and not do_snapshot:
            msg = (
                f"{VERSION}: AUTO schedule idle at {now_tehran().strftime('%Y-%m-%d %H:%M:%S %Z')}. "
                "Next analysis/snapshot window will be selected automatically."
            )
            print(msg)
            append_changelog("AUTO_IDLE", None, None, msg)
            if V11_2_HEARTBEAT_ON_IDLE:
                hb = (
                    f"🟦 {VERSION}\n"
                    f"AUTO scheduler active — no analytical/snapshot task at this hour.\n"
                    f"Tehran: {now_tehran().strftime('%Y-%m-%d %H:%M:%S')}"
                )
                hparts, hsent, herrs = send_report(hb)
                total_sent += hsent
                all_errors.extend(herrs)
            return 0
        ...
        try:
            if TELEGRAM_TOKEN and (TELEGRAM_CHAT_ID or TELEGRAM_GROUP_CHAT_ID):
                alert=f"🚨 {VERSION} FAILED\nReason: {str(e)[:900]}\n\nCheck GitHub Actions log and changelog.txt.\nTelegram delivery is not considered successful unless at least one message is confirmed by the API."

...
def main():
    ...
    if BRS:
        try:
            ...
            # Prefer the normalized USDT_IRR row if mapping already produced it.
            brs_usdt_row=next((r for r in brsfx if r.get("symbol")=="USDT_IRR"),None)

            # If normalization missed it, inspect the raw BrsApi cryptocurrency payload.
            # This handles provider spellings such as USDT/Tether/تتر without inventing a price.
            if brs_usdt_row is None:
                try:
                    raw_gc=brsapi_json(BRS_GOLD_CURRENCY)
                    crypto_raw=raw_gc.get("cryptocurrency",[]) if isinstance(raw_gc,dict) else []
                    raw_usdt=_brs_usdt_record(crypto_raw)
                    if raw_usdt:
                        px=safe_float(raw_usdt.get("price"))
                        if px is not None and px > 0:
                            brs_usdt_row={
                                ...
                                "payload":{
                                    ...
                                    "name_en":raw_usdt.get("name_en"),
                                    "name":raw_usdt.get("name"),
                                    "unit":raw_usdt.get("unit"),
                                    "change_percent":safe_float(raw_usdt.get("change_percent"))
                                }
                            }
                except Exception as e:
            ...
            if brs_usdt_row:
                # Force canonical provider metadata even when the normalized row came from brsfx.
                p=brs_usdt_row.get("payload")
                p=p if isinstance(p,dict) else {}
                p["provider"]="brsapi"
                brs_usdt_row["payload"]=p
                ...
                st["usdt_price"]=safe_float(brs_usdt_row.get("price"))
                st["usdt_provider_symbol"]=p.get("provider_symbol") or p.get("source_symbol") or "USDT"
            else:
                st["brsapi_usdt_written"]=0
                st["usdt_provider"]="NOT_MAPPED"

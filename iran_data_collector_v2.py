...
def main():
    ...
    if BRS:
        try:
            ...
            # IMPORTANT: BrsApi cryptocurrency USDT is USD-denominated (~1.0), not USDT/IRR.
            # Never relabel that raw crypto quote as USDT_IRR.
            #
            # Use BrsApi's Iranian USD quote as the IRR leg and BrsApi USDT/USD as the
            # crypto leg: USDT_IRR = USDT_USD * USD_IRR. This keeps BrsApi authoritative
            # for both inputs and preserves dimensional correctness.
            brs_usdt_row=None
            try:
                ...
                usd_row=next((r for r in brsfx if r.get("symbol")=="USD_IRR"),None)
                usdt_usd=num(raw_usdt.get("price")) if raw_usdt else None
                usd_irr=num(usd_row.get("price")) if usd_row else None

                if raw_usdt and usdt_usd and usd_irr and usdt_usd>0 and usd_irr>0:
                    ...
                    brs_usdt_row={
                        ...
                        "payload":{
                            "provider":"brsapi",
                            "method":"cross_USDTUSD_x_USDIRR",
                            "provider_symbol":raw_usdt.get("symbol"),
                            "usdt_usd":usdt_usd,
                            "usd_irr":usd_irr,
                            "raw_usdt_unit":raw_usdt.get("unit"),
                            "raw_usd_payload":usd_row.get("payload") if isinstance(usd_row,dict) else None
                        }
                    }
                    ...
                    st["usdt_cross_missing"]="USDT_USD" if not usdt_usd else "USD_IRR"
            except Exception as e:
                st["brsapi_usdt_written"]=0

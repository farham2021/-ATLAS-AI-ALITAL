...
def brs_tse_rows(status=None):
    ...
    for x in records:
        ...
        p=num(first(
            x,"pClosing","price","last","close","pDrCotVal","lastPrice",
            "finalPrice","closingPrice","closing_price"
        ))
        if not sym or not p or p<=0:continue

        y=num(first(
            x,"priceYesterday","yesterday","py","yesterdayPrice",
            "yesterday_price","price_yesterday"
        ))
        pct=num(first(
            x,"pct","percent","priceChangePercent","change_percent",
            "changePercent","percentChange"
        ))
        if pct is None and y and y>0:pct=(p/y-1)*100

        value=num(first(
            x,"qTotCap","value","trade_value","tval","tradeValue",
            "totalTradeValue","total_value"
        )) or 0
        volume=num(first(
            x,"qTotTran5J","volume","tvol","tradeVolume",
            "totalTradeVolume","total_volume"
        )) or 0
        ...
        rows.append({
            ...
            "payload":{
                ...
                "pct":pct,"volume":volume,"trade_value":value,
                "raw":clean_raw(x)
            }
        })
    ...
    summary={
        "captured_at":ts,"symbol":"TSE_MARKET","value":tv,
        "payload":{
...
def brs_fund_rows(tse_rows, status=None):
    """Discover ETFs from AllSymbols and fetch BrsApi NAV safely."""
    if not BRS:return []
    candidates=[]
    for r in tse_rows:
        if r.get("symbol")=="TSE_MARKET":continue
        raw=(r.get("payload") or {}).get("raw") or {}
        hay=(" ".join(str(v) for v in raw.values())+" "+
             str((r.get("payload") or {}).get("name") or "")).lower()
    ...
    out=[];ts=now(); nav_seen=0; first_keys=None
    for r in candidates[:max_nav]:
        ...
        nav_seen+=len(recs)
        if recs and first_keys is None:
            first_keys=[k for k in recs[0].keys() if not k.startswith("__")][:40]
        rec=recs[0] if recs else None
        if not rec:continue

        nav=num(first(
            rec,"nav","NAV","navRedemption","redemptionNav","nav_redemption",
            "redemption_nav","cancelNav","cancel_nav","ابطال","nav_ebtal"
        ))
        issue=num(first(
            rec,"navIssuance","issuanceNav","nav_issuance","issuance_nav",
            "issueNav","issue_nav","صدور","nav_sodoor"
        ))
        # Some providers expose one generic NAV value.
        generic=num(first(rec,"navValue","nav_value","value"))
        chosen=nav or issue or generic
        price=num(r.get("price"))
        if chosen and chosen>0:
            prem=(price/chosen-1)*100 if price else None
            out.append({
                "captured_at":ts,"symbol":sym,
                "name":first(rec,"name","title","nameFa","name_fa") or
                       (r.get("payload") or {}).get("name"),
                "fund_type":"ETF","price":price,"nav":chosen,
                "payload":{
                    "provider":"brsapi_nav","price":price,"nav":chosen,
                    "nav_redemption":nav,"nav_issuance":issue,
                    "premium_pct":prem,"raw":clean_raw(rec)
                }
            })

    if status is not None:
        status["brsapi_nav_records_seen"]=nav_seen
        status["brsapi_nav_rows_mapped"]=len(out)
        if first_keys:
            status["brsapi_nav_sample_keys"]=first_keys
    return out
...
def main():
    ...
    fundrows=[]
    if BRS and tse:
        try:fundrows=brs_fund_rows(tse,st);st["fund_provider"]="brsapi_nav"

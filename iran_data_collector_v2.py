...
def brs_fund_rows(tse_rows, status=None):
    ...
    out=[];ts=now(); nav_seen=0; key_shapes=[]; response_types=set()
    nav_requests=0; nav_errors=0; nav_error_samples=[]
    for r in candidates[:max_nav]:
        sym=str(r.get("symbol") or "")
        try:
            d=brs_get("/Tsetmc/Nav.php",{"l18":sym},retries=1)
            nav_requests+=1
            response_types.add(type(d).__name__)
        except Exception as e:
            nav_errors+=1
            if len(nav_error_samples)<5:
                # Error text may contain HTTP status/provider message, but never
                # include the API key because brs_get does not put it in errors.
                nav_error_samples.append({"symbol":sym,"error":str(e)[:500]})
            continue
        recs=list(walk_records(d))
        nav_seen+=len(recs)
        for rr in recs:
            shape=tuple(k for k in rr.keys() if not k.startswith("__"))
            if shape and shape not in key_shapes:
                key_shapes.append(shape)

        # Search every nested record for a NAV field; do not assume record 0.
        ...
        chosen=next((v for v in (nav,issue,generic) if v is not None and v>0),None)
        price=num(r.get("price"))
        prem=(price/chosen-1)*100 if price and chosen else None
        out.append({
            "captured_at":ts,"symbol":sym,
            "name":first(selected,"name","title","nameFa","name_fa") or
    ...
    if status is not None:
        ...
        status["brsapi_nav_error_samples"]=nav_error_samples
        status["brsapi_nav_response_types"]=sorted(response_types)
        status["brsapi_nav_records_seen"]=nav_seen
        status["brsapi_nav_rows_mapped"]=len(out)
        status["brsapi_nav_key_shapes"]=[list(s)[:50] for s in key_shapes[:12]]
    return out

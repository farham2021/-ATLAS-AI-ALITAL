...
def brs_tse_rows():
    ...
    for x in walk_records(d):
        sym=str(first(x,"l18","symbol","lVal18AFC","ticker","Symbol","insCode") or "").strip()
        name=str(first(x,"l30","name","lVal30","Name","companyName") or "").strip()
        p=num(first(x,"pClosing","price","last","close","pDrCotVal","lastPrice","finalPrice"))
        ...
        rows.append({"captured_at":ts,"symbol":sym,"price":p,"value":value,"payload":{"provider":"brsapi_allsymbols","name":name,"price":p,"pct":pct,"volume":volume,"trade_value":value,"raw":clean_raw(x)}})
    if not rows:return []
...
def brs_fund_rows(tse_rows):
    ...
    for r in candidates[:max_nav]:
        ...
        if chosen:
            ...
            out.append({"captured_at":ts,"symbol":sym,"name":first(rec,"name","title") or (r.get("payload") or {}).get("name"),"fund_type":"ETF","price":price,"nav":chosen,"payload":{"provider":"brsapi_nav","price":price,"nav":chosen,"nav_redemption":nav,"nav_issuance":issue,"premium_pct":prem,"raw":clean_raw(rec)}})
    return out

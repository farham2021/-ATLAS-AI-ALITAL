...
TSETMC_TIMEOUT=int(os.getenv("ATLAS_TSETMC_TIMEOUT","7"))
...
def supabase_headers(content=False):
    """Support both legacy JWT service_role keys and new sb_secret_* keys."""
    h={"apikey":KEY}
    if KEY.startswith("eyJ"):
        h["Authorization"]=f"Bearer {KEY}"
    if content:
        h.update({"Content-Type":"application/json","Prefer":"return=minimal"})
    return h

def http_error_detail(e):
    """Return a safe provider error without leaking API keys or request URLs."""
    ...
    try:
        ...
        pass
    parts=[]
    if code is not None: parts.append(f"HTTP {code}")
    if reason: parts.append(str(reason))
    if body: parts.append(body.replace("\n"," ").replace("\r"," "))
    return " | ".join(parts) or str(e)

def post(table, rows):
    if not rows:return 0
    req=urllib.request.Request(
        f"{SB}/rest/v1/{table}", data=json.dumps(rows,ensure_ascii=False).encode(), method="POST",
        headers=supabase_headers(content=True))
...
def brs_get(path, params=None, retries=2):
    ...
    try:
        ...
        raise RuntimeError(f"BrsApi {path}: {http_error_detail(e)}") from None

def walk_records(obj):
    """Yield dict records from common BrsApi JSON envelopes without assuming one schema."""
    if isinstance(obj,list):
        for x in obj:
            if isinstance(x,dict): yield x
    elif isinstance(obj,dict):
        for k in ("data","result","results","items","response"):
            v=obj.get(k)
            if isinstance(v,list):
                for x in v:
                    if isinstance(x,dict): yield x
                return
        # Some APIs return a dict keyed by symbol/name.
        vals=list(obj.values())
        if vals and all(isinstance(x,dict) for x in vals):
            for x in vals: yield x
        else:
            yield obj
...
def brs_gold_currency_rows():
    ...
    for x in walk_records(d):
        name=str(first(x,"symbol","name","title","key","slug","l18") or "").strip()
        norm=name.lower().replace("_","").replace("-","").replace(" ","")
        sym=None
        for a,target in aliases.items():
            if a.lower().replace(" ","") in norm: sym=target;break
        p=num(first(x,"price","value","last","close","pClosing","pDrCotVal"))
        if sym and p and p>0:
            out.append({"captured_at":ts,"symbol":sym,"price":p,"payload":{"provider":"brsapi_gold_currency","source_name":name,"price":p,"raw":x}})
    return out
...
def brs_commodity_rows():
    ...
    for x in walk_records(d):
        ...
        p=num(first(x,"price","value","last","close"))
        if sym and p and p>0:
            out.append({"captured_at":ts,"symbol":sym,"asset":asset,"price":p,"payload":{"provider":"brsapi_commodity","source_name":name,"price":p,"unit":unit,"raw":x}})
    return out
...
def brs_tse_rows():
    ...
    for x in walk_records(d):
        sym=str(first(x,"l18","symbol","lVal18AFC","ticker") or "").strip()
        name=str(first(x,"l30","name","lVal30") or "").strip()
        p=num(first(x,"pClosing","price","last","close","pDrCotVal"))
        if not sym or not p:continue
        y=num(first(x,"priceYesterday","yesterday","py")); pct=num(first(x,"pct","percent","priceChangePercent"))
        if pct is None and y:pct=(p/y-1)*100
        value=num(first(x,"qTotCap","value","trade_value","tval")) or 0
        volume=num(first(x,"qTotTran5J","volume","tvol")) or 0
        if pct is not None:adv+=pct>0;dec+=pct<0;unch+=pct==0
        tv+=value;vol+=volume;leaders.append((value,sym,p,pct))
        rows.append({"captured_at":ts,"symbol":sym,"price":p,"value":value,"payload":{"provider":"brsapi_allsymbols","name":name,"price":p,"pct":pct,"volume":volume,"trade_value":value,"raw":x}})
    if not rows:return []
    breadth=adv/(adv+dec)*100 if adv+dec else None
    summary={"captured_at":ts,"symbol":"TSE_MARKET","value":tv,"payload":{"provider":"brsapi_allsymbols","trade_value":tv,"volume":vol,"advance":adv,"decline":dec,"unchanged":unch,"breadth":breadth,"leaders":[{"symbol":s,"price":p,"pct":pc,"value":v} for v,s,p,pc in sorted(leaders,reverse=True)[:15]]}}
    return [summary]+sorted(rows,key=lambda r:r.get("value") or 0,reverse=True)[:250]

def brs_fund_rows(tse_rows):
    """Discover likely ETFs from AllSymbols, then fetch NAV only for a bounded set."""
    if not BRS:return []
    candidates=[]
    for r in tse_rows:
        if r.get("symbol")=="TSE_MARKET":continue
        raw=(r.get("payload") or {}).get("raw") or {}; text=(str(raw)+" "+str((r.get("payload") or {}).get("name") or "")).lower()
        if any(k in text for k in ("صندوق","etf","fund")):
            candidates.append(r)
    max_nav=int(os.getenv("ATLAS_BRSAPI_NAV_MAX_SYMBOLS","20")); out=[];ts=now()
    for r in candidates[:max_nav]:
        sym=str(r.get("symbol") or "")
        try:d=brs_get("/Tsetmc/Nav.php",{"l18":sym},retries=1)
        except Exception:continue
        rec=next(iter(walk_records(d)),None)
        if not rec:continue
        nav=num(first(rec,"nav","NAV","navRedemption","redemptionNav","ابطال","nav_ebtal"))
        issue=num(first(rec,"navIssuance","issuanceNav","صدور","nav_sodoor"))
        price=num(r.get("price")); chosen=nav or issue
        if chosen:
            prem=(price/chosen-1)*100 if price else None
            out.append({"captured_at":ts,"symbol":sym,"name":first(rec,"name","title") or (r.get("payload") or {}).get("name"),"fund_type":"ETF","price":price,"nav":chosen,"payload":{"provider":"brsapi_nav","price":price,"nav":chosen,"nav_redemption":nav,"nav_issuance":issue,"premium_pct":prem,"raw":rec}})
    return out
...
def navasan_rows():
    ...
    try:
        ...
        raise RuntimeError(f"Navasan: {http_error_detail(e)}") from None
...
def tsetmc():
    ...
    w=http_json("https://cdn.tsetmc.com/api/ClosingPrice/GetMarketWatch?"+urllib.parse.urlencode(qs),timeout=TSETMC_TIMEOUT,retries=1)
    w=w.get("marketwatch",[]) if isinstance(w,dict) else []
    try:c=http_json("https://cdn.tsetmc.com/api/ClientType/GetClientTypeAll",timeout=TSETMC_TIMEOUT,retries=1);c=c.get("clientTypeAllDto",[]) if isinstance(c,dict) else []
    except Exception:c=[]
...
def main():
    ...
    st={"brsapi_key_configured":bool(BRS),"supabase_key_type":("legacy_jwt" if KEY.startswith("eyJ") else "new_secret" if KEY.startswith("sb_secret_") else "other")}
    ...
    if BRS:
        try:
            ...
            if not brsfx:
                try:
                    probe=brs_get("/Market/Gold_Currency.php",retries=1)
                    st["brsapi_gold_currency_response_type"]=type(probe).__name__
                    st["brsapi_gold_currency_records_seen"]=sum(1 for _ in walk_records(probe))
                    if isinstance(probe,dict): st["brsapi_gold_currency_top_keys"]=list(probe.keys())[:12]
                except Exception as e: st["brsapi_gold_currency_probe_error"]=str(e)
    ...
    tse=[]
    if BRS:
        try:tse=brs_tse_rows();st["tse_provider"]="brsapi" if tse else "brsapi_empty"

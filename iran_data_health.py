...
def sb_headers():
 h={"apikey":KEY}
 if KEY.startswith("eyJ"):h["Authorization"]=f"Bearer {KEY}"
 return h
def fetch(t):
 q=urllib.parse.urlencode({"select":"captured_at,symbol,price,payload","order":"captured_at.desc","limit":"500"});r=urllib.request.Request(f"{SB}/rest/v1/{t}?{q}",headers=sb_headers())
...
def main():
 rules=[("atlas_iran_fx_snapshots",{"USD_IRR","GOLD18","COIN_EMAMI"},9),("atlas_market_snapshots",{"XAGUSD","SILVER999_FAIR"},4),("atlas_tse_snapshots",{"TSE_MARKET"},30),("atlas_fund_snapshots",set(),36)]
 rep={};bad=[]
 for t,req,mx in rules:
  ...
  if t=="atlas_fund_snapshots" and not FUNDS_CONFIGURED:
   ...
   continue
  # Fund health is based on real ETF market snapshots. NAV is optional
  # enrichment and may be unavailable on the current BrsApi plan.
